...
#   make release-public FLAGS=--execute PUBLISHERS_VERIFIED=1  # skip the unverifiable publisher probe
#                                           # (PyPI's JSON API hides publisher metadata; set after
#                                           # confirming the bindings once in the PyPI settings UI)
...
#   DOGFOOD_SOURCE         — git_overlay (default) or package
...
# (also: --install-claude-stop-hook, --install-codex-stop-hook,
# --install-vscode-stop-hook, --install-grok-stop-hook). Never applied to
# `status` invocations.
...
# implementation note implementation note: installed hook-surface coherence gate. Fails on any
...
ensure-hooks-path: dogfood-link ## Wire scripts/hooks -> payload + rewire core.hooksPath to scripts/hooks/git
...
release-public: ## Orchestrate the public-release flow (dry-run by default); FLAGS=--execute to push/tag/publish after confirmation, PUBLISHERS_VERIFIED=1 to attest Trusted-Publisher bindings (probe is unverifiable via PyPI's public API), FLAGS=--json for machine-readable output
	python scripts/release_public.py $(FLAGS) $(if $(PUBLISHERS_VERIFIED),--publishers-verified)
...
dogfood: ## Install latest/TAG overlay or DOGFOOD_SOURCE=package PyPI overlay into this repo
	...
	if [ "$$source" != "git_overlay" ]; then \
	    echo "DOGFOOD_SOURCE must be 'git_overlay' or 'package' (got '$$source')" >&2; \
	    exit 2; \
	fi; \
