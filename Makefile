...
.PHONY: check-system
check-system: check-disk-space ## Run the workstate-system suite (plugin/skill/Make-target contracts)
...
.PHONY: check-all
check-all: check-disk-space ## Format + lint + mypy + tests across every package, then contract test
	$(MAKE) format-all
	$(MAKE) -C packages/mcp-workstate-handoff check-handoff
