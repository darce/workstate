...
| Server                   | Command line                                                |
| ------------------------ | ----------------------------------------------------------- |
| `workstate-handoff-mcp`      | `uvx mcp-workstate-handoff@0.12.4 --workspace-root . serve-stdio`      |
| `workstate-orchestrator-mcp` | `uvx mcp-workstate-orchestrator[bridge]@0.6.1 --workspace-root . serve-stdio` |

The pins above are kept in lockstep with the installer's
`DEFAULT_MCP_SERVERS` constant; `make check-mcp-pins` fails when
this table drifts from the shipped defaults.
