...
- PyPI publish credentials are **only needed for the local-`twine` fallback**
  (`~/.pypirc`, or `TWINE_USERNAME` / `TWINE_PASSWORD`, or the OS keyring). The
  primary Trusted-Publishing path needs no local token — PyPI auth happens via
  GitHub OIDC inside `release-publish.yml`.
...
Use `make release-package PKG=<pkg>` (→ `scripts/release.sh package`) only when
Trusted Publishing is unavailable or for an out-of-band single-package upload.
It runs `uvx --with keyring twine upload`, so it needs a PyPI token reachable
non-interactively — either stored in the OS keyring
(`keyring set https://upload.pypi.org/legacy/ __token__`), in `~/.pypirc`
(`[pypi]` `username = __token__`, `password = pypi-…`), or via
`TWINE_USERNAME=__token__` + `TWINE_PASSWORD=pypi-…`. Without one, the upload
fails with an interactive-prompt `EOFError` in a non-TTY context. After a
fallback upload, add/confirm the Trusted Publisher so the next release uses the
primary path.
