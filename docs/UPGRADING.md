...
| Skill content under `.claude/skills/<slug>/SKILL.md` (Claude-only)    | Neutral source `skills/<slug>/{skill.yaml,body.md}` generates per-agent surfaces |
...
2. **Drop hand-supplied `--mcp-servers` files** unless you carry
   ...
   startup contract for Codex, VS Code, and Claude. To keep a custom
...
4. **Refresh per-agent surfaces.** The neutral skill layout
   (`skills/<slug>/{skill.yaml,body.md}`) regenerates the four
   per-agent surfaces (`.claude/skills`, `.claude/commands`,
   `.github/prompts`, `.codex/skills`) on every install. If your
   target had hand-edited any of these, expect drift on the first
