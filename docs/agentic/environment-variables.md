...
> - `AGENT_HANDOFF_*` / `AGENTIC_*` — **deprecated** one-release aliases for
>   the corresponding `WORKSTATE_HANDOFF_*` / `WORKSTATE_*` names.
> - `WORKSTATE_*` — **deprecated** compaction aliases that predate the package
>   prefix consolidation; read after the `AGENT_HANDOFF_COMPACTION_*` aliases.
...
`packages/workstate-system/docs/agentic/rules/development-workflow.md`.
...
## `WORKSTATE_*` — deprecated aliases

These three names are read as one-release aliases by `CompactionSettings.from_env()`
and trigger a `<name> is deprecated; rename to <canonical>` line on stderr when
they are the resolved source. Schedule for removal once at least one tagged
release ships with the new prefix in place.

| Legacy name | Canonical replacement | Removed in |
|---|---|---|
| `WORKSTATE_COMPACTION_DISABLED` | `WORKSTATE_HANDOFF_COMPACTION_DISABLED` | TBD (one release after this lands) |
| `WORKSTATE_COMPACTION_MIN_NEW_TURNS` | `WORKSTATE_HANDOFF_COMPACTION_MIN_NEW_TURNS` | TBD |
| `WORKSTATE_COMPACTION_MIN_NEW_TOKENS` | `WORKSTATE_HANDOFF_COMPACTION_MIN_NEW_TOKENS` | TBD |
...
| Name | Use |
|---|---|
| `WORKSTATE_DISABLE_PYTEST_PATH_GUARD` | Test harness escape hatch for `test_pytest_path_guard.py`; legacy `AGENTIC_DISABLE_PYTEST_PATH_GUARD` is accepted during cutover. |
