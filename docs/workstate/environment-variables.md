...
| `WORKSTATE_HANDOFF_COMPACTION_MIN_NEW_TOKENS` | int (≥0) | `50000` | Sole compaction trigger gate: a compaction row is written once the encoded new-turn token count reaches the threshold (code default `50000` ⇒ roughly one compaction per 50k new tokens). `0` means never fire the token gate (skip every turn); use `WORKSTATE_HANDOFF_COMPACTION_DISABLED` to silence compaction entirely. | `workstate-system/scripts/hooks/compact-session.py` |
