...
| `WORKSTATE_REINJECT_AB` | `0` / `1` | `0` (off) | When `1`, `reinject-context` assigns each session to a SELECTION arm via deterministic `session_id` parity — `treatment` = semantic top-K (arm B), `control` = current selection (arm A). Both arms emit a block and record `arm` + `emitted_chars`; the arm overrides `WORKSTATE_REINJECT_SEMANTIC`. Supersedes the implementation note emit/suppress window (control no longer suppresses). Missing/unknown `session_id` records `arm=NULL` so unattributable rows do not pollute one arm. | `workstate-system/scripts/hooks/reinject-context.py`, `harness-protocol.yaml` |
