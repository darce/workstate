...
- **MCP server identity cutover — `agent-handoff-mcp` → `workstate-handoff-mcp`
  (implementation note Slice B).** The canonical registered server name is now
  `workstate-handoff-mcp`; bootstrap dedups and forward-rewrites a stale
  `agent-handoff-mcp` entry so only one canonical registration survives
