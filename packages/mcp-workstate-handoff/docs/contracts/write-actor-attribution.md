# Write-Actor Attribution Policy (WORKSTATE-REF-52)

This doc records the source-of-truth rule for `_resolve_write_actor` after the WORKSTATE-REF-52 charter inversion (implementation note resolver precedence collapse). It is the audit gate referenced by the contract test in `tests/test_write_actor_attribution_contract.py`.
...
2. Category-(b) callers that quietly missed migration produced wrong-cwd attribution silently — exactly the WORKSTATE-REF-52 charter gap surfaced by finding `WORKSTATE52-S1S3-BR-01`.
...
After WORKSTATE-REF-52 implementation note, `handoff_state.target_worktree_path` is **no longer the source of truth** for the worktree path. The resolver derives it via `git worktree list --porcelain` keyed by canonical `target_branch`. The stored column remains in the schema for migration headroom and is dropped in a follow-up WORKSTATE-REF once one release of derive-only behavior has shipped.
