# WORKSTATE-REF Compaction vs. Host-Harness Compaction

This explainer answers a recurring operator question: *Should I run WORKSTATE-REF's
...
different problems.** WORKSTATE-REF compaction is **not** a replacement for
...
| | Host-harness compaction | WORKSTATE-REF compaction |
|---|---|---|
...
| When it fires | When the harness's context-pressure heuristic crosses an internal threshold | When the WORKSTATE-REF advisory's token/char thresholds are crossed (or on operator command) |
...
- **WORKSTATE-REF compaction** runs *outside* the model loop. The
...
- **Host-harness compaction** consumes the live model context — the same
  ...
  `handoff_state` rows that WORKSTATE-REF tracks (decisions, blockers, slice
  closures, review findings).
- **WORKSTATE-REF compaction** consumes the harness transcript file plus the
...
- **Host-harness compaction** produces prose. The conversation is collapsed
  ...
  goes with it.
- **WORKSTATE-REF compaction** produces a typed `StructuredSummary` (decisions,
...
On a successful WORKSTATE-REF record, operator-facing surfaces keep
...
describe the durable WORKSTATE-REF artifact; they are not a claim that the host
...
- **Host-harness compaction** fires on the harness's internal
  ...
  outside the harness.
- **WORKSTATE-REF compaction** fires when the advisory's token or char thresholds
  are crossed (see `packages/workstate-system/docs/agentic/contracts/harness-protocol.yaml`
  — defaults 70,000 tokens / 280,000 chars after WORKSTATE-REF-61). The
  thresholds are operator-tunable per-deployment (WORKSTATE-REF-63 overlay/env
...
prints a "reduction" percentage. It is easy to read that number as "WORKSTATE-REF
...
2. Leave the WORKSTATE-REF advisory enabled. It fires earlier (at the configured
...
If you need to silence the WORKSTATE-REF advisory in a specific environment, see
the README's *Disabling WORKSTATE-REF compaction* subsection — but understand it
silences only the WORKSTATE-REF advisory and Stop-hook write. The host
...
- [`packages/workstate-system/docs/agentic/contracts/harness-protocol.yaml`](../../../workstate-system/docs/agentic/contracts/harness-protocol.yaml)
