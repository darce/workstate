#!/usr/bin/env bash
# release_mcp_package.sh — tag a new release of mcp-workstate-handoff on the
...
INSTALL_URL="git+https://github.com/${REPO_SLUG}.git@mcp-workstate-handoff-v${VERSION}#subdirectory=packages/mcp-workstate-handoff"

echo "==> Releasing mcp-workstate-handoff v${VERSION}"
...
git -C "$REPO_ROOT" tag "mcp-workstate-handoff-v${VERSION}"
git -C "$REPO_ROOT" push origin HEAD
git -C "$REPO_ROOT" push origin "mcp-workstate-handoff-v${VERSION}"

echo ""
echo "Released: mcp-workstate-handoff-v${VERSION}"
