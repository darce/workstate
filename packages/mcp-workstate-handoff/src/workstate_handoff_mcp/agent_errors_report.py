"""Agent-error harvest report + export (WS-ERRTEL-01 / implementation note implementation note).
...
- ``errors_report`` clusters rows by ``(error_class, package_name)``
  ...
  sample designed to seed one WORKSTATE-REF-* task per cluster. Local mode
...
def build_errors_report(rows: list[dict]) -> dict:
    """Cluster rows by ``(error_class, package_name)``.

    Per cluster: row/occurrence counts, distinct repo instances,
    package-version range, first/last seen, and a representative
    sample — the row with the highest ``occurrence_count`` (latest
    ``last_seen_at`` breaks ties), i.e. the variant a WORKSTATE-REF-* task
    should reproduce first. Clusters sort busiest-first.
    """
    clusters: dict[tuple, list[dict]] = {}
    for row in rows:
        key = (str(row.get("error_class") or "other"), row.get("package_name"))
        clusters.setdefault(key, []).append(row)

    rendered = []
    for (error_class, package_name), members in clusters.items():
        sample_row = max(
            members,
    ...
    rendered.sort(key=lambda c: (-c["occurrence_count"], c["error_class"], c["package_name"] or ""))
