...
def _resolve_doctor_stdio_timeout(default: float = _DOCTOR_STDIO_TIMEOUT_DEFAULT) -> float:
        ...
        with ThreadPoolExecutor(max_workers=1) as _probe_pool:
            ...
            try:
                ...
                if stdio_result["ok"]:
                    ...
                    # A captured probe failure (e.g. handshake timeout) is a hard
                    # gate in strict mode, mirroring the CLI-probe gate below. The
                    # raise is re-caught by the broad clause, which waits on the
                    # CLI future before re-raising.
                    raise RuntimeError(stdio_result.get("error", "stdio startup probe failed"))
                else:
