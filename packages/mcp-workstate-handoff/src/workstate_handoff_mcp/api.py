...
TOOL_DESCRIPTIONS: dict[str, str] = {
    ...
    "archive": (
        ...
        "archive storage; operation='gc' to bulk-archive status=done MAINT-PLANNING-REVIEW-* "
...
class RecordErrorEvent(BaseModel):
    ...
    package_name: Annotated[
        str | None,
        Field(description="Optional workstate package implicated in the error."),
    ] = None
...
class ArchiveOpArchive(BaseModel):
    ...
    cascade_maint_review: Annotated[
        bool,
        Field(description="Also archive linked MAINT-PLANNING-REVIEW-* children of this task."),
    ] = False
    ...
    rationale: Annotated[
        ...
        Field(
            description=(
                ...
                "See docs/workstate/templates/slice-complete-template.md."
            )
        ),
    ] = None,
    actor: ActorParam = None,
...
def _resolve_doctor_deadline_seconds(default: float = _DOCTOR_DEADLINE_DEFAULT) -> float:
    ...
    Override with ``WORKSTATE_DOCTOR_DEADLINE_SECONDS``. A non-positive or
    unparseable value falls back to the default so a bad env can never disable
    the bound.
    """
    raw = resolve_env_alias("WORKSTATE_DOCTOR_DEADLINE_SECONDS", default="").strip()
    if not raw:
        return default
    try:
        value = float(raw)
...
def _resolve_doctor_stdio_timeout(
    ...
    *,
    ...
    Override with ``WORKSTATE_HANDOFF_DOCTOR_STDIO_TIMEOUT``. A non-positive or
    unparseable value falls back to the default so a bad env can never disable
    the bound. When ``deadline_cap`` is set, the returned value never exceeds it.
    """
    raw = resolve_env_alias("WORKSTATE_HANDOFF_DOCTOR_STDIO_TIMEOUT", default="").strip()
    if not raw:
        timeout_s = default
    else:
        try:
    ...
    try:
        ...
        return {
            ...
            "remediation": (
                ...
                "WORKSTATE_HANDOFF_DOCTOR_STDIO_TIMEOUT (seconds)."
            ),
        }
    ...
    *,
    ...
    try:
        ...
        return {
            ...
            "remediation": (
                "The `mcp-workstate-handoff state` CLI command did not finish within the "
                f"doctor deadline ({budget}). Check package install and workspace health, or "
                "raise WORKSTATE_DOCTOR_DEADLINE_SECONDS."
            ),
        }
...
def run_doctor(config: RuntimeConfig) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # mode (WORKSTATE_HANDOFF_DOCTOR_STRICT=1) any probe failure re-raises.
        ...
        with ThreadPoolExecutor(max_workers=1) as _probe_pool:
            ...
            try:
                ...
                cli_result = {
                    ...
                    "remediation": (
                        "The `mcp-workstate-handoff state` CLI probe did not return within "
                        "the doctor deadline. Check package install and workspace health, "
                        "or raise WORKSTATE_DOCTOR_DEADLINE_SECONDS."
                    ),
                }
