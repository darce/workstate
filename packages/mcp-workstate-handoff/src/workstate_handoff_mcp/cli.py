...
def _dispatch_compaction(args: argparse.Namespace) -> Any:
    ...
    if args.operation == "record":
        ...
        if args.task_ref is not None:
            payload["task_ref"] = args.task_ref
    else:  # disable | enable | status (WORKSTATE-REF-67)
        if args.task_ref is not None:
            payload["task_ref"] = args.task_ref
...
def _dispatch_set_handoff_state(args: argparse.Namespace) -> Any:
    ...
    WORKSTATE-REF-51 implementation note: the auto-dispatch can't represent the actor block because
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    # WORKSTATE-REF-82 implementation note: parity with sibling CLIs (compaction_cli, plan_cli).
    ...
    # WS-ERRTEL-01 implementation note: direct-path agent-error write for harness
    ...
    # WS-ERRTEL-01 implementation note: harvest surfaces. Special-cased for the same
...
# WORKSTATE-REF-82 implementation note: without this guard, `python -m workstate_handoff_mcp.cli`
