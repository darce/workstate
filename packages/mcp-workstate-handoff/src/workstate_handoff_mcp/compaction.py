...
class CompactionRecordReceipt(BaseModel):
    ...
    lines 126-127 (WORKSTATE-REF-007 fix) on slimmed ``input_chars`` only — ``raw_input_bytes``
...
def _truncate_residual(residual: str | None) -> str | None:
    ...
    clipped = residual[:PROSE_RESIDUAL_SOFT_LIMIT_CHARS] + f"... [truncated; {omitted} chars omitted]"
    assert len(clipped) <= PROSE_RESIDUAL_HARD_LIMIT_CHARS
    return clipped
...
def _slim_jsonl_record(record: dict[str, object]) -> str | None:
    ...
    if record_type in ("user", "assistant"):
        ...
        if not isinstance(message, dict):
            return None
        parts = _extract_text_from_content(message.get("content"))
        if not parts:
            return None
...
def compact_session(
    ...
    transcript = _read_transcript(transcript_path_obj, settings=resolved_settings)
