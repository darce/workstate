...
# implementation note B4: canonical names are now WORKSTATE_HANDOFF_COMPACTION_*. The
# previous canonical AGENT_HANDOFF_COMPACTION_* and the older WORKSTATE_COMPACTION_*
# are both read for one release as ordered legacy fallbacks (newer-legacy first),
# each emitting a one-shot rename warning pointing at the WORKSTATE_ name.
_COMPACTION_ENV_ALIASES: tuple[tuple[str, str, tuple[str, ...]], ...] = (
    (
        "disabled",
    ...
    (
        ...
        ("AGENT_HANDOFF_COMPACTION_MIN_NEW_TURNS", "WORKSTATE_COMPACTION_MIN_NEW_TURNS"),
    ),
    (
        "min_new_tokens",
        "WORKSTATE_HANDOFF_COMPACTION_MIN_NEW_TOKENS",
        ("AGENT_HANDOFF_COMPACTION_MIN_NEW_TOKENS", "WORKSTATE_COMPACTION_MIN_NEW_TOKENS"),
    ),
)
...
def _env_disables_compaction(env: Mapping[str, str]) -> bool:
    ...
    if raw == "":
        raw = env.get("AGENT_HANDOFF_COMPACTION_DISABLED", "")
    if raw == "":
        raw = env.get("WORKSTATE_COMPACTION_DISABLED", "")
...
class CompactionSettings(BaseModel):
    ...
    falls back to the legacy ``AGENT_HANDOFF_COMPACTION_*`` then the older
    ``WORKSTATE_COMPACTION_*`` names (newer-legacy first) with a captured
    ...
    def from_env(cls, env: Mapping[str, str] | None = None) -> "CompactionSettings":
        ...
        for field_name, canonical, legacy_names in _COMPACTION_ENV_ALIASES:
        ...
        return cls.model_validate({"legacy_warnings": tuple(warnings), **fields})
...
def resolve_compaction_disabled(
    *,
    ...
    Precedence (first match wins):
      1. ``WORKSTATE_HANDOFF_COMPACTION_DISABLED`` env var; legacy
         ``AGENT_HANDOFF_COMPACTION_DISABLED`` then ``WORKSTATE_COMPACTION_DISABLED``
         aliases preserved for resolution.
