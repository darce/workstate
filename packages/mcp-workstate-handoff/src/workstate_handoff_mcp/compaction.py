...
def slice_new_turn_transcript(transcript: str, *, since_turn: int) -> str:
    ...
    if _uses_jsonl_turn_accounting(transcript):
        return "".join(
            line for ordinal, line in _iter_jsonl_turn_records(transcript) if ordinal > since_turn
        )
    out: list[str] = []
    keeping = False
    for line in transcript.splitlines(keepends=True):
...
def _slim_jsonl_record(record: dict[str, object]) -> str | None:
    ...
    if record_type in ("user", "assistant"):
        ...
        if not isinstance(message, dict):
            return None
        parts = [
            _strip_harness_boilerplate(part)
            for part in _extract_text_from_content(message.get("content"))
        ]
