"""``python -m workstate_handoff_mcp.compaction_cli`` — implementation note implementation note.
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description="Manual compact-now launcher (implementation note implementation note).",
    )
    ...
    parser.add_argument(
        "--task-ref",
        required=True,
        help="Task ref (e.g. WORKSTATE-REF-39) to attribute the compaction to.",
    )
    parser.add_argument(
        "--transcript",
        default=None,
