...
_PACKAGE_REFERENCE_CONTRACT_RELATIVE_PATH = Path(
    "packages/workstate-system/docs/agentic/contracts/harness-protocol.yaml"
)
_OVERLAY_MANIFEST_PATH = Path(".workstate-overlay.json")
_CANONICAL_HARNESSES: tuple[str, ...] = ("claude-code", "codex", "vscode", "manual")
_HARNESS_ALIASES: dict[str, str] = {"cursor": "vscode"}

CompactionContractHarness = Literal["claude-code", "codex", "vscode", "manual"]
...
ThresholdSource = Literal["env", "overlay", "contract"]
# implementation note B4: WORKSTATE_HANDOFF_* is canonical; AGENT_HANDOFF_* is read for
# one release as a legacy fallback with a one-shot rename warning.
_ENV_VAR_TOKENS = "WORKSTATE_HANDOFF_COMPACTION_THRESHOLD_TOKENS"
_ENV_VAR_CHARS = "WORKSTATE_HANDOFF_COMPACTION_THRESHOLD_CHARS"
_LEGACY_ENV_VAR_TOKENS = "AGENT_HANDOFF_COMPACTION_THRESHOLD_TOKENS"
_LEGACY_ENV_VAR_CHARS = "AGENT_HANDOFF_COMPACTION_THRESHOLD_CHARS"
    ...
    *,
    ...
    def _resolve_knob(
        ...
        if raw_env is None or raw_env.strip() == "":
            legacy_raw = source_env.get(legacy_env_var)
            if legacy_raw is not None and legacy_raw.strip() != "":
                raw_env = legacy_raw
                used_var = legacy_env_var
                warnings.append(f"{legacy_env_var} is deprecated; rename to {env_var}")
    ...
    tokens, tokens_source = _resolve_knob(_ENV_VAR_TOKENS, _LEGACY_ENV_VAR_TOKENS, "tokens", contract.threshold_tokens)
    chars, chars_source = _resolve_knob(_ENV_VAR_CHARS, _LEGACY_ENV_VAR_CHARS, "chars", contract.threshold_chars)
