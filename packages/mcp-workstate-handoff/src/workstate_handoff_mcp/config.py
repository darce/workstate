...
class RuntimeConfig:
    ...
    def for_workspace(
        ...
        *,
        ...
        if current_task_auto_regen is None:
            env_value = resolve_env_alias("WORKSTATE_HANDOFF_CURRENT_TASK_AUTO_REGEN")
        ...
        if finding_lifecycle_states_enabled is None:
            flag_env = resolve_env_alias("WORKSTATE_HANDOFF_FINDING_LIFECYCLE_STATES")
    ...
    def from_args(cls, args: object) -> RuntimeConfig:
        ...
        ``--workspace-root``.
        """
        workspace_root = getattr(args, "workspace_root", None) or resolve_env_alias("WORKSTATE_HANDOFF_WORKSPACE_ROOT")
        ...
        state_dir = getattr(args, "state_dir", None) or resolve_env_alias("WORKSTATE_HANDOFF_STATE_DIR")
        current_task_path = getattr(args, "current_task_path", None) or resolve_env_alias(
            "WORKSTATE_HANDOFF_CURRENT_TASK_PATH"
        )
        dashboard_path = getattr(args, "dashboard_path", None) or resolve_env_alias("WORKSTATE_HANDOFF_DASHBOARD_PATH")
        exports_dir = getattr(args, "exports_dir", None) or resolve_env_alias("WORKSTATE_HANDOFF_EXPORTS_DIR")
        ...
        return cls.for_repo(
            ...
            dashboard_path=dashboard_path,
            exports_dir=exports_dir,
            tool_profile=getattr(args, "tool_profile", None) or resolve_env_alias("WORKSTATE_HANDOFF_TOOL_PROFILE"),
        )
