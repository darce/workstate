...
def get_handoff_state(
    ...
    # WORKSTATE-REF-71 implementation note: ``None`` is the sentinel for "argument not supplied".
    ...
    # WORKSTATE-REF-71 implementation note: budget planner runs before heavy section fetches.
    try:
    ...
    with _get_db_connection() as conn:
            ...
            try:
                ...
                if isinstance(exc, AmbiguousWorkspaceContextError):
                    ...
                    error_payload["resolution"] = (
                        ...
                        "WORKSTATE-REF-33: read paths surface candidates instead of failing opaque."
                    )
        ...
        result["limits"] = {
            **limits.as_payload(),
            "write": {
                ...
                # implementation note implementation note / BR-04: publish the write-contract
        ...
        # WORKSTATE-REF-71 implementation note: attach read_budget whenever a budget was supplied
