...
def _cascade_archive_maint_planning_review_rows(
    ...
    *,
    ...
    """Archive MAINT-PLANNING-REVIEW-* rows referencing ``parent_task_ref``.
    ...
    cascade_candidates = conn.execute(
        ...
        FROM handoff_state
        WHERE task_ref LIKE 'MAINT-PLANNING-REVIEW-%'
          AND task_ref != ?
          AND (
    ...
    if cascade_archived:
        ...
        rationale_lines = [
            "## Cascade-archived MAINT-PLANNING-REVIEW rows",
            "",
...
def tasks_gc(apply: bool = False) -> dict:
    """Archive ``status=done`` ``MAINT-PLANNING-REVIEW-*`` rows whose internal parent is archived.
    ...
    with _get_db_connection() as conn:
        ...
        for row in candidates:
            ...
            parent_refs = sorted(set(_internal_TASK_REF_RE.findall(search_text)))
