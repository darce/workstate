...
implementation note implementation note surface. The package owns this logic; bootstrap-hoisted
...
# ``> - **Task ID**: `WORKSTATE-REF-38```). The canonical numbered plan layout
...
# like "this plan touches WORKSTATE-REF-38 in passing".
...
# WORKSTATE-REF-69 implementation note: ``prefer="baseline"`` and ``prefer="auto"`` (when the
...
# and must not be promoted into CURRENT_TASK.json by the renderer. WORKSTATE-REF-41
...
def resolve_plan_location(
    ...
    ``prefer`` selects which snapshot the location anchors to (WORKSTATE-REF-69
    implementation note):
...
def resolve_plan_baseline_status(task_ref: str, plan_path: str | None) -> tuple[str, str | None]:
    ...
    WORKSTATE-REF-72 implementation note: a local sibling of the ``workstate-system``
...
def discover_plan_path_candidates(task_ref: str) -> list[str]:
    ...
    Two complementary signals (BR-WORKSTATE38-S5-01):
