...
class ReviewFindingsUpdateOp(BaseModel):
    ...
    status: Annotated[
        Literal["open", "fixed", "deferred", "wontfix", "resolved_on_branch", "integrated"],
        Field(
            description=(
                "New finding status to apply. Note: 'integrated' is integrate-managed and "
                "rejected on direct write (use operation='integrate'). 'resolved_on_branch' "
                "is write-derived from 'fixed' when the lifecycle feature flag is on."
            ),
        ),
    ]
...
class ReviewFindingsIntegrateOp(BaseModel):
    """WORKSTATE-REF-68 implementation note: lifecycle promotion entry point.

    Distinct from ``operation='reconcile'`` (WORKSTATE-REF-41 orchestrator
