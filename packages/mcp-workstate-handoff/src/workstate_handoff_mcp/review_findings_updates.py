...
def _workspace_has_uncommitted_changes(worktree_path: str | None = None) -> WorkspaceCleanliness:
    # WORKSTATE-REF-09: when a task worktree is derived (resolve path), inspect that
...
def _derive_resolve_worktree_path(conn: sqlite3.Connection, task_ref: str | None) -> str | None:
    ...
    WORKSTATE-REF-09: resolve must evaluate cleanliness and commit context against the
    ...
    :func:`_canonical_worktree_for_task` (WORKSTATE-REF-52 implementation note — the stored
    ...
    bypassed (``AGENT_HANDOFF_SKIP_WORKTREE_DERIVATION``), the row has no
    branch identity yet, or no matching worktree exists (a ``main``/WORKSTATE-REF row
    ...
    untouched so WORKSTATE-REF-52 implementation note cwd-wins precedence holds for other writers.
    """
    if task_ref is None:
        return None
    ...
    *,
    ...
    for key in ("agent", "branch", "commit_sha", "lane_id", "model", "model_label", "reasoning_level"):
        ...
        if value is not None:
            actor[key] = value
    # WORKSTATE-REF-09 implementation note: override branch/commit with the resolve-scoped worktree
    ...
    *,
    ...
    try:
        with _get_db_connection() as conn:
            ...
            # WORKSTATE-REF-09: anchor the resolve to the task's own worktree (derived
    ...
    # WORKSTATE-REF-09 implementation note: compute one resolve-scoped (branch, commit) anchor and
    ...
    # so WORKSTATE-REF-52 implementation note cwd-wins precedence holds for every other writer.
...
def _apply_finding_update(update_ctx: FindingUpdateContext, update_input: FindingUpdateInput) -> dict:
    ...
    # WORKSTATE-REF-68 implementation note: persist the resolution anchor on every successful fixed-close.
    ...
    # WORKSTATE-REF-68 implementation note: when the lifecycle flag is on, a successful ``fixed``
    ...
    # WORKSTATE-REF-68 implementation note: surface the resolution-anchor commit on the
...
def _validate_update_finding_input(
    ...
    # WORKSTATE-REF-68 implementation note: the new lifecycle values are integrate-managed or
    ...
    if normalized_status is FindingStatus.RESOLVED_ON_BRANCH:
        # WORKSTATE-REF-68 implementation note (BR-002): when the lifecycle flag is on, the task
...
# ---------------------------------------------------------------------------
# WORKSTATE-REF-68 implementation note: integrate operation + opportunistic trigger
# ---------------------------------------------------------------------------
...
def _is_ancestor_of_ref(candidate_sha: str, integration_ref: str) -> bool:
    """Return True iff ``candidate_sha`` is an ancestor of (or equal to)
    ``integration_ref`` HEAD. False covers both "not reachable" and "git
    error" so the caller can treat git failures as "leave it for the next
    pass" rather than promoting on uncertain evidence."""
    config = get_runtime_config()
    try:
        proc = subprocess.run(
            ["git", "merge-base", "--is-ancestor", candidate_sha, integration_ref],
            cwd=str(config.git_workspace_root),
            stdin=subprocess.DEVNULL,
            capture_output=True,
            text=True,
...
def integrate_review_findings(
    *,
    ...
    This entry point is **distinct** from WORKSTATE-REF-41's
    ...
    with _get_db_connection() as conn:
        ...
        for row in rows:
            ...
            decision_rationale = (
                f"WORKSTATE-REF-68 integrate promotion: finding `{finding_id}` "
