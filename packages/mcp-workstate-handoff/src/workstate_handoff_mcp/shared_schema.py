...
Extracted from _shared.py (implementation note of WORKSTATE-REF-12-10, task plan WORKSTATE-REF-12-10-shared-module-extraction-task-plan).
...
- Ledger-owned DDL: handoff_state, decisions, blockers, next_actions, verified_tests,
  ...
  triggers, indexes.
- Orchestration-owned DDL (currently bootstrapped here because WORKSTATE-REF-12-9 moved the Python
  orchestration code but did not relocate the DDL):
  worktree_lanes, worker_reports, lane_messages, plan_cursors, turn_metrics.
    TODO(WORKSTATE-REF-12-9-followon): Move orchestration-owned DDL to mcp-workstate-orchestrator bootstrap.
"""
...
#        already-bootstrapped DB until WORKSTATE-REF-9 fixed it).
...
#   v10 — adds compaction_settings (WORKSTATE-REF-67) as the durable runtime
#         disable store for the WORKSTATE-REF custom-compaction surface. One row
...
#   v11 — WORKSTATE-REF-68: adds the two-anchor finding lifecycle columns to
...
#   v12 — WS-ERRTEL-01 (implementation note): adds agent_errors as the durable
...
-- TODO(WORKSTATE-REF-12-9-followon): Move these to mcp-workstate-orchestrator bootstrap once that
-- package owns its own DB connection setup (tracked in WORKSTATE-REF-12-9 follow-on work).
...
def _migrate_add_column_extensions(conn: sqlite3.Connection) -> None:
    ...
    if not _has_column(conn, "handoff_state", "task_plan_path"):
        conn.execute("ALTER TABLE handoff_state ADD COLUMN task_plan_path TEXT")
...
def _migrate_add_turn_metrics(conn: sqlite3.Connection) -> None:
    ...
    TODO(WORKSTATE-REF-12-9-followon): this DDL belongs in mcp-workstate-orchestrator bootstrap.
    """
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS turn_metrics (
...
def _migrate_add_compaction_settings(conn: sqlite3.Connection) -> None:
    """Create the WORKSTATE-REF-67 compaction_settings table on warm-start.
...
def _migrate_finding_lifecycle_states(conn: sqlite3.Connection) -> None:
    """WORKSTATE-REF-68 v10 -> v11: add the two-anchor finding lifecycle columns,
    ...
    if not _has_column(conn, "review_findings", "resolved_on_branch_at_commit"):
        ...
        try:
            ...
            pass
    if not _has_column(conn, "handoff_state", "last_observed_integration_sha"):
        conn.execute("ALTER TABLE handoff_state ADD COLUMN last_observed_integration_sha TEXT")


def _migrate_add_agent_errors(conn: sqlite3.Connection) -> None:
    """Create agent_errors table and its query indexes. Idempotent.

    v12 (WS-ERRTEL-01 / implementation note): durable agent-side error telemetry
