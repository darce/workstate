...
class WorktreeNotFoundError(ValueError):
    ...
    should set ``WORKSTATE_HANDOFF_SKIP_WORKTREE_DERIVATION=1`` before importing
...
def _worktree_derivation_enabled() -> bool:
    ...
    ``WORKSTATE_HANDOFF_SKIP_WORKTREE_DERIVATION=1`` keeps those tests reading
...
def _validate_and_expand_commit_sha(value: str | None) -> str | None:
    ...
    - When git is not available or no repository is reachable from
      ...
      when the ``WORKSTATE_HANDOFF_SKIP_SHA_VALIDATION`` env var is truthy.
...
def build_write_actor(
    ...
    # by WORKSTATE_HANDOFF_SKIP_SHA_VALIDATION (both packages' test conftests
...
def _detect_git_write_context() -> tuple[str | None, str | None]:
    branch = resolve_env_alias("WORKSTATE_HANDOFF_DEFAULT_BRANCH") or _first_non_empty_env(
        "GITHUB_HEAD_REF",
        "GITHUB_REF_NAME",
        "CI_COMMIT_REF_NAME",
        "BRANCH_NAME",
    )
    commit_sha = resolve_env_alias("WORKSTATE_HANDOFF_DEFAULT_COMMIT_SHA") or _first_non_empty_env(
        "GITHUB_SHA",
        "CI_COMMIT_SHA",
    )
    try:
...
def clear_worktree_pointer_for_close(
    ...
    ``WORKSTATE_HANDOFF_SKIP_WORKTREE_DERIVATION``, so the synthetic-branch test
...
def _resolve_write_actor(
    ...
    *,
    ...
    # fallback_agent.
    self_declared_agent = _normalize_optional_text(resolve_env_alias("WORKSTATE_HANDOFF_DEFAULT_AGENT"))
