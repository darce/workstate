...
# implementation note S3 moved the overlay payload (workstate.lifecycle et al.) under
# workstate_system/payload/; source-repo helper scripts (pytest_path_guard,
# check_* gates) stayed in scripts/. Tests need both on sys.path.
WORKSTATE_SYSTEM_PAYLOAD_SCRIPTS = (
    WORKTREE_ROOT / "packages" / "workstate-system" / "workstate_system" / "payload" / "scripts"
)
...
if str(WORKSTATE_SYSTEM_PAYLOAD_SCRIPTS) not in sys.path:
    sys.path.insert(0, str(WORKSTATE_SYSTEM_PAYLOAD_SCRIPTS))
