...
def test_extractor_truncates_oversize_residual(isolated_runtime: RuntimeConfig, tmp_path: Path) -> None:
    ...
    hard_receipt = mcp_server.compact_session(
        transcript_path=hard_transcript,
        task_ref="WORKSTATE-REF-34",
        harness="codex",
        session_id="session-hard",
    )
    hard_stored = mcp_server.get_compaction(hard_receipt.compaction_id)
    assert hard_stored.prose_residual is not None
    assert hard_stored.prose_residual.endswith("chars omitted]")
