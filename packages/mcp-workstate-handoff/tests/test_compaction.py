...
#
# implementation note of the env-var consolidation lands a Pydantic model that owns
# parsing for `AGENT_HANDOFF_COMPACTION_*`, with a one-release alias for
# the legacy `WORKSTATE_COMPACTION_*` names. Bad values raise
...
def test_compaction_settings_defaults_when_unset() -> None:
    ...
    assert settings.legacy_warnings == ()


def test_compaction_settings_reads_workstate_canonical_env_names() -> None:
    """implementation note B4: the new top-tier ``WORKSTATE_HANDOFF_COMPACTION_*``
    names read silently (no deprecation warning)."""
    from workstate_handoff_mcp.compaction import CompactionSettings
...
def test_compaction_settings_reads_canonical_env_names() -> None:
    """Legacy ``AGENT_HANDOFF_COMPACTION_*`` names still read after the B4
    rename, now via the alias fallback with a one-release deprecation warning
    pointing at the canonical ``WORKSTATE_HANDOFF_COMPACTION_*`` name."""
    from workstate_handoff_mcp.compaction import CompactionSettings

    env = {
        "AGENT_HANDOFF_COMPACTION_DISABLED": "1",
        "AGENT_HANDOFF_COMPACTION_MIN_NEW_TURNS": "5",
        "AGENT_HANDOFF_COMPACTION_MIN_NEW_TOKENS": "250",
    }
    settings = CompactionSettings.from_env(env=env)
    assert settings.disabled is True
    assert settings.min_new_turns == 5
    assert settings.min_new_tokens == 250
    warning_text = "\n".join(settings.legacy_warnings)
    assert (
        "AGENT_HANDOFF_COMPACTION_DISABLED is deprecated; rename to WORKSTATE_HANDOFF_COMPACTION_DISABLED"
    ) in warning_text


def test_compaction_settings_falls_back_to_legacy_with_warning() -> None:
    from workstate_handoff_mcp.compaction import CompactionSettings

    env = {
        "WORKSTATE_COMPACTION_DISABLED": "true",
        "WORKSTATE_COMPACTION_MIN_NEW_TURNS": "3",
    }
    settings = CompactionSettings.from_env(env=env)
    assert settings.disabled is True
    assert settings.min_new_turns == 3
    assert settings.min_new_tokens == 0  # neither prefix set this knob

    warning_text = "\n".join(settings.legacy_warnings)
    assert ("WORKSTATE_COMPACTION_DISABLED is deprecated; rename to WORKSTATE_HANDOFF_COMPACTION_DISABLED") in warning_text
    assert (
        "WORKSTATE_COMPACTION_MIN_NEW_TURNS is deprecated; rename to WORKSTATE_HANDOFF_COMPACTION_MIN_NEW_TURNS"
    ) in warning_text


def test_compaction_settings_canonical_wins_silently_over_legacy() -> None:
    from workstate_handoff_mcp.compaction import CompactionSettings

    env = {
        "WORKSTATE_HANDOFF_COMPACTION_MIN_NEW_TURNS": "9",
        "AGENT_HANDOFF_COMPACTION_MIN_NEW_TURNS": "1",
    }
    settings = CompactionSettings.from_env(env=env)
    assert settings.min_new_turns == 9
    assert settings.legacy_warnings == ()


def test_compaction_settings_newer_legacy_wins_over_older_legacy() -> None:
    """implementation note B4: with no canonical set, the newer legacy
    ``AGENT_HANDOFF_COMPACTION_*`` wins over the older ``WORKSTATE_COMPACTION_*``,
    and the rename warning still points at the canonical WORKSTATE name."""
    from workstate_handoff_mcp.compaction import CompactionSettings

    env = {
        "AGENT_HANDOFF_COMPACTION_MIN_NEW_TURNS": "9",
        "WORKSTATE_COMPACTION_MIN_NEW_TURNS": "1",
    }
    settings = CompactionSettings.from_env(env=env)
    assert settings.min_new_turns == 9
    warning_text = "\n".join(settings.legacy_warnings)
    assert (
        "AGENT_HANDOFF_COMPACTION_MIN_NEW_TURNS is deprecated; rename to WORKSTATE_HANDOFF_COMPACTION_MIN_NEW_TURNS"
    ) in warning_text
...
def test_compaction_settings_invalid_int_raises_validation_error() -> None:
    ...
    env = {"AGENT_HANDOFF_COMPACTION_MIN_NEW_TOKENS": "not-an-int"}
...
def test_compaction_settings_negative_int_rejected() -> None:
    ...
    env = {"AGENT_HANDOFF_COMPACTION_MIN_NEW_TURNS": "-5"}
