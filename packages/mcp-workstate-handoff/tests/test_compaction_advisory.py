...
def _write_package_contract(
    ...
    *,
    ...
    # implementation note S3: the package-shipped contract moved under workstate_system/payload/
    # (see compaction_contract._PACKAGE_REFERENCE_CONTRACT_RELATIVE_PATH), so the
    # fixture must write where the runtime resolver now looks.
    contract_path = (
        workspace
        / "packages"
        / "workstate-system"
...
def test_get_handoff_state_publishes_latest_compaction_id(
    isolated_runtime: RuntimeConfig,
) -> None:
    """WS-REINJ-01 implementation note: SessionStart reinjection can read the stable id."""
    from datetime import datetime, timezone
    ...
    _write_contract(isolated_runtime.workspace_root)
    structured = StructuredSummary(
        compaction_id="C-WORKSTATE-REF-61-BR-03",
        session_id="br-03-fixture",
        harness="claude-code",
        task_ref="WORKSTATE-REF-61",
        turn_range=TurnRange(start_turn=11, end_turn=20),
        created_at=datetime.now(timezone.utc),
    )
    with _get_db_connection() as conn:
        conn.execute(
            ...
            """,
            (
                "C-WORKSTATE-REF-61-BR-03",
                "br-03-fixture",
                "claude-code",
                "WORKSTATE-REF-61",
                '{"start_turn":11,"end_turn":20}',
                structured.model_dump_json(),
            ),
        )
        conn.commit()
    ...
    assert envelope["ok"] is True
    data = envelope["data"]
    assert data["latest_compaction_id"] == "C-WORKSTATE-REF-61-BR-03"
    assert data["compaction_advisory"]["latest_compaction_id"] == "C-WORKSTATE-REF-61-BR-03"
