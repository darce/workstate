...
def _write_contract(workspace: Path) -> Path:
    contract_path = workspace / "docs" / "agentic" / "contracts" / "harness-protocol.yaml"
...
def _write_package_contract(
    ...
    *,
    ...
    contract_path = (
        workspace / "packages" / "workstate-system" / "docs" / "agentic" / "contracts" / "harness-protocol.yaml"
    )
    contract_path.parent.mkdir(parents=True, exist_ok=True)
...
def test_env_var_overrides_threshold_chars(
    ...
    """Legacy AGENT_HANDOFF_COMPACTION_THRESHOLD_CHARS still lowers the char
    fallback gate after the B4 rename, now via the alias fallback with a
    one-release deprecation warning pointing at the WORKSTATE canonical name."""
    from workstate_handoff_mcp.compaction import compute_compaction_advisory
    ...
    advisory = compute_compaction_advisory(
        ...
        env={
            "CLAUDE_SESSION_TRANSCRIPT_PATH": str(transcript),
            "AGENT_HANDOFF_COMPACTION_THRESHOLD_CHARS": "350000",
        },
    )
    ...
    assert (
        "AGENT_HANDOFF_COMPACTION_THRESHOLD_CHARS is deprecated; rename to WORKSTATE_HANDOFF_COMPACTION_THRESHOLD_CHARS"
    ) in advisory["warnings"]
...
def test_env_var_beats_overlay_for_tokens(
    ...
    advisory = compute_compaction_advisory(
        ...
        env={
            "CLAUDE_SESSION_TRANSCRIPT_PATH": str(transcript),
            "AGENT_HANDOFF_COMPACTION_THRESHOLD_TOKENS": "70000",
        },
    )
...
def test_invalid_env_override_falls_through_with_warning(
    ...
    advisory = compute_compaction_advisory(
        ...
        env={
            "CLAUDE_SESSION_TRANSCRIPT_PATH": str(transcript),
            "AGENT_HANDOFF_COMPACTION_THRESHOLD_TOKENS": "not-a-number",
        },
    )
    ...
    assert any(
        "compaction_threshold_override_invalid" in w and "AGENT_HANDOFF_COMPACTION_THRESHOLD_TOKENS=not-a-number" in w
