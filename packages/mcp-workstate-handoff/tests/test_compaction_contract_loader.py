...
def test_load_compaction_contract_uses_overlay_shared_root(tmp_path: Path) -> None:
        ...
        json.dumps(
            {
                ...
                "surfaces": {
                    "contracts": {
                        "shared_root": ".workstate/shared",
                        "local_root": ".workstate/local",
                    }
                },
                "configs": [],
            }
    ...
    shared_contract = tmp_path / ".workstate" / "shared" / "harness-protocol.yaml"
...
def test_load_compaction_contract_prefers_local_over_shared(tmp_path: Path) -> None:
    ...
    shared_contract = tmp_path / ".workstate" / "shared" / "harness-protocol.yaml"
    _write_contract(shared_contract)
    local_contract = tmp_path / ".workstate" / "local" / "harness-protocol.yaml"
