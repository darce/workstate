...
- The hook reads `AGENT_HANDOFF_STATE_DIR` to mirror the MCP server CLI; production callers leave it unset and the primary-worktree `.task-state` wins. Worth noting in cross-harness fallback documentation alongside the per-harness Stop event references.
"""
