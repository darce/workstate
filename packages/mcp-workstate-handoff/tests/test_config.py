...
# override via AGENT_HANDOFF_FINDING_LIFECYCLE_STATES, constructor kwarg
...
def test_finding_lifecycle_states_default_is_on() -> None:
    ...
    ``AGENT_HANDOFF_FINDING_LIFECYCLE_STATES=0``."""
    root = Path("/tmp/agent-handoff").resolve()
    with mock.patch.dict(os.environ, {}, clear=True):
        runtime = RuntimeConfig.for_workspace(root)
...
def test_finding_lifecycle_states_env_var_opts_in(
    ...
    """Operators flip the flag via AGENT_HANDOFF_FINDING_LIFECYCLE_STATES."""
    root = Path("/tmp/agent-handoff").resolve()
    monkeypatch.setenv("AGENT_HANDOFF_FINDING_LIFECYCLE_STATES", "1")
    runtime = RuntimeConfig.for_workspace(root)
    assert runtime.finding_lifecycle_states_enabled is True

    # Falsy values stay off (parity with current_task_auto_regen semantics).
    for falsy in ("0", "false", "off", "no"):
        monkeypatch.setenv("AGENT_HANDOFF_FINDING_LIFECYCLE_STATES", falsy)
        runtime = RuntimeConfig.for_workspace(root)
...
def test_finding_lifecycle_states_constructor_override(
    ...
    root = Path("/tmp/agent-handoff").resolve()
    monkeypatch.setenv("AGENT_HANDOFF_FINDING_LIFECYCLE_STATES", "0")
...
def test_finding_lifecycle_states_round_trip_through_from_args(
    ...
    monkeypatch.setenv("AGENT_HANDOFF_WORKSPACE_ROOT", str(root))
    monkeypatch.setenv("AGENT_HANDOFF_FINDING_LIFECYCLE_STATES", "1")
    runtime = RuntimeConfig.from_args(FakeArgs())
    assert runtime.finding_lifecycle_states_enabled is True
    ...
    monkeypatch.delenv("AGENT_HANDOFF_FINDING_LIFECYCLE_STATES")
    runtime = RuntimeConfig.from_args(FakeArgs())
    assert runtime.finding_lifecycle_states_enabled is True
