...
def test_resolve_write_actor_self_declared_agent_beats_inherited_last_writer(
    isolated_handoff: dict, monkeypatch: pytest.MonkeyPatch
) -> None:
    """WORKSTATE-REF-81: caller's AGENT_HANDOFF_DEFAULT_AGENT outranks the row's last writer.
    ...
    """
    _seed_row_with_updated_by("self-declared-wins", "github-copilot", "feature/self-declared-wins")
    monkeypatch.setenv("AGENT_HANDOFF_DEFAULT_AGENT", "claude-opus-4-7")
    monkeypatch.setattr(handoff_core, "_detect_git_write_context", lambda: ("main", "cwd_sha"))
    monkeypatch.setattr(handoff_core, "_detect_git_write_context_at", lambda path: (None, None))

    with handoff_core._get_db_connection() as conn:
        ctx = handoff_core._resolve_write_actor(conn, None, task_ref="self-declared-wins")
    assert ctx.agent == "claude-opus-4-7", "self-declared env identity must outrank inherited active_agent"


def test_resolve_write_actor_self_declared_agent_canonical_workstate_name(
    isolated_handoff: dict, monkeypatch: pytest.MonkeyPatch
) -> None:
    """implementation note B4: the canonical WORKSTATE_HANDOFF_DEFAULT_AGENT name resolves
    the self-declared identity (legacy AGENT_HANDOFF_DEFAULT_AGENT still honored
    via the alias fallback)."""
    _seed_row_with_updated_by("canonical-self-declared", "github-copilot", "feature/canonical-self-declared")
    monkeypatch.delenv("AGENT_HANDOFF_DEFAULT_AGENT", raising=False)
