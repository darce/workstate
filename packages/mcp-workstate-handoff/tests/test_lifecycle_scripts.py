...
def _build_fake_monorepo(tmp_path: Path) -> Path:
    ...
    (repo / "docs" / "workstate" / "contracts").mkdir(parents=True, exist_ok=True)
    (repo / "docs" / "workstate" / "contracts" / "harness-protocol.yaml").write_text(
        """version: 1
...
def _make_env(repo: Path) -> dict[str, str]:
    ...
    env["WORKSTATE_HANDOFF_SKIP_SHA_VALIDATION"] = "1"
    # Keep branch enforcement opt-in for lifecycle subprocesses too;
    # ambient shells may set WORKSTATE_HANDOFF_ENFORCE_BRANCH=1 globally.
    env["WORKSTATE_HANDOFF_SKIP_BRANCH_ENFORCEMENT"] = "1"
    return env
