...
- **MCP server identity cutover — `agent-orchestrator-mcp` →
...
- **Distribution name renamed from `workstate-orchestrator-mcp` to
  `mcp-workstate-orchestrator`.** The previous PyPI name was squatted by
  an unrelated party; the rename also aligns with the binary name
  (`mcp-workstate-orchestrator`) and the symmetric rename of
  `mcp-workstate-handoff` (formerly `workstate-handoff-mcp`).

  **Migration:** anywhere you wrote `pip install workstate-orchestrator-mcp`,
  swap to `pip install mcp-workstate-orchestrator` (or
  `mcp-workstate-orchestrator>=0.2.0,<0.3.0` for a range). The console
  script (`mcp-workstate-orchestrator`) and importable module
  (`agent_orchestrator_mcp`) are unchanged.
...
- **Console script renamed from `workstate-orchestrator-mcp` to `mcp-workstate-orchestrator`**
  to match the `mcp-*` prefix naming convention shared with sibling MCP
  servers (`mcp-workstate-handoff`, etc.). The PyPI/git package name
  (`workstate-orchestrator-mcp`) and the importable Python module
  (`agent_orchestrator_mcp`) are unchanged — only the installed CLI entry
  point flips. Consumers must update `[mcpServers].*.command` entries in
  MCP client configs (`.mcp.json`, `.vscode/mcp.json`, `.codex/config.toml`),
  release scripts, and any direct subprocess invocations.

### Changed

- `workstate-handoff-mcp` dependency advanced from `v0.4.2` to `v0.4.3`
  to pick up the paired console-script rename (`workstate-handoff-mcp` →
  `mcp-workstate-handoff`). The orchestrator's
...
- Update the `command` field wherever the server is launched:
  ...
  No Python-level changes are required: `from agent_orchestrator_mcp import ...`
