...
TOOL_DESCRIPTIONS: dict[str, str] = {
    ...
    "list_available_backends": "List supported execution backends and their capabilities. Pass probe=true to add a probed is_available flag plus availability_state/detail per backend (may shell out to codex/claude and import optional bridges; off by default).",
...
def list_available_backends(probe: bool = False) -> dict:
    """List supported execution backends and their capabilities.

    By default this copies the static declaration table only (cheap: no
    subprocess calls and no optional bridge imports), so the hot path stays fast.

    Pass ``probe=True`` to additionally classify live availability per backend via
    ``backend_registry.probe_availability``. Each entry then gains:
    ...
    * ``availability_detail`` — human-readable explanation.
    ...
    modules, so it is opt-in and kept off the default path.
    """
    try:
        backend_registry = _import_orchestration_module("backend_registry")
        backends = {}
