...
def _env_flag(name: str, *legacy: str) -> bool:
    value = resolve_env_alias(name, *legacy)
    return isinstance(value, str) and value.strip().lower() in _TRUE_ENV_VALUES


def _env_nonnegative_int(name: str, *legacy: str) -> int:
    raw_value = resolve_env_alias(name, *legacy)
...
def _runtime_attribution_from_env() -> dict[str, Any]:
    ctx7_query_count = _env_nonnegative_int(
        "WORKSTATE_HANDOFF_CTX7_QUERY_COUNT",
        "AGENT_HANDOFF_CTX7_QUERY_COUNT",
    )
    used_ctx7 = _env_flag("WORKSTATE_HANDOFF_USED_CTX7", "AGENT_HANDOFF_USED_CTX7") or ctx7_query_count > 0
    return {
        "used_ace_guidance": _env_flag(
            "WORKSTATE_HANDOFF_ACE_GUIDANCE_USED",
            "AGENT_HANDOFF_ACE_GUIDANCE_USED",
        ),
        "used_ctx7": used_ctx7,
