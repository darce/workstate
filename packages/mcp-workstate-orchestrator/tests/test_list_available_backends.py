"""Probe-gating contract for ``api.list_available_backends``.

The default call MUST stay cheap — copying the static declaration table only,
with no probed ``is_available`` field, no subprocess calls, and no optional
bridge imports. ``probe=True`` adds the probed availability view (delegated to
``backend_registry.probe_availability``) and distinguishes a declared-but-not-
installed optional bridge from a reachable one.
"""
...
def test_default_call_is_cheap_and_omits_probed_fields() -> None:
    """Default (probe=False) returns the static table only — no probed fields."""
    # No subprocess and no bridge import may happen on the default path.
    with (
        mock.patch.object(backend_registry, "probe_availability") as probe,
        mock.patch("subprocess.run", side_effect=AssertionError("default path must not shell out")),
    ):
...
def test_default_shape_matches_static_declaration() -> None:
    """The declaration semantics other tools rely on are unchanged at default."""
    result = api.list_available_backends()
