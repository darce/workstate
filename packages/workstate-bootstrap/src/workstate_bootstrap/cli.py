...
from workstate_bootstrap.subcommands import apply_hooks, doctor, repair, status, update
...
def _resolve_managed_servers(
    ...
    """
    from workstate_bootstrap.install import _manifest_source_kind
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p_install.add_argument(
        ...
        default=None,
        help=(
            "Git URL for the shared workstate-system remote. When omitted, "
    ...
    p_install.add_argument(
        ...
        help=(
            ...
            "workstate-system distribution; 'worktree' materializes from the "
    ...
    p_install.add_argument(
        ...
        help=(
            ...
            "hook into the shared <target>/.claude/settings.json (checked into "
            "git). The adapter rendered is the one declared under hook "
            "'reinject-context' in config/agent-workflows/portable_commands.json. "
            "Off by default."
        ),
    )
    p_install.add_argument(
    ...
    p_install.add_argument(
        ...
        action="store_true",
        help=(
            "Opt in to writing the bootstrap-managed PostToolUse(Bash) "
            "capture-agent-errors hook into the shared "
            "<target>/.claude/settings.json (checked into git). The adapter "
            "rendered is the one declared under hook 'capture-agent-errors' "
            "in config/agent-workflows/portable_commands.json. Off by default."
        ),
    )
    p_install.add_argument(
        ...
        help=(
            "Opt in to writing the bootstrap-managed PostToolUse(Bash) "
            "capture-agent-errors hook into the user-owned "
            "<target>/.claude/settings.local.json (gitignored). "
            "Reversible by deleting the file. Off by default."
        ),
    )
    ...
    p_doctor.add_argument(
        ...
        action="store_true",
        help=(
            "Also query PyPI for a newer workstate-stack release "
    ...
    p_update = sub.add_parser(
        ...
        help=(
            ...
            "remote ref (git_overlay) or the installed workstate-system "
    ...
    for flag, help_text in (
        (
            "--install-claude-stop-hook",
            "Apply the bootstrap-managed Claude Stop hook adapter.",
        ),
        (
            "--install-claude-stop-hook-local",
        ...
        (
            "--install-claude-reinject-hook",
            "Apply the bootstrap-managed Claude SessionStart reinject adapter.",
        ),
        (
            "--install-claude-reinject-hook-local",
        ...
        (
            "--install-claude-error-hook",
            "Apply the bootstrap-managed Claude PostToolUse error hook adapter.",
        ),
        (
            "--install-claude-error-hook-local",
            "Apply the bootstrap-managed Claude PostToolUse error hook adapter locally.",
        ),
    ):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "install":
        ...
        try:
            manifest = install(
                ...
                install_claude_reinject_hook=args.install_claude_reinject_hook,
                install_claude_reinject_hook_local=args.install_claude_reinject_hook_local,
                install_claude_error_hook=args.install_claude_error_hook,
                install_claude_error_hook_local=args.install_claude_error_hook_local,
            )
    ...
    if args.command == "doctor":
        ...
        # are listed but never affect the exit code (implementation note implementation note; WORKSTATE-REF-09
    ...
    if args.command == "update":
        ...
        if update_kind == "package":
            print(
                ...
                f"workstate-system=={manifest['package_version']}.",
                file=sys.stdout,
            )
        ...
        for path in manifest.get("adopted_redundant") or []:
            print(
                ...
                "(identical to payload; backup under .workstate/backup/)",
                file=sys.stdout,
            )
        _print_local_precedence_skips(manifest)
        return 0
