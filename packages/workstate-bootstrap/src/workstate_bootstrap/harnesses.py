...
def cursor_native_surface_problems(target: Path) -> list[str]:
    source_root = plugin_tree_out(target, "effective") / "cursor"
    if not source_root.is_dir():
        return []
    ...
    for rel in (CURSOR_COMMANDS_DEST, CURSOR_SKILLS_DEST):
        ...
        if not actual.is_dir():
            ...
            continue
        expected_names = {p.name for p in expected.iterdir() if p.is_dir() or p.is_file()}
        actual_names = {p.name for p in actual.iterdir() if p.is_dir() or p.is_file()}
        if expected_names != actual_names:
            problems.append(f"{rel.as_posix()} entry set differs from effective cursor tree")
...
def materialize_cursor_plugin(
    ...
    for source_name, dest in (
        ...
        if not source.is_dir():
            raise FileNotFoundError(f"missing generated Cursor {source_name}: {source}")
        action = "updated" if dest.exists() or dest.is_symlink() else "created"
        surface_actions.append(action)
        if dest.exists() or dest.is_symlink():
            if dest.is_dir() and not dest.is_symlink():
                surface = {"path": dest.relative_to(target).as_posix(), "source": "local"}
