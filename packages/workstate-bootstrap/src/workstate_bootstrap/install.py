...
DEFAULT_MCP_SERVERS: dict[str, dict[str, Any]] = {
    ...
    "workstate-orchestrator-mcp": {
        "type": "stdio",
        "command": "uvx",
        "args": ["mcp-workstate-orchestrator@0.5.0", "--workspace-root", ".", "serve-stdio"],
    },
}
...
#
# The two *distributed* runtime-managed servers were renamed from their
# legacy ``agent-*`` identities to Workstate-native names. Read-side
# compatibility for one release: whenever a managed map carries the new
# canonical name, every render seam drops the matching legacy name from the
# existing on-disk config so an old ``agent-*`` registration is rewritten
# forward to the canonical name instead of left as a stale duplicate.
#
# NOTE (D1): ``design-canvas-mcp`` is intentionally NOT in this map. Canvas
# stays private/non-distributed per implementation note §9-D and implementation note §0.5 D1; its
# server identity is deliberately left unrenamed and is owned by a separate
# follow-up.
LEGACY_MCP_SERVER_RENAMES: dict[str, str] = {
    "agent-handoff-mcp": "workstate-handoff-mcp",
    "agent-orchestrator-mcp": "workstate-orchestrator-mcp",
}


def _legacy_prune_for(
    mcp_servers: Mapping[str, Mapping[str, Any]],
    prune_names: Iterable[str],
) -> tuple[str, ...]:
    """Return ``prune_names`` augmented with any legacy server name whose
    canonical replacement is present in ``mcp_servers``.

    This is the always-on read-side compat seam: when the managed map
    carries ``workstate-handoff-mcp``, the stale ``agent-handoff-mcp``
    registration (if any) is pruned from the existing config before the
    canonical entry is merged in, collapsing a duplicate old+new pair to a
    single canonical registration. Idempotent — a config with only the
    canonical name is unaffected.
    """
    managed = set(mcp_servers)
    legacy = {
        legacy_name
        for legacy_name, canonical in LEGACY_MCP_SERVER_RENAMES.items()
        if canonical in managed
    }
    return tuple(dict.fromkeys((*prune_names, *legacy)))
...
# ---------------------------------------------------------------------------
# Runtime-path rename migration (implementation note Slice D)
# ---------------------------------------------------------------------------
#
# implementation note §11 items #3/#4 renamed the runtime root ``.agentic/`` and the
# mirrored docs path ``docs/agentic/`` to ``.workstate/`` / ``docs/workstate/``.
# Bootstrap install/update detects a legacy tree in a consumer checkout and
# migrates it forward exactly once, archive-backed and idempotent. The
# (legacy -> canonical) pairs come from :data:`RUNTIME_PATH_RENAMES` in
# ``workstate_protocol`` so the names live in one place.


def _migration_archive_relroot(stamp: str) -> Path:
    """Relative archive root for an in-place legacy-tree archive."""
    return Path(RUNTIME_ROOT_DIRNAME, f"_migrated-from-agentic-{stamp}")


def plan_runtime_path_migration(target: Path) -> list[dict[str, str]]:
    """Return the moves :func:`migrate_runtime_paths` would perform.

    Each entry is ``{"legacy": <rel>, "canonical": <rel>, "action": <action>}``
    where ``action`` is one of:

    - ``"move"`` — the legacy path exists and the canonical path is absent;
      the legacy tree is moved/renamed to the canonical path.
    - ``"archive"`` — both paths exist; the legacy tree is archived under a
      timestamped ``.workstate/_migrated-from-agentic-<stamp>/`` directory and
      removed (the canonical tree is never overwritten).

    Paths with no legacy tree present produce no entry (already migrated /
    fresh install). This is the dry-run surface; it performs no filesystem
    mutation.
    """
    target = Path(target).resolve()
    moves: list[dict[str, str]] = []
    for legacy_rel, canonical_rel in RUNTIME_PATH_RENAMES:
        legacy = target / legacy_rel
        if not (legacy.exists() or legacy.is_symlink()):
            continue
        canonical = target / canonical_rel
        action = "archive" if (canonical.exists() or canonical.is_symlink()) else "move"
        moves.append(
            {"legacy": legacy_rel, "canonical": canonical_rel, "action": action}
        )
    return moves


def migrate_runtime_paths(
    target: Path,
    *,
    stamp: str | None = None,
    dry_run: bool = False,
) -> list[dict[str, str]]:
    """Migrate a legacy ``.agentic/`` / ``docs/agentic/`` tree forward.

    For each (legacy -> canonical) pair in :data:`RUNTIME_PATH_RENAMES`:

    - **move**: when the legacy path exists and the canonical path is absent,
      rename the legacy tree to the canonical location (``git mv`` when the
      target is a git worktree so the rename is tracked, falling back to
      ``shutil.move``).
    - **archive**: when both exist, never silently overwrite — move the legacy
      tree under ``<target>/.workstate/_migrated-from-agentic-<stamp>/<legacy>``
      first, then leave the canonical tree in place.

    Idempotent: once migrated (no legacy tree) this is a no-op. ``stamp`` is
    passed in (defaulting to a fresh :func:`_utc_stamp`) — never computed at
    import time. Returns the list of moves actually performed (or that would be
    performed under ``dry_run``).
    """
    target = Path(target).resolve()
    moves = plan_runtime_path_migration(target)
    if dry_run or not moves:
        return moves

    stamp = stamp or _utc_stamp()
    for move in moves:
        legacy = target / move["legacy"]
        canonical = target / move["canonical"]
        if move["action"] == "move":
            canonical.parent.mkdir(parents=True, exist_ok=True)
            try:
                _git("mv", move["legacy"], move["canonical"], cwd=target)
            except (subprocess.CalledProcessError, FileNotFoundError):
                shutil.move(str(legacy), str(canonical))
        else:  # archive
            archive_rel = _migration_archive_relroot(stamp) / move["legacy"]
            archive_dest = target / archive_rel
            archive_dest.parent.mkdir(parents=True, exist_ok=True)
            shutil.move(str(legacy), str(archive_dest))
            move["archived_to"] = archive_rel.as_posix()
    return moves
...
def install(
    *,
    ...
    # implementation note Slice D: migrate a legacy .agentic/ + docs/agentic/ checkout
    # forward before the clone/surface materialization runs against the new
    # .workstate/ paths. Idempotent; a no-op once migrated.
    migrate_runtime_paths(target)
...
def _run_init_state(
    ...
    *,
    ...
    env_has_state_dir = isinstance(raw_env, Mapping) and "WORKSTATE_HANDOFF_STATE_DIR" in raw_env
...
def _render_mcp_json(
    ...
    *,
    ...
    prune_names = _legacy_prune_for(mcp_servers, prune_names)
    if prune_names:
