...
def _preserved_mcp_servers(
    ...
    # implementation note Slice B read-side compat: an existing consumer config may
    # still register the legacy ``agent-*`` server identity. Rewrite it
    # forward to the canonical Workstate-native name so update re-emits only
    # the canonical registration. A canonical entry already present wins over
    # a stale legacy one (never resurrect the old name).
    from workstate_bootstrap.install import LEGACY_MCP_SERVER_RENAMES

    preserved: dict[str, Mapping[str, Any]] = {}
    for name, spec in servers.items():
        if not (isinstance(name, str) and isinstance(spec, dict)):
            continue
        canonical = LEGACY_MCP_SERVER_RENAMES.get(name, name)
        if canonical != name and canonical in preserved:
            # Canonical already captured from a non-legacy entry; drop legacy.
            continue
        preserved[canonical] = spec
    return preserved
...
def _resolve_init_state_check_command(
    ...
    raw_env = spec.get("env")
    env_has_state_dir = isinstance(raw_env, dict) and "WORKSTATE_HANDOFF_STATE_DIR" in raw_env
    if (
...
def doctor(
    *,
    ...
    - ``legacy_runtime_path`` — a stale legacy ``.agentic/`` / ``docs/agentic/``
      tree (implementation note Slice D) is present and not yet migrated forward to
      ``.workstate/`` / ``docs/workstate/``. Carries the planned migration
      ``action`` (``move`` or ``archive``); install/update perform it
      automatically.
    ...
    findings.extend(_doctor_legacy_runtime_paths(target))

    return findings


def _doctor_legacy_runtime_paths(target: Path) -> list[Finding]:
    """Flag a stale legacy ``.agentic/`` / ``docs/agentic/`` tree.

    implementation note Slice D renamed the runtime root and docs mirror. Install/update
    migrate a legacy checkout forward automatically; this finding surfaces a
    legacy tree that has not yet been migrated (e.g. a manual edit, or a
    consumer that has not re-run install) so ``doctor`` reports it as
    ``legacy_runtime_path`` drift. Each finding carries the planned migration
    ``action`` (``move`` or ``archive``).
    """
    return [
        {
            "kind": "legacy_runtime_path",
            "path": move["legacy"],
            "action": move["action"],
        }
        for move in plan_runtime_path_migration(target)
    ]
