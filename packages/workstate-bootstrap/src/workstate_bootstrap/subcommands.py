...
# WS-HOOKOPTIN-01: opt-in flag -> install() keyword. Derivation mirrors the
...
_HOOK_OPT_IN_KWARGS: dict[str, str] = {
    "--install-claude-stop-hook": "install_claude_stop_hook",
    ...
    "--install-claude-reinject-hook": "install_claude_reinject_hook",
...
def doctor(
    *,
    ...
    # Hook-surface coherence (implementation note implementation note): runs for ALL source kinds
...
def _manifest_hook_adapters(clone: Path) -> list[tuple[str, str, Mapping[str, Any]]]:
    ...
    WS-REINJ-01 implementation note generalized this from the compact-session-only
...
def _managed_hook_adapters(
    ...
    rows WS-HOOKOPTIN-01 tags in the install manifest — NOT per path, because
...
def _doctor_managed_stop_adapters(
    ...
    WS-REINJ-01 implementation note generalized the WORKSTATE-REF-80 compact-session check to
...
def _doctor_hook_coherence(target: Path) -> list[Finding]:
    """Hook-surface coherence facet (implementation note implementation note).
...
def _plugin_tree_integrity_problems(root: Path, harness: str) -> list[str]:
    ...
    elif not (root / ".mcp.json").is_file():
...
def _doctor_grok_plugin_materialization(target: Path) -> list[Finding]:
    """Validate materialized ``.grok/plugins/workstate-system/`` against effective tree."""
            ...
            return [
                {
                    ...
                    "message": (
                        "materialized grok plugin is missing; re-run install or update"
                    ),
                }
            ]
        return []
    ...
    if effective_root.is_dir():
        expected_skills = {
            path.name
            for path in (effective_root / "skills").iterdir()
            if path.is_dir()
        }
        actual_skills = {
        ...
        if expected_skills != actual_skills:
            problems.append(
                "skills/ slug set differs from effective grok plugin tree"
            )
        for rel_path in (
            ...
            actual_path = dest_root / rel_path
            if expected_path.is_file() and actual_path.is_file():
                if expected_path.read_bytes() != actual_path.read_bytes():
                    problems.append(
                        f"{rel_path.as_posix()} differs from effective grok plugin tree"
                    )
        manifest_path = dest_root / ".grok-plugin" / "plugin.json"
