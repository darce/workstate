...
def _patch_grok_cli_unavailable() -> "contextlib.ExitStack":
    """Force ``_grok_cli_available() -> False`` for every activation module copy.

    ``test_doctor_hook_coherence.py`` / ``test_hook_coherence.py`` pop
    ``workstate_bootstrap.*`` from ``sys.modules`` at import time, so in a
    full-suite run this file's module-level ``adopt_worktree`` import can
    reference an *older* ``workstate_bootstrap.activation`` module object than
    the one ``mock.patch("workstate_bootstrap.activation...")`` resolves. On a
    machine with a real ``grok`` CLI the stale copy then shells out for real
    (``applied`` instead of ``skipped_no_cli``). Patch both: the name-resolved
    module AND the globals of the activation function captured by the in-use
    ``adopt_worktree``.
    """
    ...
    stack.enter_context(
        patch("workstate_bootstrap.activation._grok_cli_available", return_value=False)
    )
    captured = adopt_worktree.__globals__.get("write_plugin_activation")
    if captured is not None:
        stack.enter_context(
            patch.dict(captured.__globals__, {"_grok_cli_available": lambda: False})
        )
    return stack


def test_adopt_materializes_and_activates_grok_plugin_surface(
    ...
    primary.mkdir()
    _git("init", "--initial-branch=main", cwd=primary)
    ...
    with _patch_grok_cli_unavailable():
        install(target=primary, remote_url=url, remote_ref=ref)

    wt = _add_worktree(primary, tmp_path / "wt")
    with _patch_grok_cli_unavailable():
        receipt = adopt_worktree(target=wt, primary=primary)

    grok_link = wt / GROK_PLUGIN_DEST
    assert grok_link.is_symlink()
    assert (grok_link.resolve() / ".grok-plugin" / "plugin.json").is_file()
    surface_paths = {entry["path"] for entry in receipt["surfaces"]}
    assert GROK_PLUGIN_DEST.as_posix() in surface_paths
    activation_entries = [
        entry
        for entry in receipt.get("configs", [])
    ...
    assert _compute_drift(wt, primary, clone) == []
    grok_link.unlink()
    assert GROK_PLUGIN_DEST.as_posix() in _compute_drift(wt, primary, clone)


def _grok_primary_with_worktree(
    tmp_path: Path, fake_remote_with_generator: tuple[str, str]
) -> tuple[Path, Path]:
    from unittest.mock import patch
...
def test_adopt_skips_foreign_grok_plugin_content(
    tmp_path: Path, fake_remote_with_generator: tuple[str, str]
) -> None:
    """A pre-existing real (non-symlink) grok plugin dir in the worktree is
    never clobbered: surface source=local, config action=skipped_foreign_content
    (WORKSTATE09-R2-B-005a)."""
    from unittest.mock import patch

    from workstate_bootstrap.install import GROK_PLUGIN_DEST

    primary, wt = _grok_primary_with_worktree(tmp_path, fake_remote_with_generator)

    foreign = wt / GROK_PLUGIN_DEST
    ...
    grok_surfaces = [
        entry
        for entry in receipt["surfaces"]
        if entry["path"] == GROK_PLUGIN_DEST.as_posix()
    ]
    assert grok_surfaces and grok_surfaces[0]["source"] == "local"
    grok_configs = [
        entry
        for entry in receipt.get("configs", [])
        if entry.get("kind") == "grok_plugin"
    ]
    assert grok_configs and grok_configs[0]["action"] == "skipped_foreign_content"


def test_foreign_grok_content_is_not_drift_and_doctor_warns(
    tmp_path: Path, fake_remote_with_generator: tuple[str, str]
) -> None:
    """Foreign-preserved grok content (skipped_foreign_content, local
    precedence) is NOT perpetual drift: _compute_drift stays clean and doctor
    reports a non-fatal local-precedence warning instead of grok_plugin_drift
    (WORKSTATE-REF-09-REV-B-r4-01)."""
    from unittest.mock import patch
    ...
    foreign = wt / GROK_PLUGIN_DEST
    foreign.mkdir(parents=True)
    (foreign / "sentinel.txt").write_text("operator-owned\n")
    ...
    clone = primary / ".workstate" / "remote"
    assert GROK_PLUGIN_DEST.as_posix() not in _compute_drift(wt, primary, clone)

    findings = _doctor_grok_plugin_materialization(wt)
    assert not any(f["kind"] == "grok_plugin_drift" for f in findings)
    precedence = [f for f in findings if f["kind"] == "grok_plugin_local_precedence"]
    assert precedence and precedence[0]["severity"] == "warning"


def test_repair_skips_foreign_grok_content_instead_of_false_success(
    tmp_path: Path, fake_remote_with_generator: tuple[str, str]
) -> None:
    """repair() must not claim grok_plugin_drift repaired when materialization
    declines to overwrite foreign content (WORKSTATE-REF-09-REV-A-r4-01)."""
    from unittest.mock import patch
    ...
    # The manifest marker is normally tracked and survives into the worktree;
    # this fixture installs after the seed commit, so mirror it by hand.
    import shutil

    shutil.copy2(
        primary / ".workstate-bootstrap.json", wt / ".workstate-bootstrap.json"
    )

    # Current doctor no longer emits grok_plugin_drift for foreign content;
    # fabricate one (e.g. from a stale doctor run) to pin the repair guard.
    drift_finding = {
        "kind": "grok_plugin_drift",
        "path": GROK_PLUGIN_DEST.as_posix(),
        "message": "stale finding",
    }
    with patch("workstate_bootstrap.subcommands.doctor", return_value=[drift_finding]):
        with patch(
            "workstate_bootstrap.activation._grok_cli_available", return_value=False
        ):
            report = repair(target=wt)

    assert drift_finding in report["skipped"], report
    assert drift_finding not in report["repaired"], report
    assert not foreign.is_symlink()
    assert sentinel.read_text() == "operator-owned\n"


def test_repair_skips_foreign_grok_content_in_primary_repo(
    tmp_path: Path, fake_remote_with_generator: tuple[str, str]
) -> None:
    """repair() in the PRIMARY repo (non-worktree path) must honest-skip
    operator-owned grok content instead of rmtree'ing it via
    _materialize_grok_plugin (WORKSTATE-REF-09-REV-C-r5-001)."""
    import shutil
    from unittest.mock import patch
    ...
    primary, _wt = _grok_primary_with_worktree(tmp_path, fake_remote_with_generator)

    # Replace the managed primary copy with operator-owned content: a real
    # directory lacking the generated .grok-plugin/plugin.json manifest.
    dest = primary / GROK_PLUGIN_DEST
    shutil.rmtree(dest)
...
def test_grok_surface_problems_tolerate_missing_effective_skills(
    tmp_path: Path, fake_remote_with_generator: tuple[str, str]
) -> None:
    """A partial effective tree (no skills/) must not abort the diagnostic
    with FileNotFoundError; grok_plugin_surface_problems returns normally
    (WORKSTATE-REF-09-REV-C-r5-002)."""
    import shutil

    from workstate_bootstrap.activation import grok_plugin_surface_problems
    from workstate_bootstrap.install import GROK_PLUGIN_DEST, _plugin_tree_out

    primary, _wt = _grok_primary_with_worktree(tmp_path, fake_remote_with_generator)

    dest = primary / GROK_PLUGIN_DEST
    assert (dest / "skills").is_dir(), "managed copy should carry skills/"
    effective_skills = _plugin_tree_out(primary, "effective") / "grok" / "skills"
    assert effective_skills.is_dir()
    shutil.rmtree(effective_skills)

    problems = grok_plugin_surface_problems(primary)
    assert isinstance(problems, list)
    assert not any("skills/ slug set differs" in problem for problem in problems)


def test_adopt_check_reports_broken_grok_symlink(
    tmp_path: Path, fake_remote_with_generator: tuple[str, str]
) -> None:
    """A grok plugin symlink whose target vanished is drift, not clean
    (WORKSTATE09-R2-B-005b)."""
    from unittest.mock import patch
    ...
    assert grok_link.is_symlink()
    grok_link.unlink()
    grok_link.symlink_to("../missing-effective-grok-tree", target_is_directory=True)

    clone = primary / ".workstate" / "remote"
    assert GROK_PLUGIN_DEST.as_posix() in _compute_drift(wt, primary, clone)


def test_adopt_check_reports_grok_skills_set_mismatch(
    tmp_path: Path, fake_remote_with_generator: tuple[str, str]
) -> None:
    """A materialized grok tree whose skills/ slug set differs from the
    effective tree is drift (WORKSTATE09-R2-B-005c)."""
    import shutil
    from unittest.mock import patch
    ...
    grok_link = wt / GROK_PLUGIN_DEST
    effective_root = grok_link.resolve()
    grok_link.unlink()
    shutil.copytree(effective_root, grok_link, symlinks=True)
    skill_dirs = [p for p in (grok_link / "skills").iterdir() if p.is_dir()]
    assert skill_dirs, "expected at least one materialized skill"
    shutil.rmtree(skill_dirs[0])

    problems = grok_plugin_surface_problems(wt)
    assert any("skills/ slug set differs" in problem for problem in problems)
    clone = primary / ".workstate" / "remote"
    assert GROK_PLUGIN_DEST.as_posix() in _compute_drift(wt, primary, clone)
