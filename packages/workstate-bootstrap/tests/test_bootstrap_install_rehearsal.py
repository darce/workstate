...
def test_install_with_default_servers_requires_uvx_in_args() -> None:
    ...
    assert DEFAULT_MCP_SERVERS["workstate-handoff-mcp"]["args"][0] == "mcp-workstate-handoff@0.11.5"
    assert DEFAULT_MCP_SERVERS["workstate-orchestrator-mcp"]["args"][0] == "mcp-workstate-orchestrator@0.4.7"
...
def test_rehearsal_hoists_plan_targets_makefile_and_shell_wrapper(
    ...
    ``WORKSTATE_HANDOFF_PLAN_CLI ?= $(or $(AGENT_HANDOFF_PLAN_CLI),uvx --from mcp-workstate-handoff python -m
    ...
    assert (
        "WORKSTATE_HANDOFF_PLAN_CLI ?= $(or $(AGENT_HANDOFF_PLAN_CLI),uvx --from mcp-workstate-handoff python -m workstate_handoff_mcp.plan_cli)"
        in plans_mk_text
    ), (
