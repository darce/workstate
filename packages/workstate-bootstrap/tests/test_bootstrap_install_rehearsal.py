...
def test_install_with_default_servers_satisfies_rehearsal_invariants(
    ...
    for helper in SHARED_HOOK_HELPER_NAMES:
        ...
        assert helper_path.is_file(), f"missing hook helper {helper} at {helper_path}"
...
def test_install_with_default_servers_requires_uvx_in_args() -> None:
    ...
    assert (
        DEFAULT_MCP_SERVERS["workstate-handoff-mcp"]["args"][0]
        == "mcp-workstate-handoff@0.12.7"
    )
    assert (
        DEFAULT_MCP_SERVERS["workstate-orchestrator-mcp"]["args"][0]
...
def test_rehearsal_hoists_plan_targets_makefile_and_shell_wrapper(
    ...
    assert by_path["Makefile.d/plans.mk"]["source"] == "shared", by_path.get(
        "Makefile.d/plans.mk"
    )
