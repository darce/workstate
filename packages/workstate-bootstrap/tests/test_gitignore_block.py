...
def test_stale_block_reconciled_with_new_managed_entry(tmp_path: Path) -> None:
    """REV17A2E2E-B-01 regression: a consumer whose managed block predates a
    newly-managed surface (e.g. the Grok plugin tree) must have the missing
    entry reconciled INTO the existing block on the next install/adopt — not
    short-circuited as ``already_present`` while the surface leaks forever."""
    repo = tmp_path / "consumer"
    _init_repo(repo)
    # A stale managed block: every current entry EXCEPT the grok tree.
    stale_entries = [
        e for e in _consumer_gitignore_entries() if "grok" not in e
    ]
    ...
    _git("commit", "-m", "stale managed block", cwd=repo)
    # The newly-managed surface is materialized and would leak.
    grok_tree = repo / ".grok" / "plugins" / "workstate-system"
    grok_tree.mkdir(parents=True)
    (grok_tree / "plugin.json").write_text("{}\n")

    result = _ensure_consumer_gitignore_block(repo)

    assert result == {"path": ".gitignore", "action": "updated"}
    text = (repo / ".gitignore").read_text()
    assert text.count(GITIGNORE_SENTINEL_BEGIN) == 1  # reconciled, not duplicated
    assert "/.grok/plugins/workstate-system" in text
    assert _is_ignored(repo, ".grok/plugins/workstate-system/plugin.json")
    # Second pass is a clean no-op.
    assert _ensure_consumer_gitignore_block(repo)["action"] == "already_present"
...
def test_block_skipped_when_surfaces_are_tracked_source(tmp_path: Path) -> None:
    ...
    # The repo still ignores its runtime dirs and the materialized Grok plugin
    # tree itself (as the live monorepo does).
    (repo / ".gitignore").write_text(
...
def test_entries_derive_from_harness_delivery_registry() -> None:
    """Pin: every materialized harness plugin tree in HARNESS_PLUGIN_DELIVERY is
    ignored (regenerable content), and every marketplace pointer is NOT (it is
    the consumer's committed install surface). A new harness registered with a
    materialized tree gets ignore coverage without a one-off append."""
    from workstate_bootstrap.install import HARNESS_PLUGIN_DELIVERY

    entries = {e.lstrip("/") for e in _consumer_gitignore_entries()}
    for harness, delivery in HARNESS_PLUGIN_DELIVERY.items():
        for surface in delivery["materialized"]:
            assert surface in entries, f"{harness}: {surface} must be ignored"
        for pointer in delivery["marketplace"]:
            assert pointer not in entries, f"{harness}: {pointer} must stay tracked"
    # The Grok tree is the live materialized case — pin it explicitly.
    assert ".grok/plugins/workstate-system" in entries
