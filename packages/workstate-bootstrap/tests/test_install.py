...
def test_install_keeps_effective_pins_and_recomposes_passthrough_when_overrides_disappear(
    ...
    effective_skill = effective_root / "claude" / "skills" / "branch-review" / "SKILL.md"
...
def test_install_with_default_sentinel_uses_built_in_server_map(
    ...
    assert mcp_doc["mcpServers"]["workstate-handoff-mcp"]["args"][0] == "mcp-workstate-handoff@0.12.3"
    assert mcp_doc["mcpServers"]["workstate-orchestrator-mcp"]["args"][0] == "mcp-workstate-orchestrator@0.6.0"
...
def test_run_init_state_retries_with_cloned_handoff_project_when_uvx_fails(
    ...
    assert calls[0][0][1] == "mcp-workstate-handoff@0.12.3"
    assert calls[1][0][:5] == [
        "uv",
        "run",
        "--project",
