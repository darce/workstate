...
def test_install_points_plugin_pins_at_effective_tree_when_overrides_exist(
    ...
    assert claude_marketplace["plugins"][0]["source"] == (
        "./.agentic/generated/plugins/workstate-system/effective/claude"
    )
    ...
    assert codex_marketplace["plugins"][0]["source"] == {
        "source": "local",
        "path": "./.agentic/generated/plugins/workstate-system/effective/codex",
    }
...
def test_install_reverts_effective_tree_pins_to_base_tree_when_overrides_disappear(
    ...
    assert codex_marketplace["plugins"][0]["source"] == {
        "source": "local",
        "path": "./.agentic/generated/plugins/workstate-system/base/codex",
    }
...
def test_shared_surface_set_includes_docs_workflow_rules() -> None:
    """WORKSTATE-REF-50 implementation note: ``docs/agentic/rules`` must be a hoisted shared
...
def test_planning_artifact_home_rule_materializes(
    ...
    real ``packages/workstate-system/docs/agentic/rules/`` content — the
    ...
    for surface in SHARED_SURFACES:
        if surface in SURFACE_CHILD_EXCLUSIONS:
            ...
            assert child.resolve() == (
                target / ".agentic" / "remote" / surface / "MARKER.md"
            ).resolve()
...
# the legacy clone-root path (e.g. ``scripts/hooks -> ../.agentic/remote/
...
#       lexically under <target>/.agentic/remote/ -> repoint, source='shared',
#       emit `repointed: <surface>` to stdout
#   (3) live or broken with raw link target outside <target>/.agentic/remote/
...
def test_install_repoints_stale_shared_surface_symlink(
    ...
    (target string ``../.agentic/remote/scripts/hooks``) must be repointed to
    the current nested source (``../.agentic/remote/packages/workstate-system/
    ...
    # Lexically under <target>/.agentic/remote/ but at the legacy root path
    # that does not exist in the nested-layout remote.
    os.symlink("../.agentic/remote/scripts/hooks", stale_link)
    ...
    expected = target / ".agentic" / "remote" / "packages" / "workstate-system" / "scripts" / "hooks"
    assert stale_link.is_symlink()
...
def test_install_preserves_foreign_shared_surface_symlink_as_local(
    ...
    """A symlink whose raw target points OUTSIDE <target>/.agentic/remote/
...
def test_install_preserves_broken_shared_surface_symlink_outside_remote_as_local(
    ...
    """A broken symlink whose raw target is OUTSIDE <target>/.agentic/remote/
...
def test_install_manifest_classifies_repaired_stale_symlink_as_shared(
    ...
    target.mkdir()
    (target / "scripts").mkdir()
    os.symlink("../.agentic/remote/scripts/hooks", target / "scripts" / "hooks")
...
def test_install_with_default_sentinel_uses_built_in_server_map(
    ...
    assert mcp_doc["mcpServers"]["workstate-handoff-mcp"]["args"][0] == "mcp-workstate-handoff@0.11.5"
    assert mcp_doc["mcpServers"]["workstate-orchestrator-mcp"]["args"][0] == "mcp-workstate-orchestrator@0.4.7"
...
def test_run_init_state_retries_with_cloned_handoff_project_when_uvx_fails(
    ...
    target = tmp_path / "consumer"
    target.mkdir()
    project = target / ".agentic" / "remote" / "packages" / "mcp-workstate-handoff"
    ...
    assert calls[0][0][1] == "mcp-workstate-handoff@0.11.5"
    assert calls[1][0][:5] == [
        "uv",
        "run",
        "--project",
...
def test_branch_install_uses_cloned_local_mcp_servers_for_default_mapping(
    ...
    target = tmp_path / "consumer"
    target.mkdir()
    handoff_project = target / ".agentic" / "remote" / "packages" / "mcp-workstate-handoff"
    orchestrator_project = target / ".agentic" / "remote" / "packages" / "mcp-workstate-orchestrator"
    ...
    assert resolved == {
        "workstate-handoff-mcp": {
            ...
            "args": [
                "run",
                "--project",
                ".agentic/remote/packages/mcp-workstate-handoff",
                "mcp-workstate-handoff",
                "--workspace-root",
                ".",
                "serve-stdio",
        ...
        "workstate-orchestrator-mcp": {
            ...
            "args": [
                "run",
                "--project",
                ".agentic/remote/packages/mcp-workstate-orchestrator",
