...
def test_install_is_idempotent_on_repeat_call(
    ...
    install(target=target, remote_url=remote_url, remote_ref=ref)
    first_sha = json.loads((target / ".workstate-bootstrap.json").read_text())[
        "remote_sha"
    ]

    install(target=target, remote_url=remote_url, remote_ref=ref)
    second_sha = json.loads((target / ".workstate-bootstrap.json").read_text())[
        "remote_sha"
    ]
...
def _build_remote(
    workdir: Path, name: str, seed_file: str = "skill.md"
) -> tuple[Path, str]:
...
def test_install_switches_managed_clone_when_manifest_remote_changes(
    ...
    assert (
        _git("remote", "get-url", "origin", cwd=target / ".workstate" / "remote")
        == first_url
    )
...
# Shared surfaces that are single files (symlinked as the file itself), not
# directories of children.
FILE_SURFACES_EXPECTED = frozenset({".codex/hooks.json"})
...
def fake_remote_with_surfaces(tmp_path: Path) -> tuple[str, str]:
    ...
    for surface in SHARED_SURFACES_EXPECTED:
        p = src / surface
        if surface in FILE_SURFACES_EXPECTED:
            # File surface (e.g. .codex/hooks.json): a single shared file,
            # not a directory of children.
            p.parent.mkdir(parents=True, exist_ok=True)
            p.write_text('{"hooks": {}}\n')
            continue
        p.mkdir(parents=True)
        (p / "MARKER.md").write_text(f"shared {surface}\n")
    # WS-HOOKGEN-01: ship the Codex hook config even if it ever drops out of
    # SHARED_SURFACES_EXPECTED, so the materialization test fails on
    # materialization rather than on fixture contents.
    codex_hooks = src / ".codex" / "hooks.json"
...
def fake_remote_with_generator(tmp_path: Path) -> tuple[str, str]:
    ...
    # implementation note S4: plugin override composition ships beside the generator.
    compose_helper = payload_root / "scripts" / "plugin_override_compose.py"
    if compose_helper.is_file():
        shutil.copy2(
            compose_helper,
            system_subdir / "scripts" / "plugin_override_compose.py",
        )
    shutil.copytree(
...
def test_install_discovers_plugin_override_root_and_composes_effective_tree(
    ...
    structured = yaml.safe_load(
        (system_root / "skills" / "branch-review" / "skill.yaml").read_text()
    )
    ...
    assert effective_skill_path.is_file(), (
        "install must materialize the effective plugin tree"
    )
    ...
    assert (
        manifest["remote_sha"] == json.loads(plugin_lock.read_text())["base_remote_sha"]
    )


def test_install_materializes_grok_plugin_tree(
    tmp_path: Path, fake_remote_with_generator: tuple[str, str]
) -> None:
    """Bootstrap copies effective grok plugin to .grok/plugins/workstate-system/."""
    ...
    grok_plugin_root = target / GROK_PLUGIN_DEST
    assert (grok_plugin_root / ".grok-plugin" / "plugin.json").is_file()
    assert (grok_plugin_root / "hooks" / "hooks.json").is_file()
    assert not (grok_plugin_root / ".mcp.json").exists()
    ...
    materialized_skills = {
        path.name
        for path in (grok_plugin_root / "skills").iterdir()
        if path.is_dir()
    }
    assert materialized_skills == effective_skills
    assert effective_skills, "effective grok plugin must expose at least one skill"

    surface_paths = {entry["path"] for entry in manifest["surfaces"]}
    assert GROK_PLUGIN_DEST.as_posix() in surface_paths
    config_paths = {entry["path"] for entry in manifest["configs"]}
    assert GROK_PLUGIN_DEST.as_posix() in config_paths
...
def test_install_writes_tracked_override_lock_for_plugin_overrides(
    ...
    assert override_lock.is_file(), (
        "install must emit overrides.lock.json for tracked override provenance"
    )
...
def test_install_points_plugin_pins_at_effective_tree_when_overrides_exist(
    ...
    claude_marketplace = json.loads(
        (target / ".claude-plugin" / "marketplace.json").read_text()
    )
    ...
    assert claude_settings["extraKnownMarketplaces"]["workstate-marketplace"][
        "source"
    ] == {
        "source": "directory",
        "path": ".",
    ...
    assert (
        claude_settings["enabledPlugins"]["workstate-system@workstate-marketplace"]
        is True
    )

    codex_marketplace = json.loads(
        (target / ".agents" / "plugins" / "marketplace.json").read_text()
    )
    ...
    assert (
        codex_config["plugins"]["workstate-system@workstate-marketplace"]["enabled"]
        is True
    )
...
def test_install_keeps_effective_pins_and_recomposes_passthrough_when_overrides_disappear(
    tmp_path: Path, fake_remote_with_generator: tuple[str, str]
) -> None:
    from workstate_bootstrap.install import install
    ...
    # WORKSTATE-REF-07 always-effective: removing the override root does NOT flip pins
    # back to base; the effective tree is recomposed as a passthrough copy and
    # the pins keep targeting it.
    claude_marketplace = json.loads(
        (target / ".claude-plugin" / "marketplace.json").read_text()
    )
    assert claude_marketplace["plugins"][0]["source"] == (
    ...
    effective_skill = (
        effective_root / "claude" / "skills" / "branch-review" / "SKILL.md"
    )
    assert effective_skill.is_file()
    assert "Bootstrap-composed override body." not in effective_skill.read_text()
    lock = json.loads((effective_root / "plugin-lock.json").read_text())
...
def test_install_reset_overrides_removes_clean_override_root(
    ...
    # WORKSTATE-REF-07 always-effective: pins stay on the effective tree after a reset;
    # the tree is recomposed as a passthrough copy of base.
    claude_marketplace = json.loads(
        (target / ".claude-plugin" / "marketplace.json").read_text()
    )
    assert claude_marketplace["plugins"][0]["source"] == (
...
def test_install_materializes_codex_hooks_config_surface(
    tmp_path: Path, fake_remote_with_surfaces: tuple[str, str]
) -> None:
    """WS-HOOKGEN-01: `.codex/hooks.json` is a shared file surface.

    The generated Codex hook config (statusMessage-labelled handlers) ships
    from the workstate-system payload; install materializes it as a file
    symlink into the clone and records source='shared'.
    """
    from workstate_bootstrap.install import install
    ...
    link = target / ".codex" / "hooks.json"
    assert link.is_symlink(), ".codex/hooks.json must be materialized as a symlink"
    assert link.resolve() == (
        target / ".workstate" / "remote" / ".codex" / "hooks.json"
    ).resolve()
    assert json.loads(link.read_text()) == {"hooks": {}}
    by_path = {entry["path"]: entry for entry in manifest["surfaces"]}
    assert by_path[".codex/hooks.json"]["source"] == "shared"
...
def test_install_materializes_known_shared_surfaces(
    ...
    for surface in SHARED_SURFACES:
        if surface in SURFACE_CHILD_EXCLUSIONS:
            ...
            assert (
                child.resolve()
                == (target / ".workstate" / "remote" / surface / "MARKER.md").resolve()
            )
        ...
        elif surface in FILE_SURFACES_EXPECTED:
            # File surface: the symlink itself is the shared file.
            link = target / surface
            ...
            assert link.is_symlink(), f"{surface} must be a symlink"
            assert (
                link.resolve() == (target / ".workstate" / "remote" / surface).resolve()
            )
...
def test_materialize_surfaces_removes_bootstrap_owned_excluded_children(
    ...
    makefile_d.mkdir()
    (makefile_d / "evals.mk").symlink_to(os.path.relpath(md / "evals.mk", makefile_d))
...
def test_install_idempotent_rerun_does_not_rewrite_shared_surface_symlink(
    tmp_path: Path,
    fake_remote_with_surfaces: tuple[str, str],
    capsys: pytest.CaptureFixture[str],
) -> None:
...
def test_install_writes_all_four_config_surfaces_when_mcp_servers_provided(
    ...
    assert (
        codex_doc["mcp_servers"]["workstate-handoff-mcp"]["command"] == sys.executable
    )
...
def test_install_with_default_sentinel_uses_built_in_server_map(
    ...
    manifest = install(
        target=target, remote_url=url, remote_ref=ref, mcp_servers="default"
    )
    ...
    assert (
        mcp_doc["mcpServers"]["workstate-handoff-mcp"]["args"][0]
        == "mcp-workstate-handoff@0.12.7"
    )
    assert (
        mcp_doc["mcpServers"]["workstate-orchestrator-mcp"]["args"][0]
        == "mcp-workstate-orchestrator[bridge]@0.6.4"
    )
...
def test_run_init_state_retries_with_cloned_handoff_project_when_uvx_fails(
    ...
    def fake_run(cmd: list[str], **kwargs: object) -> subprocess.CompletedProcess[str]:
        ...
        return subprocess.CompletedProcess(
            rendered, 0, stdout='{"ok": true}', stderr=""
        )

    import workstate_bootstrap.external as external_module

    monkeypatch.setattr(external_module, "run_external", fake_run)
    ...
    assert calls[0][0][1] == "mcp-workstate-handoff@0.12.7"
    assert calls[1][0][:5] == [
        "uv",
        "run",
        "--project",
...
def test_branch_install_uses_cloned_local_mcp_servers_for_default_mapping(
    ...
    target.mkdir()
    handoff_project = (
        target / ".workstate" / "remote" / "packages" / "mcp-workstate-handoff"
    )
    orchestrator_project = (
        target / ".workstate" / "remote" / "packages" / "mcp-workstate-orchestrator"
    )
...
def test_install_presyncs_branch_default_local_mcp_specs(
    ...
    monkeypatch.setattr(
        ...
        lambda target_arg, remote_url: (remote_url, None),
    )
    monkeypatch.setattr(
        install_module, "_materialize_surfaces", lambda target_arg, clone: []
    )
    monkeypatch.setattr(
        install_module, "_prepare_generated_surfaces", lambda target_arg, clone: []
    )
    monkeypatch.setattr(
        install_module,
        "_prepare_plugin_generated_surfaces",
        lambda target_arg, clone, override_root: [],
    )
    monkeypatch.setattr(install_module, "_run_generator", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        install_module,
        "_write_configs",
        lambda target_arg, mcp_servers, include_hooks=True, receipt=None: [],
    )
...
def test_install_lean_profiles_do_not_presync_local_mcp_specs(
    ...
    def fake_presync(
        target_arg: Path, mcp_servers: dict[str, dict[str, object]]
    ) -> list[Path]:
...
def test_presync_local_mcp_envs_syncs_bridge_extra_when_declared(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    # The uvx pin sites carry [bridge] so the codex-subagent backend is
    # importable (finding 421); the local-source launch path must match by
    # presyncing the declared `bridge` extra — otherwise a locally-launched
    # orchestrator still probes declared_not_installed while the published
    # one works.
    install_module = _load_local_install_module(monkeypatch)
    ...
    (orchestrator_project / "pyproject.toml").write_text(
        ...
        'bridge = ["workstate-codex-bridge>=0.1.0,<0.2.0"]\n'
    )
    ...
    assert calls == [
        ...
        [
            ...
            "sync",
            "--project",
            str(orchestrator_project.resolve()),
            "--extra",
            "bridge",
        ],
    ]
...
def test_prewarm_uvx_mcp_envs_warms_each_published_spec(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    # Round-3 review finding bridge_pin_hard_fails_offline_orchestrator_launch:
    # the [bridge] uvx pin moves bridge resolution to uvx's env-build layer, so
    # a session boot on an offline host (or a mirror without the bridge wheel)
    # now hard-fails orchestrator launch instead of degrading to
    # declared_not_installed. Hoist env construction to install time — the
    # _presync_local_mcp_envs pattern — by warming uv's tool-env cache for
    # every published uvx spec while the installer is online.
    install_module = _load_local_install_module(monkeypatch)
    ...
    warmed = install_module._prewarm_uvx_mcp_envs(
        target, install_module.DEFAULT_MCP_SERVERS
    )

    handoff_ref = install_module.DEFAULT_MCP_SERVERS["workstate-handoff-mcp"]["args"][0]
    orchestrator_ref = install_module.DEFAULT_MCP_SERVERS["workstate-orchestrator-mcp"][
        "args"
    ][0]
    assert orchestrator_ref.startswith("mcp-workstate-orchestrator[bridge]@")
    assert warmed == [handoff_ref, orchestrator_ref]
    assert calls == [
        ["uvx", "--from", handoff_ref, "python", "-c", "import sys"],
        ["uvx", "--from", orchestrator_ref, "python", "-c", "import sys"],
    ]


def test_prewarm_uvx_mcp_envs_is_best_effort_and_skips_non_uvx_specs(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    # A warm-up failure (offline installer, mirror missing the bridge wheel)
    # must not fail the install: the spec still works on any host with a warm
    # cache or a reachable index, and the handoff server must stay installable
    # even when the orchestrator extra cannot resolve. Local `uv run` specs
    # are _presync_local_mcp_envs territory and are skipped here.
    install_module = _load_local_install_module(monkeypatch)
    ...
    specs = {
        ...
        "workstate-handoff-mcp": {
            "type": "stdio",
            "command": "uv",
            "args": ["run", "--project", "x", "mcp-workstate-handoff"],
        },
    }
    ...
    warmed = install_module._prewarm_uvx_mcp_envs(target, specs)

    # Warm-up attempted for the uvx spec only; failure returns it as
    # not-warmed instead of raising, and the local uv spec is skipped.
    assert warmed == []
    assert len(attempted) == 1
    assert attempted[0][0] == "uvx"


def test_install_prewarms_published_uvx_mcp_specs(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    # install(profile=all) with the published uvx specs must warm the uv
    # tool-env cache at install time so `uvx ...[bridge]@<ver>` session boots
    # are cache-served (offline-safe after an online install).
    install_module = _load_local_install_module(monkeypatch)

    target = tmp_path / "consumer"
    target.mkdir()
    # Remote WITHOUT local mcp packages: the resolver keeps the published
    # uvx DEFAULT_MCP_SERVERS instead of switching to local uv run specs.
    src = tmp_path / "no-mcp-src"
    src.mkdir()
    _git("init", "--initial-branch=main", cwd=src)
    _git("config", "user.email", "test@example.com", cwd=src)
    _git("config", "user.name", "Test", cwd=src)
    ...
    _git("add", "-A", cwd=src)
    _git("commit", "-m", "seed", cwd=src)
    bare = tmp_path / "no-mcp.git"
    _git("clone", "--bare", str(src), str(bare), cwd=tmp_path)

    prewarm_calls: list[tuple[Path, object]] = []

    def fake_prewarm(target_arg: Path, mcp_servers: object) -> list[str]:
        prewarm_calls.append((target_arg, mcp_servers))
        return []

    monkeypatch.setattr(install_module, "_prewarm_uvx_mcp_envs", fake_prewarm)
    monkeypatch.setattr(
        install_module,
        "_prepare_state_for_remote_switch",
        lambda target_arg, remote_url: (remote_url, None),
    ...
    assert prewarm_calls[0][0] == target
    resolved = prewarm_calls[0][1]
    assert isinstance(resolved, dict)
    assert resolved["workstate-orchestrator-mcp"]["command"] == "uvx"
...
def test_install_aborts_when_hook_coherence_has_errors(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """implementation note implementation note: install must refuse a broken hook surface before
    writing a success receipt."""
    install_module = _load_local_install_module(monkeypatch)
    BootstrapManifestValidationError = install_module.BootstrapManifestValidationError
    install = install_module.install

    src = tmp_path / "broken-src"
    src.mkdir()
    _git("init", "--initial-branch=main", cwd=src)
    _git("config", "user.email", "test@example.com", cwd=src)
    _git("config", "user.name", "Test", cwd=src)
    hooks = src / ".github" / "hooks"
    hooks.mkdir(parents=True)
    (hooks / "terminal-guard.json").write_text(
        json.dumps(
            {
                "hooks": {
                    "PreToolUse": [
                        {
                            "matcher": "Bash",
                            "type": "command",
                            "command": "python3 scripts/hooks/missing-guard.py",
                        }
                    ]
                }
            }
    ...
    script_dir = src / "scripts" / "hooks"
    script_dir.mkdir(parents=True)
    (script_dir / "guard-ok.py").write_text("print('ok')\n")
    _git("add", "-A", cwd=src)
    _git("commit", "-m", "broken hook config", cwd=src)
    _git("tag", "v0.1.0", cwd=src)
    bare = tmp_path / "broken.git"
    _git("clone", "--bare", str(src), str(bare), cwd=tmp_path)
    assert "terminal-guard.json" in _git(
        "ls-tree", "-r", "--name-only", "v0.1.0", cwd=bare
    )

    target = tmp_path / "consumer"
    target.mkdir()

    with pytest.raises(BootstrapManifestValidationError) as exc:
        install(
            target=target,
            remote_url=f"file://{bare}",
            remote_ref="v0.1.0",
    ...
    assert "hook-surface coherence" in str(exc.value)
    assert "missing-guard.py" in str(exc.value)
    assert not (target / ".workstate-bootstrap.json").exists()


def test_update_aborts_when_unmanaged_codex_hook_config_is_broken(
    tmp_path: Path,
    fake_remote_with_surfaces: tuple[str, str],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """implementation note implementation note: update catches receipt-unknown hook configs that
    survive refresh and would otherwise stay invisible to lifecycle checks."""
    install_module = _load_local_install_module(monkeypatch)
    BootstrapManifestValidationError = install_module.BootstrapManifestValidationError
    install = install_module.install
    ...
    # Model the committed-orphan class: the config exists on disk, names a
    # missing script, and is not claimed by the current receipt.
    manifest_path = target / ".workstate-bootstrap.json"
    manifest = json.loads(manifest_path.read_text())
    manifest["surfaces"] = [
        entry
        for entry in manifest["surfaces"]
        if entry.get("path") != ".codex/hooks.json"
    ]
                    ...
                    "PreToolUse": [
                        {
                            "matcher": "Bash",
                            "type": "command",
                            "command": "python3 scripts/hooks/missing-codex.py",
                        }
                    ]
                }
            }
    ...
    with pytest.raises(BootstrapManifestValidationError) as exc:
        update(target=target, remote_ref=ref)

    assert "hook-surface coherence" in str(exc.value)
    assert "missing-codex.py" in str(exc.value)
...
def test_render_vscode_and_codex_enforce_no_sync_on_local_uv_specs(
    ...
    vscode = json.loads(
        install_module._render_vscode_mcp_json(target, servers).decode()
    )
...
def test_write_plugin_pins_always_target_effective(tmp_path: Path) -> None:
    """WORKSTATE-REF-07 implementation note: pins target effective/ unconditionally, even with no
    override root — the conditional base/effective switching is removed."""
    from workstate_bootstrap.install import _write_plugin_pins

    target = tmp_path / "consumer"
    target.mkdir()

    _write_plugin_pins(target, None, include_codex_activation=False)
...
def test_install_without_overrides_composes_effective_passthrough(
    tmp_path: Path, fake_remote_with_generator: tuple[str, str]
) -> None:
    """WORKSTATE-REF-07 implementation note: a zero-override install still materializes effective/
    (byte-equivalent to base) plus a passthrough plugin-lock.json, and pins
    target effective."""
    from workstate_bootstrap.install import install
    ...
    assert (
        surfaces[".workstate/generated/plugins/workstate-system/effective"]
        == "generated"
    )

    generated_root = (
        target / ".workstate" / "generated" / "plugins" / "workstate-system"
    )
    base_root = generated_root / "base"
    effective_root = generated_root / "effective"
    assert base_root.is_dir()
    assert effective_root.is_dir(), (
        "zero-override install must materialize the effective plugin tree"
    )
    ...
    effective_files = {
        path.relative_to(effective_root): path.read_bytes()
        for path in effective_root.rglob("*")
        if path.is_file()
    }
...
# ---------------------------------------------------------------------------
# implementation note implementation note — self-host mount repoint
# ---------------------------------------------------------------------------


def _seed_self_host_payload(target: Path) -> Path:
    """Give ``target`` an in-tree live payload mirroring the self-host layout.

    Ships every shared surface so the same-snapshot gate sees ONE mount root
    (zero skew) once install repoints the overlay at the live payload.
    """
    payload = (
        target / "packages" / "workstate-system" / "workstate_system" / "payload"
    )
    for surface in SHARED_SURFACES_EXPECTED:
        p = payload / surface
        if surface in FILE_SURFACES_EXPECTED:
        ...
        (p / "MARKER.md").write_text(f"live {surface}\n")
    (payload / "scripts" / "hooks" / "guard-ok.py").write_text("print('ok')\n")
    (payload / ".github" / "hooks" / "terminal-guard.json").write_text(
        json.dumps(
            {
                "hooks": {
                    "PreToolUse": [
                        {
                            "matcher": "Bash",
                            "type": "command",
                            "command": "python3 scripts/hooks/guard-ok.py",
                        }
                    ]
                }
            }
...
def test_self_host_install_mounts_hook_surfaces_from_live_payload(
    tmp_path: Path, fake_remote_with_surfaces: tuple[str, str]
) -> None:
    """When the target itself ships the workstate-system payload (self-host),
    hook surfaces must symlink the LIVE in-tree payload — not the
    ``.workstate/remote`` clone — so a payload edit can never skew against a
    stale clone mount (the terminal-guard incident substrate)."""
    from workstate_bootstrap.install import install

    target = tmp_path / "selfhost"
    target.mkdir()
    payload = _seed_self_host_payload(target)

    url, ref = fake_remote_with_surfaces
    manifest = install(target=target, remote_url=url, remote_ref=ref)

    github_hooks = target / ".github" / "hooks"
    assert github_hooks.is_symlink()
    assert github_hooks.resolve() == (payload / ".github" / "hooks").resolve()
    scripts_hooks = target / "scripts" / "hooks"
    assert scripts_hooks.is_symlink()
    assert scripts_hooks.resolve() == (payload / "scripts" / "hooks").resolve()
...
def test_self_host_install_repoints_stale_clone_hook_symlink(
    tmp_path: Path,
    fake_remote_with_surfaces: tuple[str, str],
    capsys: pytest.CaptureFixture[str],
) -> None:
    """Migration starting state A: the original stale-clone symlink. Install
    must repoint it at the live payload as an explicit, receipted operation
    (rg-017: never a silent rewrite of local content — this link is
    bootstrap-owned, it names our clone)."""
    from workstate_bootstrap.install import install
    ...
    payload = _seed_self_host_payload(target)
    (target / ".github").mkdir()
    stale = target / ".github" / "hooks"
    os.symlink(
        target / ".workstate" / "remote" / ".github" / "hooks", stale
    )
    ...
    assert "repointed: .github/hooks" in captured.out
    assert stale.resolve() == (payload / ".github" / "hooks").resolve()


def test_self_host_install_idempotent_on_phase0_payload_symlink(
    tmp_path: Path,
    fake_remote_with_surfaces: tuple[str, str],
    capsys: pytest.CaptureFixture[str],
) -> None:
    """Migration starting state B: the Phase-0 hand-repointed payload symlink.
    Install must leave it as-is (no repointed: audit line) and classify it
    shared."""
    from workstate_bootstrap.install import install
    ...
    payload = _seed_self_host_payload(target)
    (target / ".github").mkdir()
    link = target / ".github" / "hooks"
    os.symlink(
        os.path.relpath(payload / ".github" / "hooks", link.parent), link
    )
    first_readlink = os.readlink(link)

    url, ref = fake_remote_with_surfaces
    manifest = install(target=target, remote_url=url, remote_ref=ref)
    captured = capsys.readouterr()

    assert "repointed: .github/hooks" not in captured.out
    assert os.readlink(link) == first_readlink
    by_path = {entry["path"]: entry for entry in manifest["surfaces"]}
    assert by_path[".github/hooks"]["source"] == "shared"


def test_self_host_install_coherence_zero_skew(
    tmp_path: Path, fake_remote_with_surfaces: tuple[str, str]
) -> None:
    """The structural closer's proof: after a self-host install every shared
    surface resolves to the SAME live-payload mount, so the same-snapshot
    gate reports zero findings."""
    from workstate_bootstrap.coherence import assess_hook_coherence
    from workstate_bootstrap.install import install

    target = tmp_path / "selfhost"
    target.mkdir()
    _seed_self_host_payload(target)

    url, ref = fake_remote_with_surfaces
    manifest = install(target=target, remote_url=url, remote_ref=ref)

    findings = assess_hook_coherence(target, receipt=manifest)
    assert findings == [], [f"{f.kind}:{f.path}" for f in findings]


def test_consumer_install_without_in_tree_payload_keeps_clone_mounts(
    tmp_path: Path, fake_remote_with_surfaces: tuple[str, str]
) -> None:
    """Consumer single-clone invariant stays byte-identical: without an
    in-tree payload, surfaces keep symlinking the clone."""
    from workstate_bootstrap.install import install
    ...
    github_hooks = target / ".github" / "hooks"
    assert github_hooks.is_symlink()
    resolved = str(github_hooks.resolve())
    assert str((target / ".workstate" / "remote").resolve()) in resolved


def test_install_receipt_records_surface_provenance_key(
    tmp_path: Path, fake_remote_with_surfaces: tuple[str, str]
) -> None:
    """implementation note implementation note (REV-B-001): every shared surface entry records the
    optional additive ``provenance_key`` snapshot identity; live derivation
    stays the authoritative fallback for receipts without the field."""
    from workstate_bootstrap.install import install
    ...
    by_path = {entry["path"]: entry for entry in manifest["surfaces"]}
    for surface in (".github/hooks", "scripts/hooks"):
        entry = by_path[surface]
        assert entry["source"] == "shared"
        assert entry.get("provenance_key", "").startswith("clone:"), entry


def test_self_host_install_receipt_provenance_key_is_payload_mount(
    tmp_path: Path, fake_remote_with_surfaces: tuple[str, str]
) -> None:
    """On self-host the recorded provenance_key names the live payload mount
    (one shared link: root across hook surfaces), not the clone."""
    from workstate_bootstrap.install import install
    ...
    by_path = {entry["path"]: entry for entry in manifest["surfaces"]}
    keys = {
        by_path[surface]["provenance_key"]
        for surface in (".github/hooks", "scripts/hooks")
    }
    assert len(keys) == 1, keys
    (key,) = keys
    assert key.startswith("link:")
    assert "workstate_system/payload" in key
