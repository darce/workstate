...
def test_install_with_default_sentinel_uses_built_in_server_map(
    ...
    assert (
        ...
        == "mcp-workstate-handoff@0.12.5"
    )
    assert (
        mcp_doc["mcpServers"]["workstate-orchestrator-mcp"]["args"][0]
        == "mcp-workstate-orchestrator[bridge]@0.6.2"
    )
...
def test_run_init_state_retries_with_cloned_handoff_project_when_uvx_fails(
    ...
    assert calls[0][0][1] == "mcp-workstate-handoff@0.12.5"
    assert calls[1][0][:5] == [
        "uv",
        "run",
        "--project",
...
def test_install_presyncs_branch_default_local_mcp_specs(
    ...
    monkeypatch.setattr(
        install_module,
        "_write_configs",
        lambda target_arg, mcp_servers, include_hooks=True: [],
    )
