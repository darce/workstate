...
def test_package_install_preserves_preexisting_local_surface(tmp_path: Path) -> None:
    """Package-source install must keep local precedence just like git_overlay."""
    from workstate_bootstrap.install import install

    package_root = tmp_path / "payload"
    (package_root / "scripts" / "hooks").mkdir(parents=True)
    (package_root / "scripts" / "hooks" / "guard.py").write_text(
        "payload\n", encoding="utf-8"
    )
    ...
    (local_hooks / "guard.py").write_text("consumer override\n", encoding="utf-8")
    ...
    assert (local_hooks / "guard.py").read_text(
        encoding="utf-8"
    ) == "consumer override\n"
    assert {"path": "scripts/hooks", "source": "local"} in manifest["surfaces"]


def test_package_update_refreshes_previous_shared_copy(tmp_path: Path) -> None:
    """Receipt-owned copies still update; only local-precedence paths are held."""
    from workstate_bootstrap.install import install

    package_root = tmp_path / "payload"
    hooks = package_root / "scripts" / "hooks"
    hooks.mkdir(parents=True)
    (hooks / "guard.py").write_text("payload v1\n", encoding="utf-8")
    ...
    assert (target / "scripts" / "hooks" / "guard.py").read_text(
        encoding="utf-8"
    ) == "payload v1\n"

    (hooks / "guard.py").write_text("payload v2\n", encoding="utf-8")
    manifest = install(
        target=target,
        source="package",
        package_root=package_root,
    ...
    by_path = {e["path"]: e for e in manifest["surfaces"]}
    # provenance_key is the implementation note implementation note additive receipt field.
    assert by_path["scripts/hooks"]["source"] == "shared"
    assert by_path["scripts/hooks"]["provenance_key"].startswith("package:")


def test_package_install_merges_carved_surface_per_child(tmp_path: Path) -> None:
    """Carved parents merge child-by-child (git_overlay parity), never skip wholesale.

    revC regression: a fresh package install into a consumer whose carved dir
    (Makefile.d/) already held any file used to mark the WHOLE surface local,
    so managed shared children silently never landed.
    """
    from workstate_bootstrap.install import install

    package_root = tmp_path / "payload"
    makefiles = package_root / "Makefile.d"
    makefiles.mkdir(parents=True)
    (makefiles / "plans.mk").write_text("payload plans\n", encoding="utf-8")
    (makefiles / "workflows.mk").write_text("payload workflows\n", encoding="utf-8")

    target = tmp_path / "consumer"
    local = target / "Makefile.d"
    local.mkdir(parents=True)
    (local / "custom.mk").write_text("consumer custom\n", encoding="utf-8")
    (local / "plans.mk").write_text("consumer plans\n", encoding="utf-8")
    ...
    # Unrelated consumer file and same-name override both keep local precedence…
    assert (local / "custom.mk").read_text(encoding="utf-8") == "consumer custom\n"
    assert (local / "plans.mk").read_text(encoding="utf-8") == "consumer plans\n"
    # …while the managed child still lands on first install.
    assert (local / "workflows.mk").read_text(encoding="utf-8") == "payload workflows\n"
    surfaces = manifest["surfaces"]
    assert {"path": "Makefile.d/plans.mk", "source": "local"} in surfaces
    by_path = {e["path"]: e for e in surfaces}
    assert by_path["Makefile.d/workflows.mk"]["source"] == "shared"
    assert {"path": "Makefile.d", "source": "local"} not in surfaces
...
def test_bootstrap_depends_on_workstate_system_for_package_source() -> None:
    ...
    dependencies = [
        dep.lower().replace("_", "-") for dep in pyproject["project"]["dependencies"]
    ]
