...
def test_overlay_resolver_and_doctor_agree_on_broken_shared_surface(
    ...
    _init_git_repo(target)
    url, ref = fake_remote_with_surfaces
    install(target=target, remote_url=url, remote_ref=ref, mcp_servers=SAMPLE_MCP_SERVERS)
...
def test_overlay_manifest_contract_documents_canonical_and_legacy() -> None:
    ...
    assert ".workstate-bootstrap.json" in text, "must document the canonical bootstrap ledger"
    assert ".workstate-overlay.json" in text, "must still document the legacy overlay manifest"
    lowered = text.lower()
...
def _seed_branch_review_override_at(override_root: Path, system_root: Path) -> None:
    ...
    manifest_payload = json.loads(
        (payload_root / "config" / "agent-workflows" / "portable_commands.json").read_text()
    )
...
def test_repair_restores_missing_effective_plugin_tree_when_overrides_exist(
    ...
    effective_root = target / ".workstate" / "generated" / "plugins" / "workstate-system" / "effective"
...
def test_stale_override_detection_uses_plugin_lock_not_stderr(tmp_path: Path) -> None:
    ...
    lock_dir = tmp_path / ".workstate" / "generated" / "plugins" / "workstate-system" / "effective"
