...
def test_check_file_edit_allows_code_on_feature_branch() -> None:
    patch = """*** Begin Patch
*** Update File: /repo/packages/agent-orchestrator-mcp/src/workstate_orchestrator_mcp/api.py
@@
-old
+new
*** End Patch"""
...
*** End Patch"""
    ...
    assert result == ("main", ["packages/agent-orchestrator-mcp/src/workstate_orchestrator_mcp/api.py"])
...
def test_hook_blocks_on_non_conforming_branch(tmp_path: Path) -> None:
    ...
    assert "AGENTIC_ALLOW_NONCONFORMING_BRANCH" in reason
...
def test_hook_allows_with_override_env_var(tmp_path: Path) -> None:
    """``AGENTIC_ALLOW_NONCONFORMING_BRANCH=1`` is the documented escape
    ...
    env = {**os.environ, "AGENTIC_ALLOW_NONCONFORMING_BRANCH": "1"}
