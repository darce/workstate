...
- Do not write findings under the coordinator `task_ref`. The coordinator
  merges reviewer findings via `review_findings(operation="merge")` after all
  reviewers return; writing directly produces interleaved rows with no
  provenance.
...
- Finding IDs must be stable and include the reviewer scope, e.g.
