...
`workstate-handoff-mcp` is the portable MCP server for agent coordination state. After the WORKSTATE-REF-6 event, review, next-action, and artifact-domain consolidation plus profile-removal stretch work, WORKSTATE-REF-8 verified-test search/read support, WORKSTATE-REF-23 observatory dashboard split, the WORKSTATE-REF-DASHBOARD-AUTOREGEN slice-complete decision-id preflight surface, and the WORKSTATE-REF-45 discriminated-operation flattening pass, it exposes a single **21-tool** MCP surface for task state, review findings, verification evidence, artifacts, touch ledger, integrity checks (working-tree / post-merge / close), session compaction, export/import, decision-id and write preflight, and DASHBOARD.txt generation. Orchestration, daemon lifecycle, lane management, and turn metrics are served by [`workstate-orchestrator-mcp`](workstate-orchestrator-mcp.md).
...
| `review_findings` | action | no | Typed review-findings domain surface. `review.operation` selects `record`, `batch_record`, `update`, `resolve`, or `list`. `resolve` is the commit-backed reconciliation path: it classifies selected open findings against the current workspace commit, marks only eligible findings `fixed`, reports `pending_uncommitted` when the fix is not yet commit-backed, and refuses ambiguous root-workspace writes instead of guessing. Any fixed-finding mutation through this path refreshes `DASHBOARD.txt` in the same action. |
...
Extends the WORKSTATE-REF-4 typed-tool-surface ADR (`packages/mcp-workstate-handoff/docs/tasks/WORKSTATE-REF-4-typed-tool-surface-consolidation-adr-task-plan.md`). The typed ops already give every tool one Pydantic-validated input surface; this convention governs how a tool's **closed value sets** are surfaced to agents.
...
Worked example — `touched_files` `change_kind` (WORKSTATE-REF-84): the field is `Literal["edit","add","delete"]` (announced as a schema `enum`), backed by `ChangeKind` (`touched_files.py`). The description maps git vocabulary — `edit` = modified content (git `M` / "modified"), `add` = new file (git `A`), `delete` = removed (git `D`) — because agents reflexively reach for git's "modified". The `record_file_touch` runtime guard still rejects unknown values for the CLI/direct path, and `test_change_kind_schema_enum_bound_to_changekind_source_of_truth` binds the schema `enum` to `CHANGE_KIND_VALUES` so adding a `ChangeKind` member without extending the announced `Literal` fails CI. As of WORKSTATE-REF-84, `change_kind` was the only closed-enum input field still typed as bare `str`; the other closed enums (`status`, `severity`, `verdict`, `review_mode`, `subject_kind`, terminal-guard `decision`, and the `operation`/`kind` discriminators) already use `Literal`.
...
## Cascade Archive of WORKSTATE-REF-PLANNING-REVIEW Rows

`archive(payload={"operation": "archive", "task_ref": "<WORKSTATE-REF-N>", "cascade_maint_review": False})`
...
`WORKSTATE-REF-PLANNING-REVIEW-*` row whose `objective` or `task_plan_path`
...
```python
get_handoff_state(
    task_ref="WORKSTATE-REF-36",
    sections="decisions_recent",
    decision_id_prefix="codex_slice_complete_WORKSTATE_36_",
...
- `finding_id` naming convention: prefix with the owning task-ref or review scope to minimize cross-scope collisions (e.g., `WORKSTATE-REF-12-3-001`, `REVIEW-COVERAGE-WORKSTATE-REF-12-3-001`). Global uniqueness is not schema-enforced; ambiguity errors serve as the collision safety net. Repo-scoped findings should use the `__repo__` task-ref prefix or the review subject path as the prefix.
...
As of WORKSTATE-REF-7, responses use compact serialization: no indentation, null/empty fields stripped, and no legacy field mirroring. MCP tool handlers return `dict` to FastMCP (which serializes once), eliminating double-serialization escapes in MCP responses. Core functions continue returning `str` for orchestrator in-process callers. The `data` block is the canonical payload; callers should read fields from `data`, not from top-level mirrors. In-process Python callers that need flat access should use `_flatten_v2()`.

```json
{"ok":true,"schema_version":2,"tool":"get_handoff_state","scope":{"task_ref":"WORKSTATE-REF-3"},"data":{"active":{...},"limits":{...},...},"task_ref":"WORKSTATE-REF-3"}
```
