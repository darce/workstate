...
Plugin trees are **skills-only** (implementation note,
`docs/plans/0028-mcp-registration-single-source-ownership.md`): the
...
```json
{
  "mcpServers": {
    "workstate-handoff-mcp": {
      "type": "stdio",
      "command": "uvx",
      "args": ["mcp-workstate-handoff@0.12.6", "--workspace-root", ".", "serve-stdio"]
    },
    "workstate-orchestrator-mcp": {
      "type": "stdio",
      "command": "uvx",
      "args": ["mcp-workstate-orchestrator[bridge]@0.6.3", "--workspace-root", ".", "serve-stdio"]
    }
  }
}
```
...
(WORKSTATE-REF-07 always-effective model). Bootstrap composes the effective tree on
