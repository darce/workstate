...
.workstate/generated/plugins/workstate-system/base/grok/
  .grok-plugin/plugin.json     # skills path only; no mcpServers (compat .mcp.json is authoritative)
...
```json
{
  "mcpServers": {
    "workstate-handoff-mcp": {
      "type": "stdio",
      "command": "uvx",
      "args": ["mcp-workstate-handoff@0.12.4", "--workspace-root", ".", "serve-stdio"]
    },
    "workstate-orchestrator-mcp": {
      "type": "stdio",
      "command": "uvx",
      "args": ["mcp-workstate-orchestrator[bridge]@0.6.1", "--workspace-root", ".", "serve-stdio"]
    }
  }
}
```
...
Codex delivery proof result: pass. The marketplace registered from the isolated repo root, `codex plugin marketplace list` showed `workstate-marketplace`, `codex plugin list --marketplace workstate-marketplace` showed `workstate-system@workstate-marketplace` available, and `codex plugin add workstate-system@workstate-marketplace` installed the plugin into `~/.codex/plugins/cache/workstate-marketplace/workstate-system/<plugin_version>/`. The installed cache contained `.codex-plugin/plugin.json`, `.mcp.json`, and the `skills/` directory. Codex may print non-blocking remote-plugin or icon warnings in disposable homes; plugin marketplace discovery and install still completed successfully.
...
1. Run `make dogfood` (or `workstate-bootstrap install`) from the consumer
   repo so bootstrap composes the effective plugin tree and materializes
   `.grok/plugins/workstate-system/` from
   `.workstate/generated/plugins/workstate-system/effective/grok/`.
...
3. Run `/hooks-trust` in the Grok session so plugin-delivered PreToolUse
   ...
   load) — the grok plugin manifest intentionally omits `mcpServers` so
   `grok inspect` lists each server exactly once.
...
- `grok inspect` lists project plugin `workstate-system` with eleven
  portable-command skills and two MCP servers (each server listed once —
...
4. Bump the developer-local `.mcp.json` (gitignored) and
   `workstate-bootstrap`'s `DEFAULT_MCP_SERVERS` in lockstep when MCP runtime
   pins changed.
