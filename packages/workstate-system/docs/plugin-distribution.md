...
in
[ADR-001: Agentic Plugin Distribution](agentic/adrs/ADR-001-agentic-plugin-distribution.md)
and
[ADR-003: Plugin Consumer Overrides](agentic/adrs/ADR-003-plugin-consumer-overrides.md);
...
| `config/agent-workflows/mcp_servers.yaml`                  | MCP server registration manifest; sources both the `mcpServers` block and the plugin version. |

## Emitted Layout

`make plugins-build` writes one tree per supported harness under
`.agentic/generated/plugins/workstate-system/base/{claude,codex}`. That is
...
.agentic/generated/plugins/workstate-system/base/codex/
  .codex-plugin/plugin.json    # same body as claude/.claude-plugin/plugin.json
  .mcp.json                    # byte-identical to base/claude/.mcp.json
...
The two `.mcp.json` blobs include `"type": "stdio"` so the launch shape
matches `workstate-bootstrap install` exactly:
...
```bash
make plugins-build   # emit .agentic/generated/plugins/workstate-system/base/{claude,codex}/
...
No-override consumers keep their marketplace pins pointed at `.agentic/generated/plugins/workstate-system/base/{claude,codex}`. Override-aware consumers repoint those marketplace pins to `.agentic/generated/plugins/workstate-system/effective/{claude,codex}`.
...
1. Run `make plugins-build` from the APM root (or in the consumer repo
   ...
   `.agentic/generated/plugins/workstate-system/base/claude/` exists.
...
3. **Fresh clone, one-time per checkout:** register the local
  ...
  `.agentic/generated/plugins/workstate-system/base/claude/` source. Skipping this step makes
...
> local `.agentic/generated/plugins/workstate-system/base/claude/` and
> `.agentic/generated/plugins/workstate-system/base/codex/` trees only; it is not yet
...
Claude delivery proof result: pass. Validation passed with one non-blocking warning (`description: No marketplace description provided`). The install succeeded at project scope, `claude plugin list --json` showed `workstate-system@workstate-marketplace` enabled with `mcp-workstate-handoff@0.11.5` and `mcp-workstate-orchestrator@0.4.7`, and `claude plugin details workstate-system@workstate-marketplace` listed 10 skills plus the two MCP servers.

### Codex

1. Run `make plugins-build` so `.agentic/generated/plugins/workstate-system/base/codex/`
   exists.
...
Codex delivery proof result: pass. The marketplace registered from the isolated repo root, `codex plugin marketplace list` showed `workstate-marketplace`, `codex plugin list --marketplace workstate-marketplace` showed `workstate-system@workstate-marketplace` available, and `codex plugin add workstate-system@workstate-marketplace` installed the plugin into `~/.codex/plugins/cache/workstate-marketplace/workstate-system/0.2.0/`. The installed cache contained `.codex-plugin/plugin.json`, `.mcp.json`, and the `skills/` directory. Codex printed a non-blocking warning because the disposable HOME was under the system temp directory and helper binaries were not added to PATH; plugin marketplace discovery and install still completed successfully.
...
claude --plugin-dir path/to/repo/.agentic/generated/plugins/workstate-system/base/claude /skills
```
...
- do not edit `.agentic/generated/plugins/workstate-system/base/...`,
  harness plugin caches, or
  `.agentic/generated/plugins/workstate-system/effective/...` by hand.
...
4. Confirm the generated body now lives under
   `.agentic/generated/plugins/workstate-system/effective/{claude,codex}/skills/<slug>/SKILL.md`
...
1. Create the skill source under
   `agentic-overrides/agentic-system/skills/<slug>/SKILL.md` with the
   normal `SKILL.md` frontmatter, for example `name` and `description`,
   followed by the skill body.
2. Declare it in `agentic-overrides/agentic-system/overrides.yaml` with
   `mode: add` and the relative file path. Added skills are repo-owned;
   they do not need an upstream digest.
3. Run:

   ```bash
   agentic-bootstrap install --plugin-overrides agentic-overrides/agentic-system
   ```

4. Confirm the generated body now lives under
   `.agentic/generated/plugins/agentic-system/effective/{claude,codex}/skills/<slug>/SKILL.md`
...
root under `.agentic/override-backups/<timestamp>/` before removal,
