...
`.workstate/generated/plugins/workstate-system/base/{claude,codex,cursor,grok}`.
...
.workstate/generated/plugins/workstate-system/base/cursor/
...
.workstate/generated/plugins/workstate-system/base/grok/
...
plugin tree — is a bug, and `workstate-bootstrap doctor` flags a stray
...
```json
{
  "mcpServers": {
    "workstate-handoff-mcp": {
      "type": "stdio",
      "command": "uvx",
      "args": ["mcp-workstate-handoff@0.13.2", "--workspace-root", ".", "serve-stdio"]
    },
    "workstate-orchestrator-mcp": {
      "type": "stdio",
      "command": "uvx",
      "args": ["mcp-workstate-orchestrator[bridge]@0.7.0", "--workspace-root", ".", "serve-stdio"]
    }
  }
}
```
...
```bash
make plugins-build   # emit .workstate/generated/plugins/workstate-system/base/{claude,codex,cursor,grok}/
...
| `.claude/settings.json`               | Claude  | Workstate-owned project settings: marketplace pin, `enabledPlugins`, contract-rendered guard hooks (`_managed_by: workstate-bootstrap`). |
...
`.claude/settings.json` is owned wholesale by workstate-bootstrap for the
...
committed belong in `workstate-overrides/workstate-system/` via
...
[Claude Code hooks](https://code.claude.com/docs/en/hooks)); workstate relies
...
1. Run `workstate-bootstrap install` or `workstate-bootstrap update` from
...
3. **Fresh clone, one-time per checkout:** register the local
   ...
   `.workstate/generated/plugins/workstate-system/effective/claude/` tree.
...
> trees only: `.workstate/generated/plugins/workstate-system/base/{claude,codex}`
...
Claude delivery proof result: pass (recorded pre-plan-0028, when the plugin tree still carried the MCP pins: `claude plugin list --json` showed the plugin enabled with the workstate MCP pins and `claude plugin details` listed 10 skills plus the two MCP servers). Post-plan-0028 expectation: `claude plugin details` lists the 10 skills only; the two MCP servers register once, from the root `.mcp.json` — a session listing a `plugin:workstate-system:` duplicate of either server is the dual-registration bug this plan removed. Re-proof pending the next sandbox pass.

### Codex

1. Run `workstate-bootstrap install` or `workstate-bootstrap update` so
   `.workstate/generated/plugins/workstate-system/effective/codex/` exists.
...
Codex delivery proof result: pass. The marketplace registered from the isolated repo root, `codex plugin marketplace list` showed `workstate-marketplace`, `codex plugin list --marketplace workstate-marketplace` showed `workstate-system@workstate-marketplace` available, and `codex plugin add workstate-system@workstate-marketplace` installed the plugin into `~/.codex/plugins/cache/workstate-marketplace/workstate-system/<plugin_version>/`. The installed cache contained `.codex-plugin/plugin.json`, `.mcp.json` (pre-plan-0028 tree; post-0028 trees are skills-only and Codex MCP registration moves to `.codex/config.toml`), and the `skills/` directory. Codex may print non-blocking remote-plugin or icon warnings in disposable homes; plugin marketplace discovery and install still completed successfully.
...
1. Run `make dogfood` (or `workstate-bootstrap install`) from the consumer
   repo so bootstrap composes the effective plugin tree, materializes
   `.grok/plugins/workstate-system/` from
   `.workstate/generated/plugins/workstate-system/effective/grok/`, and
   **activates** the plugin via the Grok CLI when `grok` is on `PATH`:
   `grok plugin install .grok/plugins/workstate-system --trust` followed by
   `grok plugin enable workstate-system` (bare-name selector only — bootstrap
   never writes unstable `project/<hash>/workstate-system` selectors).
   When `grok` is absent the install still succeeds and records a
   `configs[]` entry with `kind: grok_plugin_activation` and
   `action: skipped_no_cli` in `.workstate-bootstrap.json`;
   `workstate-bootstrap doctor` surfaces the gap with remediation text.
   ...
   `.gitignore` block covers `/.grok/plugins/workstate-system`).
...
4. Optional: opt in to the managed Stop adapter with
   ...
   the same flag to `workstate-bootstrap install`). Bootstrap writes
   `.grok/hooks/stop.json` and records the opt-in in
   `.workstate-bootstrap.json`.

#### Grok worktree adoption

Linked worktrees share the primary's `.workstate/generated` symlink but not
gitignored plugin surfaces. `workstate-bootstrap adopt` (also run by
`make task-start`) symlinks `.grok/plugins/workstate-system` to the
...
A worktree's checked-out copy of the git-tracked `.workstate-bootstrap.json`
...
make dogfood
grok inspect
grok plugin validate .workstate/generated/plugins/workstate-system/base/grok
workstate-bootstrap doctor
```

Expected after restart + `/hooks-trust`:

- `grok inspect` lists project plugin `workstate-system` **enabled** with
...
- `workstate-bootstrap doctor` reports no `grok_activation_drift` on a
  healthy install; stale `project/<hash>/workstate-system` selectors in
...
durable, repo-owned edits to *shipped* workstate recipes — review
...
1. **Fork.** Copy the current effective skill body twice into the override
   ...
   ```bash
   SLUG=branch-review
   SRC=.workstate/generated/plugins/workstate-system/effective/claude/skills/$SLUG/SKILL.md
   DST=workstate-overrides/workstate-system/skills/$SLUG
   mkdir -p "$DST"
...
3. **Update.** Re-run install/update/repair. Composition merges
   ...
   `workstate-bootstrap overrides status --target .` shows
...
5. **Accept upstream.** To re-baseline after reviewing the upstream change:
   ...
   ```bash
   workstate-bootstrap overrides accept-upstream --target . branch-review
   ```
...
1. Create the skill source under
   `workstate-overrides/workstate-system/skills/<slug>/SKILL.md` with the
...
4. When MCP runtime pins changed, run `make mcp-pins-sync` to regenerate
   `workstate-bootstrap`'s `DEFAULT_MCP_SERVERS` from `mcp_servers.yaml`
   (`make check-mcp-pins` guards the copy), then refresh the
   developer-local `.mcp.json` (gitignored) via `workstate-bootstrap
   mcp-sync`.
