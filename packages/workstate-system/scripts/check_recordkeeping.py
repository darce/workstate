#!/usr/bin/env python3
"""Lint workflow source files for recordkeeping discipline (implementation note implementation note).
...
2. ``make dashboard`` mentions inside cold-start orientation blocks.
   ...
   when findings are fixed (WORKSTATE-REF-82); the manual command is a
   deprecation alias.
"""
    ...
    for path in _iter_source_files(root):
        ...
        for line_no, _block_first in _make_dashboard_in_cold_start(text):
            errors.append(
                ...
                f"resolve_review_findings when findings are fixed (WORKSTATE-REF-82)."
            )
    return errors
