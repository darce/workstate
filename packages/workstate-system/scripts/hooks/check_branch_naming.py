...
_OVERRIDE_ENV_VAR = "AGENTIC_ALLOW_NONCONFORMING_BRANCH"
_OVERRIDE_REASON_ENV_VAR = "AGENTIC_ALLOW_NONCONFORMING_BRANCH_REASON"
...
_PUSH_OVERRIDE_ENV_VAR = "AGENTIC_ALLOW_NONCONFORMING_BRANCH_PUSH"
_PUSH_OVERRIDE_REASON_ENV_VAR = "AGENTIC_ALLOW_NONCONFORMING_BRANCH_PUSH_REASON"
...
def _format_warning(branch: str, suggested: str | None) -> str:
    ...
    lines.append(
        ...
        "AGENTIC_ALLOW_NONCONFORMING_BRANCH=1 to override (audited)."
    )
    return "\n".join(lines) + "\n"
...
def _record_override_decision_blocking(branch: str, reason: str | None) -> None:
    ...
    record_event(
        event={
            ...
            "rationale": (
                f"AGENTIC_ALLOW_NONCONFORMING_BRANCH override accepted for "
                f"branch '{branch}'.\n"
...
def run_pre_commit(
    ...
    *,
    ...
    - Non-conforming + ``AGENTIC_ALLOW_NONCONFORMING_BRANCH=1`` →
...
def _format_pre_push_block(branch: str) -> str:
    return (
        ...
        "  Note: the commit-side AGENTIC_ALLOW_NONCONFORMING_BRANCH override\n"
...
def _record_push_override_decision_blocking(branch: str, reason: str | None) -> None:
    ...
    record_event(
        event={
            ...
            "decision": decision_id,
            "rationale": (
                f"AGENTIC_ALLOW_NONCONFORMING_BRANCH_PUSH override accepted for "
                f"branch '{branch}'.\n"
                f"Reason: {reason or '(not provided)'}\n"
                f"Author: {_commit_author()}\n"
            ),
...
def run_pre_push(
    ...
    *,
    ...
    - reads ``AGENTIC_ALLOW_NONCONFORMING_BRANCH_PUSH`` (not the commit
