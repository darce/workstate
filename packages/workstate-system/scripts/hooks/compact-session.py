...
(``AGENTIC_HOOK_PROTOCOL_STRICT=1`` plus a malformed event payload):
...
def _resolve_harness() -> str:
    """Derive the harness label from AGENT_HANDOFF_HARNESS.

    The Stop hook is wired into multiple harnesses (Claude Code, Codex,
    Cursor, manual scripted runs); each launcher exports
    ``AGENT_HANDOFF_HARNESS`` so the compaction row reflects the actual
    ...
    """
    raw = os.environ.get("AGENT_HANDOFF_HARNESS", "").strip()
    if not raw:
        return "claude-code"
...
def _compact(
    *,
    ...
    built once in ``main()`` so the deprecation warning fires exactly
    once per invocation; this function never re-reads compaction env
    vars. See BR-WORKSTATE-REF-55-01.
    """
    try:
    ...
    # Honor AGENT_HANDOFF_STATE_DIR to mirror the MCP server CLI's
    # from_args resolution; production callers leave it unset and the
    # primary worktree's .task-state wins.
    state_dir_override = os.environ.get("AGENT_HANDOFF_STATE_DIR") or None
    try:
        configure_runtime(
...
def main() -> int:
    ...
    # here (not inside ``_compact``) means the deprecation warning for
    # any WORKSTATE_COMPACTION_* alias fires exactly once: the disabled-skip
    # path no longer pre-emits its own warning. See BR-WORKSTATE-REF-55-01.
    try:
    ...
    try:
        ...
        return 0
    for warning in settings.legacy_warnings:
        _emit(warning)
