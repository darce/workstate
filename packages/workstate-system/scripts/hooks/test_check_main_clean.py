"""Unit tests for ``check_main_clean.py`` mode flags (WORKSTATE-REF-53 implementation note).

This file is created by WORKSTATE-REF-53 implementation note to pin the new explicit
...
CONTRACT_SOURCE = PACKAGE_ROOT / "docs" / "agentic" / "contracts" / "harness-protocol.yaml"
...
def _seed_contract_from(repo: Path, source: Path) -> None:
    ...
    ``docs/agentic/contracts/harness-protocol.yaml`` relative to the
...
def feature_branch_repo(tmp_path: Path) -> Path:
    ...
    subprocess.run(
        ["git", "-C", str(repo), "checkout", "-q", "-b", "feature/WORKSTATE-53-x"],
        check=True,
    )
    return repo
...
# WORKSTATE-REF-53 implementation note: publish-boundary tuning. The git hooks run
...
def test_package_script_prefers_local_hook_helpers_over_repo_mirror(clean_main_repo: Path) -> None:
    ...
    WORKSTATE-REF-03 added ``find_dirty_state_files`` to the package-local helper. The
...
# ---------------------------------------------------------------------------
# WORKSTATE-REF-03 implementation note: ``check-main-clean`` tripwires only on state-file dirt.
#
...
def test_planning_artifact_dirt_on_main_does_not_block_pre_push(clean_main_repo: Path) -> None:
    ...
    WORKSTATE-REF-51 close-out friction was operators being told to stash and
...
def test_gitignored_task_state_spools_do_not_block_pre_push(clean_main_repo: Path) -> None:
    ...
    not the canonical state files WORKSTATE-REF-03's hard block protects.
    """
    repo = clean_main_repo
...
def test_stale_root_contract_without_state_surface_uses_package_fallback(clean_main_repo: Path) -> None:
    """A stale root overlay must not silently disable WORKSTATE-REF-03's state-file
    tripwire in a source checkout.
    """
    repo = clean_main_repo
    stale_contract = repo / "docs" / "agentic" / "contracts" / "harness-protocol.yaml"
...
def test_post_merge_hook_no_longer_uses_hard_block_flag() -> None:
    ...
    WORKSTATE-REF-53 implementation note retunes the publish boundary so routine post-merge
    ...
    assert "--block" not in invocations, (
        "WORKSTATE-REF-53 implementation note: post-merge git hook must not pass --block "
        f"to check_main_clean.py:\n{invocations}"
    )
    assert "--mode" in invocations, (
        "WORKSTATE-REF-53 implementation note: post-merge git hook should opt into the "
...
def test_pre_push_hook_keeps_hard_block_flag() -> None:
    ...
    assert "--block" in invocations or "--mode block" in invocations, (
        "WORKSTATE-REF-53 implementation note: pre-push git hook must still hard-block "
