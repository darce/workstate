"""Resolve every `docs/workstate/rules/*.md` path cited from a hook script.
...
canonical home is `packages/workstate-system/docs/workstate/rules/`; a
...
materializes the symlink under `<consumer>/docs/workstate/rules/`.

Hoisting note: `docs/workstate/rules` is NOT in `workstate-bootstrap`'s
...
RULES_SEARCH_ROOTS = (
    REPO_ROOT / "packages" / "workstate-system" / "docs" / "workstate" / "rules",
    REPO_ROOT / "docs" / "workstate" / "rules",
)
...
# Match: docs/workstate/rules/<name>.md   or   docs/workstate/rules/<name>.md#anchor
_CITATION_RE = re.compile(
    r"docs/workstate/rules/(?P<rel>[A-Za-z0-9_\-./]+\.md)(?:#(?P<anchor>[A-Za-z0-9_\-]+))?"
)
...
def test_hook_citations_exist() -> None:
    ...
    assert citations, (
        "expected at least one docs/workstate/rules/*.md citation in hook "
    ...
    for source, lineno, rel, _anchor in citations:
        if _resolve_rules_path(rel) is None:
            missing.append(f"{source.relative_to(REPO_ROOT)}:{lineno} -> docs/workstate/rules/{rel}")
    assert not missing, (
        "hook scripts cite docs/workstate/rules/*.md paths that do not "
        "resolve under either the package tree "
        "(packages/workstate-system/docs/workstate/rules/) or the clone root "
        "(docs/workstate/rules/). Operators see these citations on a "
    ...
    for source, lineno, rel, anchor in citations:
        ...
        if anchor not in by_file[target]:
            bad.append(
                ...
                f"docs/workstate/rules/{rel}#{anchor} (anchor not found)"
            )
    assert not bad, (
