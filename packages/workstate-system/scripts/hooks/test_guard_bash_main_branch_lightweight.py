"""WORKSTATE-REF-03 implementation note: regression coverage for the Bash main-branch surface.
...
- **Plain branch creation is allowed.** ``git checkout -b
  feature/WORKSTATE-03-smoke`` from a main worktree must exit 0 — the
...
- **The legacy planning-artefact surface still blocks Bash writes.**
  ...
  WORKSTATE-REF-03 transition for back-compat with this scanner; an ``echo > ``
...
def test_plain_branch_creation_allowed_on_main(main_repo: Path) -> None:
    proc = _invoke(main_repo, "git checkout -b feature/WORKSTATE-03-smoke")
...
def test_maint_branch_creation_allowed_on_main(main_repo: Path) -> None:
    proc = _invoke(main_repo, "git checkout -b WORKSTATE-REF-cleanup-2026-05-23")
    assert proc.returncode == 0, proc.stderr
    assert "BLOCKED" not in proc.stderr


def test_branch_creation_with_set_upstream_allowed(main_repo: Path) -> None:
    proc = _invoke(main_repo, "git checkout -b feature/WORKSTATE-03-smoke && git push -u origin HEAD")
    assert proc.returncode == 0, proc.stderr
