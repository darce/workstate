      ...
      * If SKILL.md is a real file with frontmatter, split it into
        ...
        <package>/skills/<slug>/.
            * If SKILL.md is a dangling symlink, rescue the source from
                WORKSTATE_SKILL_RESCUE_SOURCE/<slug>/SKILL.md.
...
_RESCUE_SOURCE_RAW = os.environ.get("WORKSTATE_SKILL_RESCUE_SOURCE")
...
def _resolve_source(slug_dir: pathlib.Path) -> pathlib.Path:
    ...
    if RESCUE_SOURCE is None:
        raise SystemExit(
            f"{slug_dir.name}: SKILL.md is missing or dangling and WORKSTATE_SKILL_RESCUE_SOURCE is unset"
        )
