...
# (e.g. when the consumer manages its own venv). The legacy
# `AGENT_HANDOFF_PLAN_CLI` name is still honored for the cutover release. Any
...
PLAN_CLI=${WORKSTATE_HANDOFF_PLAN_CLI:-${AGENT_HANDOFF_PLAN_CLI:-uvx --from mcp-workstate-handoff python -m workstate_handoff_mcp.plan_cli}}
