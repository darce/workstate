...
LEGACY_MCP_BIN_ENV = "MCP_AGENT_HANDOFF_BIN"
...
LEGACY_MCP_ORCHESTRATOR_BIN_ENV = "MCP_AGENT_ORCHESTRATOR_BIN"
...
def live_active_statuses() -> tuple[str, ...]:
    ...
    ``packages/workstate-system/docs/agentic/contracts/workstate-handoff-mcp.md``:
...
def mcp_handoff_bin() -> str:
    return os.environ.get(MCP_BIN_ENV) or os.environ.get(LEGACY_MCP_BIN_ENV) or DEFAULT_MCP_BIN
...
def run_checklist_sync(repo: Path, task_ref: str) -> dict[str, Any]:
...
def mcp_orchestrator_bin() -> str:
    return (
        os.environ.get(MCP_ORCHESTRATOR_BIN_ENV)
        or os.environ.get(LEGACY_MCP_ORCHESTRATOR_BIN_ENV)
        or DEFAULT_MCP_ORCHESTRATOR_BIN
    )
