"""Read-only ``context`` subcommand (implementation note implementation note).
...
``docs/plans/0009-git-first-lifecycle-primitives.md``.
"""
...
def run(argv: list[str]) -> int:
    ...
    if (
        ...
        if status in ("spooled", "pending", "error"):
            # WORKSTATE-REF-52 implementation note: surface ``spooled`` (CLI rejection) in
    ...
    if isinstance(plan_path, str) and isinstance(active_task_ref, str) and active_task_ref:
        ...
        # WORKSTATE-REF-72 implementation note: when the evaluator says the baseline is
