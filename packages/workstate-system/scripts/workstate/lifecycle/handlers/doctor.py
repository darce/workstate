...
REMEDIATION_DIRTY = [
    ...
    "see docs/agentic/rules/development-workflow.md#root-main-is-a-control-plane",
]


_PROBE_TIMEOUT_SECONDS = 0.5
# Bounded retry/backoff for MCP reachability probes. The per-attempt timeout
# stays at _PROBE_TIMEOUT_SECONDS so warm calls return in well under the
# previous budget. Inter-attempt sleeps cap total wall-clock near ~5s
# (3 sleeps of 0.5+1.0+2.0 = 3.5s plus up to 4 * 0.5s probe budgets ≈ 5.5s
# worst case before declaring unreachable). The first successful attempt is the
# fast path; later success surfaces ``mcp_status="warming"`` so callers can
# distinguish cold-start from outage. NOTE: this ~5.5s is the budget for a
# single probe. `_probe_mcp` runs two sequential probes (handoff then
# orchestrator), so `make doctor` compounds to ~2x (~11s) only when BOTH
# endpoints genuinely miss the budget; the common attempt-0-success path on
# each probe stays sub-second.
_PROBE_BACKOFF_SLEEPS = (0.5, 1.0, 2.0)
...
def _probe_mcp(repo: Path) -> tuple[DoctorMcp, list[ReceiptWarning]]:
    ...
    - the agent-handoff endpoint via ``_common.run_handoff_json`` →
      ``mcp_status``;
    - the agent-orchestrator endpoint via ``_common.run_orchestrator_json``
      → ``orchestrator_status``.
...
def _load_compact_session_adapters() -> list[dict[str, Any]]:
    """Return the compact-session Stop adapter declarations from the manifest.

    Missing or malformed manifests degrade to an empty list so the doctor
    receipt stays renderable on a checkout without the manifest.
    """
    try:
    ...
    for hook in manifest.get("hooks", []) or []:
        if hook.get("hook_id") != "compact-session":
            continue
...
def _stop_adapter_installed(repo: Path, target: str) -> bool:
    """True when ``repo/target`` carries a managed compact-session Stop entry."""
    path = repo / target
    try:
        ...
        return False
    stop_entries = (
        data.get("hooks", {}).get("Stop", []) if isinstance(data, dict) else []
    )
    if not isinstance(stop_entries, list):
        return False
    return any(
        isinstance(entry, dict) and entry.get("_managed_by") == _MANAGED_BY
...
def _probe_hooks(
    ...
    *,
    ...
    """Report compact-session adapter and git-hook hoist state.

    Adapter *availability* comes from the package manifest. Bootstrap-reported
    managed drift takes precedence, then *installed* state is checked against
    the inspected ``repo``. Git-hook hoist readiness is read from the inspected
    checkout's ``core.hooksPath`` so a linked worktree can be diagnosed
    independently of the primary tree. Optional-not-installed adapters and an
    unset ``core.hooksPath`` are reported but never fail the doctor receipt.
    """
    available: list[str] = []
    installed: list[str] = []
    drifted: list[str] = []
    optional_not_installed: list[str] = []
    remediation: list[str] = []
    drifted_paths = _hook_adapter_drift_paths(env_findings)

    for adapter in _load_compact_session_adapters():
        ...
        if target_path in drifted_paths:
            drifted.append(key)
            remediation.append(
                f"repair the {key} stop adapter via `workstate-bootstrap repair`"
            )
        elif _stop_adapter_installed(repo, target_path):
            ...
            if opt_in_flag:
                remediation.append(
                    f"install the {key} stop adapter via "
