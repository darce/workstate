"""``plan-accept`` subcommand (WORKSTATE-REF-69 implementation note).
...
Gates (per Plan WORKSTATE-REF-69 §Acceptance criteria):
...
def _query_handoff_identity(repo: Path, task_ref: str) -> tuple[dict[str, Any] | None, str]:
    ...
    finding WORKSTATE72-FULL-BR-01).
    """
...
def run(argv: list[str]) -> int:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "WORKSTATE-REF-row review task ref instead of the implementation task. "
    ...
    if already_accepted:
        # WORKSTATE-REF-05 implementation note: an accepted baseline is a satisfied lifecycle
