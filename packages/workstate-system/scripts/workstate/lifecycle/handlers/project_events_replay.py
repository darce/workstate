"""``project-events-replay`` subcommand (implementation note implementation note.2).
...
canonical receipt schema so consumers can diff fixtures by-byte. Only
``test_result`` entries are drained today — ``decision`` and
``state_sync`` kinds round-trip through their own projection helpers
and will land alongside in a follow-up sub-slice.
"""
...
def _replay_entry(repo: Path, entry: dict[str, Any]) -> str:
    ...
    if proc.returncode in _CLI_UNREACHABLE_RETURNCODES:
        return "pending"
    # WORKSTATE-REF-52 implementation note: CLI ran and rejected the payload — keep it in
...
def run(argv: list[str]) -> int:
    ...
    # WORKSTATE-REF-52 implementation note: prefer ``spooled`` when *any* entry was actively
