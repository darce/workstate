"""``review-ready`` subcommand (implementation note implementation note).
...
# WORKSTATE-REF-53 implementation note: reason -> owner bucket so close-check / review-ready
...
def reasons_by_owner(reasons: list[str]) -> dict[str, list[str]]:
    """Group ``reasons`` into the canonical owner buckets (WORKSTATE-REF-53 implementation note).
...
def next_command_for(
    *,
    ...
    Mirrors the WORKSTATE-REF-42 / WORKSTATE-REF-53 ``status`` next-command grammar so
    callers can route on a single field across the whole workflow loop.

    WORKSTATE-REF-53 implementation note (finding WORKSTATE53-S5-BR-01): when ``derived_task_ref``
    ...
    for raw in proc.stdout.splitlines():
        ...
        if is_planning_path(path):
            warnings.append(
                ...
                "see docs/agentic/rules/planning-artifact-home.md"
            )
    return warnings
...
def _query_open_findings(
    ...
    # finding BR-WORKSTATE40-S5-04-01).
    argv = [
...
def _probe_checklist_sync_pending(
    ...
    checklist items on the active task's plan (WORKSTATE-REF-70 implementation note).

    Reuses the WORKSTATE-REF-64 ``sync_task_plan_checklist`` parse/resolve pipeline
...
def _query_active_task_identity(
    ...
    fail-closed semantics for the plan-baseline gate (WORKSTATE-REF-72 PR-04).
...
def augment_with_handoff_state(
    ...
    * :func:`_query_active_task_identity` + :func:`evaluate_plan_baseline` —
      ...
      retry of the gate (WORKSTATE72-S3-BR-01).

      The gate is fail-closed (WORKSTATE-REF-72 PR-04) when MCP is reachable
    ...
    if not identity_ok and mcp_reachable:
        ...
        # already be accepted (WORKSTATE72-S3-BR-01).
...
def run(argv: list[str]) -> int:
    ...
    # WORKSTATE-REF-70 implementation note: warn-only checklist guardrail. review-ready is
