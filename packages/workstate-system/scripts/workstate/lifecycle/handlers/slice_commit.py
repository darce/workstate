...
landed on a descendant (WORKSTATE-REF-SLICE-COMMIT-PROJECTION-20260509).
...
``sync-task-plan-checklist --apply --quiet`` (WORKSTATE-REF-64 implementation note; ordering
fix from WORKSTATE-REF-64-BR-01) so any box the sync flips lands inside the
...
def _project_commit_sha(
    ...
    directly via the explicit-actor channel introduced in WORKSTATE-REF-51
