"""Mutating ``slice-start`` subcommand (implementation note implementation note).
...
def _run_test_cmd(
    ...
    ``env`` overrides the child environment when provided (WORKSTATE-REF-07: used to
...
def run(argv: list[str]) -> int:
    ...
    # WORKSTATE-REF-41 implementation note: defensive ``uv sync --extra dev`` so a forgotten
    ...
    # WORKSTATE-REF-07 implementation note: refresh package sync AND root ``.venv`` provisioning so
