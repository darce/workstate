...
8. Merge the reviewed branch and return the root worktree to `main`. Do **not** delete the feature branch by hand — `make task-finish` does it as the final step (after the linked worktree is gone), so the branch is still live for the handoff write-side guard at the time the row is set to `done` and archived.
...
10. Run `make task-finish TASK=<task-ref>` so teardown, archive, dashboard regeneration, feature-branch deletion, and a final full-plan `sync-task-plan-checklist` sweep happen in the repo's canonical order. The branch delete uses `git branch -d` (safe variant — git itself refuses unmerged branches) and only fires for a fully-merged `target_branch` that is not the current HEAD and not checked out elsewhere; the receipt's `branch_status` surfaces `deleted` / `skipped_unmerged` / `skipped_missing` / `skipped_unset` / `skipped_primary` / `skipped_checked_out` / `failed` for diagnosis.
...
- If a task-plan `- [ ]` box did not auto-tick after `make slice-commit` / `make task-finish`, the evidence anchor in the item's body did not match a recorded `close_slice` decision's `changed_files`, a recorded `test_result` command, or an explicit decision id reference — verify the anchor or tick manually. The sync never unticks (one-way ratchet) and never auto-ticks `## Stretch Goals` items.
...
1. Detect the mismatch:
   ...
   - If they differ and the active task is not `WORKSTATE-REF-*`, you are cold-starting in the wrong shell.
...
3. Hook layering (see `harness-protocol.yaml:127-169`):
   - `advise-worktree-cd.py` — `SessionStart` / `UserPromptSubmit` advisory. Proactive nudge; may stay silent for `WORKSTATE-REF-*`, ambiguous task, or missing `target_worktree_path`.
   ...
   - MCP writes (`record_event`, `close_slice`, `review_findings`, `review_runs`, `get_handoff_state`, …) are covered by the rationale/shape hooks and by WORKSTATE-REF-44 write-context attribution; an MCP write from the wrong cwd may warn or attribute to the cwd-resolved task rather than block. The runbook's job is to prevent that drift, not to rely on a blocker that does not fire here.
   - If the implementation task's `target_branch` has no linked worktree yet, do **not** call `set_handoff_state`, `record_event`, `close_slice`, `review_findings`, or `review_runs` against that implementation task. The write side fails with `WorktreeNotFoundError`. Safe choices are: run `make task-start TASK=<task-ref> OBJECTIVE="..."` once the accepted baseline exists, use a `target_branch=main` `WORKSTATE-REF-*` row for planning/review work, or wait until the feature worktree exists.
