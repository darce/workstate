...
0. **Ensure task scope before any cwd-resolving MCP read.** `search_handoff`, `review_findings`, `get_handoff_state` without `task_ref` all resolve from cwd. On cold start:
   ...
   - **Ad-hoc investigation on main:** register a maintenance task first — `set_handoff_state(task_ref="WORKSTATE-REF-<slug>-<YYYYMMDD>", objective="...", status="in_progress", target_branch="main")`.
