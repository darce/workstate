...
- Martin Fowler with Kent Beck, `Refactoring: Improving the Design of Existing Code`, local extracted text at `/Users/daniel/Development/context-alt-text-monorepo/literature/extracted/refactoring/Refactoring-Improving-the-Design-of-Existing-Code-MartinFowlerKentBeck.txt`.

Use the local text as a reference source for smell names, refactoring names, and the behavior-preserving discipline. Do not copy long passages from it. Paraphrase, cite short method names, and tie recommendations to inspected code evidence.
