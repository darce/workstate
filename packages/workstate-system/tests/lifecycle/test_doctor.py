...
def _write_managed_stop_adapter(
    repo: Path, target: str, *, shape: str = "flat"
) -> None:
    """Seed a managed compact-session Stop entry.

    ``shape="flat"`` is the codex/vscode adapter shape (and the pre-fix
    broken Claude shape); ``shape="claude"`` is the nested matcher+hooks[]
    shape Claude Code requires.
    """
    ...
    if shape == "claude":
        ...
        entry = {"_managed_by": "workstate-bootstrap", "command": command}
    path = repo / target
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps({"hooks": {"Stop": [entry]}}))
...
_CLAUDE_SHARED_ADAPTER_TARGET = ".claude/settings.json"


def test_hooks_facet_detects_installed_nested_claude_adapter(git_repo: Path) -> None:
    """A claude-code Stop entry in the nested matcher+hooks[] shape is installed."""
    _write_managed_stop_adapter(git_repo, _CLAUDE_SHARED_ADAPTER_TARGET, shape="claude")

    _, hooks = _probe_hooks_via_handler(git_repo)

    assert any(
        _CLAUDE_SHARED_ADAPTER_TARGET in a for a in hooks.stop_adapters_installed
    ), f"nested claude adapter missing from {hooks.stop_adapters_installed!r}"
    assert not any(
        _CLAUDE_SHARED_ADAPTER_TARGET in a for a in hooks.stop_adapters_drifted
    )


def test_hooks_facet_flags_stale_flat_claude_adapter_as_drift(git_repo: Path) -> None:
    """The pre-fix FLAT claude entry (shape Claude Code ignores) must read as
    drift with a re-install remediation, never as healthy/installed."""
    _write_managed_stop_adapter(git_repo, _CLAUDE_SHARED_ADAPTER_TARGET, shape="flat")

    _, hooks = _probe_hooks_via_handler(git_repo)

    assert any(
        _CLAUDE_SHARED_ADAPTER_TARGET in a for a in hooks.stop_adapters_drifted
    ), "a stale flat claude entry must classify as drifted"
    assert not any(
        _CLAUDE_SHARED_ADAPTER_TARGET in a for a in hooks.stop_adapters_installed
    ), "a stale flat claude entry must not be reported installed"
    assert not any(
        _CLAUDE_SHARED_ADAPTER_TARGET in a
        for a in hooks.stop_adapters_optional_not_installed
    ), "a present managed entry must not be reported as never opted in"
    assert any(
        "stale" in r.lower() and _CLAUDE_SHARED_ADAPTER_TARGET in r
        for r in hooks.remediation
    ), "stale claude adapters must surface a re-install remediation"


def test_hooks_facet_vscode_flat_adapter_still_installed(git_repo: Path) -> None:
    """The flat shape stays legitimate for non-claude harnesses (vscode/codex)."""
    _write_managed_stop_adapter(git_repo, _VSCODE_ADAPTER_TARGET, shape="flat")

    _, hooks = _probe_hooks_via_handler(git_repo)

    assert any(_VSCODE_ADAPTER_TARGET in a for a in hooks.stop_adapters_installed), (
        "flat vscode adapter must still classify as installed"
    )
    assert not any(_VSCODE_ADAPTER_TARGET in a for a in hooks.stop_adapters_drifted)
...
def test_WORKSTATE80_hooks_facet_reports_managed_adapter_drift_not_optional(
    ...
    assert any(
        "repair" in r.lower() and _VSCODE_ADAPTER_TARGET in r
        for r in hooks.remediation
    ), "managed drift must surface a drift->repair remediation for the target"


# ---------------------------------------------------------------------------
# WS-REINJ-01 implementation note: doctor generalization — every manifest hook family is
# reported via the additive ``hook_adapters`` facet field, keyed by hook_id.
# The legacy ``stop_adapters_*`` keys stay compact-session-only (additive
# receipt contract; the WORKSTATE-REF-80 assertions above must keep passing
# unchanged).
# ---------------------------------------------------------------------------


def _write_managed_reinject_adapter(repo: Path, target: str) -> None:
    """Seed a managed reinject-context SessionStart entry (nested Claude shape)."""
    entry = {
        "_managed_by": "workstate-bootstrap",
        "matcher": "",
        "hooks": [
            {
                "type": "command",
                "command": f"{repo}/scripts/hooks/reinject-context.py",
            }
        ],
    }
    path = repo / target
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps({"hooks": {"SessionStart": [entry]}}))


def test_hooks_facet_hook_adapters_reports_every_manifest_family(
    git_repo: Path,
) -> None:
    """``hook_adapters`` covers all manifest hooks[] families, including the
    reinject-context SessionStart family; with no adapter files everything
    is optional-not-installed."""
    _, hooks = _probe_hooks_via_handler(git_repo)

    families = hooks.hook_adapters
    assert "compact-session" in families
    assert "reinject-context" in families, (
        "doctor must discover the reinject-context family from the manifest"
    )
    reinject = families["reinject-context"]
    assert reinject["available"], "reinject adapters must be discovered"
    assert reinject["installed"] == []
    assert sorted(reinject["optional_not_installed"]) == sorted(reinject["available"])
    # Legacy keys stay compact-session-only.
    assert not any("reinject" in a for a in hooks.stop_adapters_available), (
        "stop_adapters_* must not absorb the reinject family (additive contract)"
    )
    assert any("--install-claude-reinject-hook" in r for r in hooks.remediation), (
        "optional reinject adapters must surface their opt-in flag remediation"
    )


def test_hooks_facet_detects_installed_reinject_adapter(git_repo: Path) -> None:
    """A managed SessionStart entry in the shared settings file reads as
    installed for the reinject family and never as drift."""
    _write_managed_reinject_adapter(git_repo, _CLAUDE_SHARED_ADAPTER_TARGET)

    _, hooks = _probe_hooks_via_handler(git_repo)

    reinject = hooks.hook_adapters["reinject-context"]
    assert any(_CLAUDE_SHARED_ADAPTER_TARGET in a for a in reinject["installed"]), (
        f"installed reinject adapter missing from {reinject['installed']!r}"
    )
    assert not any(_CLAUDE_SHARED_ADAPTER_TARGET in a for a in reinject["drifted"])
    # The compact-session family must NOT read this SessionStart-only file
    # as an installed Stop adapter.
    assert not any(
        _CLAUDE_SHARED_ADAPTER_TARGET in a for a in hooks.stop_adapters_installed
    ), "a SessionStart-only settings file must not satisfy the Stop family"
...
# --- WORKSTATE-REF-DOCTOR-PROBE-BUDGET-20260605: per-attempt probe budget ------------


def test_probe_timeout_default_covers_cli_cold_start() -> None:
    """Each probe attempt spawns a fresh CLI process (full Python import
    cost); observed cold start is ~1.6s (worktree venv bin) to ~2.7s
    (pyenv shim). The previous 0.5s per-attempt budget timed out every
    attempt, so doctor reported a permanent false ``mcp=unreachable`` and
    the ``warming`` state was unreachable by construction. The default
    budget must cover the observed cold start with margin."""
    from workstate.lifecycle.handlers import doctor as doctor_handler  # noqa: PLC0415

    assert doctor_handler._probe_timeout_seconds() >= 3.0, (
        "WORKSTATE-REF-DOCTOR-PROBE-BUDGET: default per-attempt probe budget must "
        "cover CLI cold start (~2.7s observed via pyenv shim); got "
        f"{doctor_handler._probe_timeout_seconds()!r}"
    )


def test_probe_timeout_env_override_reaches_runner(
    git_repo: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """``WORKSTATE_DOCTOR_PROBE_TIMEOUT_SECONDS`` overrides the per-attempt
    budget and is passed verbatim to the probe runner (operators on slow or
    fast hosts can tune without a code change)."""
    from workstate.lifecycle.handlers import _common as common_mod  # noqa: PLC0415
    ...
    monkeypatch.setenv("WORKSTATE_DOCTOR_PROBE_TIMEOUT_SECONDS", "0.25")
    monkeypatch.setattr(common_mod, "run_handoff_json", _fake_probe)
    _stub_orchestrator_unreachable(monkeypatch)
    monkeypatch.setattr("shutil.which", lambda _name: "/usr/bin/true")

    _probe_mcp_via_handler(git_repo)

    assert seen["timeout"] == 0.25, (
        "WORKSTATE-REF-DOCTOR-PROBE-BUDGET: env override must reach the runner; "
        f"got {seen.get('timeout')!r}"
    )


def test_probe_timeout_env_invalid_falls_back_to_default(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Garbage, non-positive, or empty override values fall back to the
    default rather than raising or silently producing a 0s budget."""
    from workstate.lifecycle.handlers import doctor as doctor_handler  # noqa: PLC0415

    default = doctor_handler._PROBE_TIMEOUT_DEFAULT_SECONDS
    for bad in ("banana", "-1", "0", ""):
        monkeypatch.setenv("WORKSTATE_DOCTOR_PROBE_TIMEOUT_SECONDS", bad)
        assert doctor_handler._probe_timeout_seconds() == default, (
            "WORKSTATE-REF-DOCTOR-PROBE-BUDGET: invalid override "
            f"{bad!r} must fall back to the default {default!r}"
        )
