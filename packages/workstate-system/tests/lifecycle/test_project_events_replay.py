...
* ``pending_remaining``: ``int`` — entries left in the spool after the
  replay run.
* ``replay_results``: ``list[{"kind": str, "status": "synced" | "pending" | "spooled"}]``
...
# ---------------------------------------------------------------------------
# WS-TASKSTART: ``state_sync`` entries (spooled by a task-start whose ``set``
# could not land online) are now drained too — by re-issuing the ``set`` and
# applying the same exit-0-on-``ok:false`` guard the online path uses.
# ---------------------------------------------------------------------------


def _state_sync_entry(**overrides: object) -> dict[str, object]:
    entry: dict[str, object] = {
        "kind": "state_sync",
        "task_ref": "WS-REPLAY-01",
        "target_branch": "feature/ws-replay-01",
        "target_worktree_path": "/tmp/wt-ws-replay-01",
        "task_plan_path": None,
        "objective": "drain me",
        "status": "in_progress",
        "branch": "feature/ws-replay-01",
...
def test_replay_drains_state_sync_with_full_set_argv(
    git_repo: Path, tmp_path: Path
) -> None:
    """A spooled ``state_sync`` entry replays through ``set`` with the full
    identity argv (mirroring the online projection) and drains on ``ok:true``.
    """
    spool = git_repo / PENDING_REL
    spool.parent.mkdir(parents=True, exist_ok=True)
    spool.write_text(json.dumps(_state_sync_entry()) + "\n")
    fake_cli = tmp_path / "fake-mcp"
    argv_log = tmp_path / "argv.log"
    _write_fake_cli(
        fake_cli,
    ...
    assert receipt["pending_remaining"] == 0
    assert receipt["replay_results"] == [{"kind": "state_sync", "status": "synced"}]
    assert receipt["handoff_projection"] == "synced"
    assert not spool.exists() or spool.read_text() == ""

    blocks = _split_argv_blocks(argv_log)
    set_blocks = [b for b in blocks if "set" in b]
    assert len(set_blocks) == 1, blocks
    args = set_blocks[0]
    for flag, expected in (
        ("--task-ref", "WS-REPLAY-01"),
        ("--target-branch", "feature/ws-replay-01"),
        ("--target-worktree-path", "/tmp/wt-ws-replay-01"),
        ("--status", "in_progress"),
        ("--objective", "drain me"),
        ("--branch", "feature/ws-replay-01"),
        ("--commit-sha", "abcd1234" * 5),
    ):
        assert flag in args, f"missing {flag} in {args!r}"
        assert args[args.index(flag) + 1] == expected, args
    # No plan path on this entry → flag omitted.
    assert "--task-plan-path" not in args


def test_replay_state_sync_benign_row_exists_is_drained(
    git_repo: Path, tmp_path: Path
) -> None:
    """The benign exit-0 ``ok:false`` (row already exists → update needs
    ``expected_revision``, signalled by ``current_revision``) means the
    desired row is live, so the entry is DRAINED, not kept spooling forever.
    """
    spool = git_repo / PENDING_REL
    spool.parent.mkdir(parents=True, exist_ok=True)
    spool.write_text(json.dumps(_state_sync_entry()) + "\n")
    ...
    _write_fake_cli(
        ...
        'for updates.","current_revision":4}}\'\n'
        'exit 0\n',
    )
...
def test_replay_state_sync_zero_exit_real_rejection_stays_spooled(
    git_repo: Path, tmp_path: Path
) -> None:
    """The core guard: a zero-exit ``ok:false`` with NO ``current_revision``
    is a genuine rejection. A returncode-only classifier would have falsely
    drained it (the WS-TASKSTART silent no-op); the entry must instead stay
    spooled and the receipt report ``spooled``.
    """
    spool = git_repo / PENDING_REL
    spool.parent.mkdir(parents=True, exist_ok=True)
    spool.write_text(json.dumps(_state_sync_entry()) + "\n")
    ...
    _write_fake_cli(
        fake_cli,
        '#!/usr/bin/env bash\n'
        'echo \'{"ok":false,"data":{"error":"some real rejection"}}\'\n'
        'exit 0\n',
    )
    ...
    assert proc.returncode == 0, proc.stderr
    receipt = json.loads(proc.stdout)
    assert receipt["drained"] == 0, receipt
    assert receipt["pending_remaining"] == 1
    assert receipt["replay_results"] == [{"kind": "state_sync", "status": "spooled"}]
    assert receipt["handoff_projection"] == "spooled"
    remaining = [json.loads(line) for line in spool.read_text().splitlines() if line]
    assert len(remaining) == 1
    assert remaining[0]["task_ref"] == "WS-REPLAY-01"


def test_replay_test_result_zero_exit_ok_false_stays_spooled(
    git_repo: Path, tmp_path: Path
) -> None:
    """The exit-0 guard is generic: a ``test_result`` replay whose
    ``record_event`` returns a zero-exit ``ok:false`` must also stay spooled
    instead of being falsely drained (record_event shares the always-exit-0
    CLI contract).
    """
    spool = git_repo / PENDING_REL
    spool.parent.mkdir(parents=True, exist_ok=True)
    ...
    assert receipt["drained"] == 0, receipt
    assert receipt["pending_remaining"] == 1
    assert receipt["replay_results"] == [{"kind": "test_result", "status": "spooled"}]
