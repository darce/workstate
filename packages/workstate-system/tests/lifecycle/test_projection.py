...
def fake_cli(tmp_path: Path):
    ...
    def _set(body: str) -> Path:
        ...
        os.environ["MCP_AGENT_HANDOFF_BIN"] = str(target)
        return target

    yield _set
    os.environ.pop("MCP_AGENT_HANDOFF_BIN", None)
...
def test_project_decision_missing_cli_spools_pending(
    ...
    os.environ["MCP_AGENT_HANDOFF_BIN"] = "/nonexistent/path/no-such-binary-xyz"
    try:
        status, returned = projection.project_decision(
            repo_root,
