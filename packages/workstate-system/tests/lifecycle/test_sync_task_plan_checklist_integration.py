...
LIFECYCLE_MK_SOURCE = PACKAGE_ROOT / "Makefile.d" / "lifecycle.mk"
...
def _make_env(repo: Path, shim: Path) -> dict[str, str]:
    ...
    env["AGENT_HANDOFF_STATE_DIR"] = str(repo / ".task-state")
    env["AGENT_HANDOFF_CURRENT_TASK_PATH"] = str(repo / "CURRENT_TASK.json")
    env["AGENT_HANDOFF_DASHBOARD_PATH"] = str(repo / "DASHBOARD.txt")
...
def test_make_task_finish_runs_checklist_sync_post_step(tmp_path: Path) -> None:
    """task-finish's recipe-level post-step must invoke a full-plan sync
    after archive and merge the receipt under ``checklist_sync`` (WORKSTATE-REF-64
    implementation note)."""
    repo = _consumer_repo(tmp_path)
    task_ref = "WORKSTATE-REF-64"
    plan = repo / "docs" / "tasks" / "WORKSTATE-REF-64-plan.md"
