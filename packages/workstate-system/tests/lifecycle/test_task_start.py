...
# ---------------------------------------------------------------------------
# implementation note S1: worktree bootstrap hook (WORKSTATE_WORKTREE_BOOTSTRAP_CMD)
# ---------------------------------------------------------------------------


def _write_bootstrap_sentinel_script(
    path: Path, *, exit_code: int = 0, body: str | None = None
) -> None:
    if body is None:
        body = (
            "#!/usr/bin/env bash\n"
            'echo "ran" > .bootstrap-sentinel\n'
            f"exit {exit_code}\n"
        )
...
def test_task_start_worktree_bootstrap_runs_with_worktree_cwd(
    git_repo: Path,
    fake_cli_dir: Path,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Env set -> bootstrap runs with cwd = worktree root; receipt ok."""
    fake_cli_dir.mkdir()
    fake_cli = fake_cli_dir / "mcp-workstate-handoff"
    _write_fake_cli(fake_cli, "#!/usr/bin/env bash\nexit 0\n")
    sentinel = tmp_path / "bootstrap-ok.sh"
    _write_bootstrap_sentinel_script(sentinel)
    monkeypatch.setenv("WORKSTATE_WORKTREE_BOOTSTRAP_CMD", str(sentinel))
    ...
    receipt = json.loads(proc.stdout)
    wt = Path(receipt["worktree_path"])
    assert receipt["worktree_bootstrap"] == {
        "ran": True,
        "ok": True,
        "skipped": None,
        "exit": 0,
    }
    assert (wt / ".bootstrap-sentinel").read_text().strip() == "ran"


def test_task_start_worktree_bootstrap_skipped_when_env_unset(
    git_repo: Path, fake_cli_dir: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Unset/empty env -> bootstrap not run; receipt ran=false."""
    fake_cli_dir.mkdir()
    fake_cli = fake_cli_dir / "mcp-workstate-handoff"
    _write_fake_cli(fake_cli, "#!/usr/bin/env bash\nexit 0\n")
    monkeypatch.delenv("WORKSTATE_WORKTREE_BOOTSTRAP_CMD", raising=False)
...
def test_task_start_worktree_bootstrap_skipped_when_env_empty(
    git_repo: Path,
    fake_cli_dir: Path,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Explicit empty-string env disables the hook like an unset var.

    Covers the ``LIFECYCLE_WORKTREE_BOOTSTRAP=`` bridge path (Make forwards an
    empty value, not an absent var) and an empty TIMEOUT, which must fall back
    to the default rather than raise.
    """
    fake_cli_dir.mkdir()
    fake_cli = fake_cli_dir / "mcp-workstate-handoff"
    _write_fake_cli(fake_cli, "#!/usr/bin/env bash\nexit 0\n")
    sentinel = tmp_path / "bootstrap-should-not-run.sh"
    _write_bootstrap_sentinel_script(sentinel)
    monkeypatch.setenv("WORKSTATE_WORKTREE_BOOTSTRAP_CMD", "")
    monkeypatch.setenv("WORKSTATE_WORKTREE_BOOTSTRAP_TIMEOUT", "")
    ...
    assert not (Path(receipt["worktree_path"]) / ".bootstrap-sentinel").exists()


def test_task_start_worktree_bootstrap_best_effort_on_failure(
    git_repo: Path,
    fake_cli_dir: Path,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Failing bootstrap (exit 1) is best-effort: task-start ok, no rollback."""
    fake_cli_dir.mkdir()
    fake_cli = fake_cli_dir / "mcp-workstate-handoff"
    _write_fake_cli(fake_cli, "#!/usr/bin/env bash\nexit 0\n")
    sentinel = tmp_path / "bootstrap-fail.sh"
    _write_bootstrap_sentinel_script(sentinel, exit_code=1)
    monkeypatch.setenv("WORKSTATE_WORKTREE_BOOTSTRAP_CMD", str(sentinel))
    ...
    assert _git(git_repo, "branch", "--list", "feature/ws-wtboot-s1c")


def test_task_start_worktree_bootstrap_best_effort_on_timeout(
    git_repo: Path,
    fake_cli_dir: Path,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Timeout maps to exit 124; task-start stays ok (best-effort)."""
    fake_cli_dir.mkdir()
    fake_cli = fake_cli_dir / "mcp-workstate-handoff"
    _write_fake_cli(fake_cli, "#!/usr/bin/env bash\nexit 0\n")
    sentinel = tmp_path / "bootstrap-sleep.sh"
    _write_bootstrap_sentinel_script(
        sentinel,
        body="#!/usr/bin/env bash\nsleep 5\n",
    )
    monkeypatch.setenv("WORKSTATE_WORKTREE_BOOTSTRAP_CMD", str(sentinel))
    monkeypatch.setenv("WORKSTATE_WORKTREE_BOOTSTRAP_TIMEOUT", "1")
    ...
    receipt = json.loads(proc.stdout)
    assert receipt["ok"] is True
    assert receipt["worktree_bootstrap"]["ran"] is True
    assert receipt["worktree_bootstrap"]["ok"] is False
    assert receipt["worktree_bootstrap"]["exit"] == 124


def test_task_start_worktree_bootstrap_best_effort_on_missing_command(
    git_repo: Path,
    fake_cli_dir: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Missing interpreter/command maps to exit 127; task-start stays ok."""
    fake_cli_dir.mkdir()
    fake_cli = fake_cli_dir / "mcp-workstate-handoff"
    _write_fake_cli(fake_cli, "#!/usr/bin/env bash\nexit 0\n")
    monkeypatch.setenv(
        "WORKSTATE_WORKTREE_BOOTSTRAP_CMD",
        "/nonexistent/bootstrap-binary-xyz",
    )
    ...
    assert receipt["worktree_bootstrap"]["ran"] is True
    assert receipt["worktree_bootstrap"]["ok"] is False
    assert receipt["worktree_bootstrap"]["exit"] == 127


def test_task_start_here_mode_skips_worktree_bootstrap(
    git_repo: Path,
    fake_cli_dir: Path,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """MODE=here excludes bootstrap by mode gate (worktree_path == primary)."""
    fake_cli_dir.mkdir()
    fake_cli = fake_cli_dir / "mcp-workstate-handoff"
    _write_fake_cli(fake_cli, "#!/usr/bin/env bash\nexit 0\n")
    sentinel = tmp_path / "bootstrap-should-not-run.sh"
    ...
    assert not (git_repo / ".bootstrap-sentinel").exists()


def test_task_start_claim_mode_runs_worktree_bootstrap(
    git_repo: Path,
    fake_cli_dir: Path,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """MODE=claim runs bootstrap on the claimed linked worktree."""
    fake_cli_dir.mkdir()
    fake_cli = fake_cli_dir / "mcp-workstate-handoff"
    _write_fake_cli(fake_cli, "#!/usr/bin/env bash\nexit 0\n")
    existing_wt = tmp_path / "claim-wt"
    subprocess.run(
        [
            "git",
            "-C",
            ...
            "-q",
            "-b",
            "feature/ws-wtboot-s1g",
            str(existing_wt),
        ],
        check=True,
    )
    sentinel = tmp_path / "bootstrap-claim.sh"
    _write_bootstrap_sentinel_script(sentinel)
    monkeypatch.setenv("WORKSTATE_WORKTREE_BOOTSTRAP_CMD", str(sentinel))
    ...
    assert receipt["worktree_bootstrap"]["ran"] is True
    assert receipt["worktree_bootstrap"]["ok"] is True
    assert (existing_wt / ".bootstrap-sentinel").exists()
