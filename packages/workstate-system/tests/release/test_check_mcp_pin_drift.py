"""Drift guard for the managed-MCP-server pin surfaces (implementation note:
canonical mcp_servers.yaml + generated bootstrap _mcp_pins.py + coupled docs).

Reproduces the v0.1.22 ship bug (orchestrator bumped 0.5.0 -> 0.5.1 while the
pin surfaces kept saying @0.5.0) as a regression and pins the guard's two entry
...
def _build_repo(
    ...
    *,
    ...
    orchestrator_pins_pin: str = "0.5.1",
    orchestrator_yaml_pin: str = "0.5.1",
    orchestrator_doc_pin: str = "0.5.1",
    handoff_pins_pin: str | None = None,
    handoff_yaml_pin: str | None = None,
    handoff_doc_pin: str | None = None,
    handoff_consumer_pin: str | None = None,
    orchestrator_consumer_pin: str | None = None,
    extra_pins_line: str | None = None,
    omit_yaml_orchestrator: bool = False,
    omit_doc: bool = False,
    omit_consumer_doc: bool = False,
    ...
    h_install = handoff_pins_pin if handoff_pins_pin is not None else handoff_pyproject
    ...
    # Generated copy: DEFAULT_MCP_SERVERS in bootstrap _mcp_pins.py (plan
    # 0028; rendered by `make mcp-pins-sync`, text-scanned here).
    # ``extra_pins_line`` injects a stray <dist>@<ver> reference elsewhere in
    ...
    _write(
        ...
        f'        "args": ["mcp-workstate-orchestrator@{orchestrator_pins_pin}", "serve-stdio"],\n'
        "    },\n"
        "}\n" + (f"{extra_pins_line}\n" if extra_pins_line else ""),
    )

    # Canonical pin site: mcp_servers.yaml. ``omit_yaml_orchestrator`` drops
    # the orchestrator pin entirely (a deleted canonical pin is drift while
    # the generated copy still carries the server).
    yaml_text = (
        "mcp_servers:\n"
    ...
    # Coupled surface: docs/CONSUMER.md. Same tolerance contract as
    # plugin-distribution.md — absent is OK, a stale pin anywhere in it is drift.
    if not omit_consumer_doc:
        h_consumer = (
            handoff_consumer_pin
            if handoff_consumer_pin is not None
            else handoff_pyproject
        )
        o_consumer = (
            orchestrator_consumer_pin
            if orchestrator_consumer_pin is not None
            else orchestrator_pyproject
        )
        _write(
            repo / "docs" / "CONSUMER.md",
            "# Consuming the workstate\n\n"
            f"| `workstate-handoff-mcp` | `uvx mcp-workstate-handoff@{h_consumer} ...` |\n"
            f"| `workstate-orchestrator-mcp` | `uvx mcp-workstate-orchestrator@{o_consumer} ...` |\n",
        )
...
def test_single_site_drift_is_isolated(tmp_path: Path) -> None:
    ...
    repo = _build_repo(
        tmp_path,
        orchestrator_pins_pin="0.5.0",  # stale
    ...
    blob = "\n".join(messages)
    assert not ok
    # The generated-copy line is DRIFT; the canonical yaml line is ok.
...
def test_cli_exit_code_on_drift(tmp_path: Path) -> None:
    module = _load_module()
    repo = _build_repo(tmp_path, orchestrator_pins_pin="0.5.0")
...
def test_handoff_drift_is_detected(tmp_path: Path) -> None:
    ...
    generated _mcp_pins.py surface (vs its 0.12.0 pyproject) while orchestrator stays
    ...
    module = _load_module()
    repo = _build_repo(tmp_path, handoff_pins_pin="0.11.9")  # pyproject is 0.12.0
    ok, messages = module.check_all(repo)
    blob = "\n".join(messages)
    assert not ok
...
def test_stray_version_reference_is_drift(tmp_path: Path) -> None:
    ...
    repo = _build_repo(
        tmp_path,
        extra_pins_line="# legacy note: previously mcp-workstate-orchestrator@0.4.0",
    )
    ok, messages = module.check_all(repo)
    blob = "\n".join(messages)
    assert not ok
...
def test_pre_release_pin_does_not_truncate_to_base_triple(tmp_path: Path) -> None:
    ...
    module = _load_module()
    repo = _build_repo(tmp_path, orchestrator_pins_pin="0.5.1rc1")
    ok, messages = module.check_all(repo)
    assert not ok
    drift_lines = [m for m in messages if "DRIFT" in m]
    assert any("DEFAULT_MCP_SERVERS" in m for m in drift_lines)
...
def test_deleted_primary_pin_is_drift(tmp_path: Path) -> None:
    """Removing the orchestrator pin entirely from the canonical site is drift;
    the server stays 'managed' via the generated copy (_mcp_pins.py)."""
    module = _load_module()
...
def test_consumer_doc_stale_pin_is_drift(tmp_path: Path) -> None:
    """docs/CONSUMER.md is a coupled pin surface (WS-PKGDOCS-01): a stale
    consumer-facing pin must be reported as drift naming the file."""
    module = _load_module()
    repo = _build_repo(
        tmp_path,
        orchestrator_consumer_pin="0.4.7",  # stale vs pyproject 0.5.1
    )
    ok, messages = module.check_all(repo)
    blob = "\n".join(messages)
    assert not ok
    drift_lines = [m for m in messages if "DRIFT" in m]
    assert any("CONSUMER.md" in m for m in drift_lines), blob
    # The aligned primary surfaces stay ok — drift is isolated to the doc.
    assert "ok" in blob


def test_absent_consumer_doc_is_tolerated(tmp_path: Path) -> None:
    """An absent docs/CONSUMER.md is tolerated, like the other coupled doc."""
    module = _load_module()
    repo = _build_repo(tmp_path, omit_consumer_doc=True)
    ok, messages = module.check_all(repo)
    assert ok, "\n".join(messages)
...
def test_pin_workflow_docs_describe_single_source_model() -> None:
    """implementation note implementation note: operator-facing pin-workflow prose must describe the
    single-source model (canonical mcp_servers.yaml -> generated _mcp_pins.py
    via `make mcp-pins-sync`), not the retired two-hand-copies lockstep."""
    consumer = (REPO_ROOT / "docs" / "CONSUMER.md").read_text(encoding="utf-8")
    assert "mcp_servers.yaml" in consumer, (
        "docs/CONSUMER.md MCP registration section must name the canonical "
        "pin source mcp_servers.yaml (implementation note)"
    )
    assert "kept in lockstep with the installer's" not in consumer, (
        "docs/CONSUMER.md still describes the retired hand-maintained "
        "DEFAULT_MCP_SERVERS lockstep; pins are generated since implementation note"
    )

    release_prepare = SOURCE_RELEASE_PREPARE.read_text(encoding="utf-8")
    assert "the two pin sites" not in release_prepare, (
        "scripts/release_prepare.py comment still describes two hand-"
        "maintained pin sites; the bootstrap copy is generated via "
        "`make mcp-pins-sync` since implementation note"
    )
...
def test_release_bump_gate_passes_when_pins_match(tmp_path: Path) -> None:
    ...
    repo = _build_repo(
        tmp_path,
        orchestrator_pins_pin="0.5.2",
        orchestrator_yaml_pin="0.5.2",
        orchestrator_doc_pin="0.5.2",
        orchestrator_consumer_pin="0.5.2",
    )
    ok, messages = module.check_release_bump(
        repo, "mcp-workstate-orchestrator", "0.5.2"
    )
