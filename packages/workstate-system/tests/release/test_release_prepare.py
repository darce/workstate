...
def _build_prepare_fixture(tmp_path: Path, *, version: str) -> Path:
    ...
    # release_prepare imports its sibling pin-drift guard at module load, so the
    # fixture repo must ship it too. The fixture bumps only non-server packages
    # whose distributions are absent from the (also absent) pin sites, so the
    # guard is a clean no-op here.
    (repo / "scripts" / "check_mcp_pin_drift.py").write_text(
