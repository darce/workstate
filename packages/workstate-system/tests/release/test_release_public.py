...
def test_publishers_verified_passes_preflight_without_probing(tmp_path: Path) -> None:
    ...
    result = _run(
        repo, env, "--execute", "--assume-yes", "--preflight-only",
        "--publishers-verified", "--json",
    )
...
def test_execute_runs_export_push_tagsync_in_order(tmp_path: Path) -> None:
    ...
    main_push_idx = next(
        i for i, ln in enumerate(lines) if "push --force test-remote HEAD:refs/heads/main" in ln
    )
    ...
    tag_push = next(
        (i for i, ln in enumerate(lines) if "push --force test-remote" in ln and "-v1.2.3" in ln),
        None,
    )
    ...
    # pushed tag set.
    assert any("push --force test-remote" in ln and " v0." in f" {ln} " for ln in lines), lines
