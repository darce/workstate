...
def test_generate_agent_workflows_writes_expected_files(tmp_path: Path) -> None:
    manifest = _write_temp_manifest(
        ...
        {
            "version": 1,
            "global_instructions": ["keep user-facing replies terse"],
            "commands": [
                {
    ...
    codex_router_out = temp_repo / "docs" / "workstate" / "generated"
    ...
    assert "Global instructions:\n\n- keep user-facing replies terse\n\n" in prompt_body
    assert "Load the `branch-review` skill" in prompt_body
    assert "Test stub." in prompt_body

    claude_body = (temp_repo / "CLAUDE.md").read_text()
    assert "Global instructions:\n\n- keep user-facing replies terse\n\n" in claude_body

    router_body = (codex_router_out / "codex-command-router.md").read_text()
    assert "Global instructions:\n\n- keep user-facing replies terse\n\n" in router_body


def test_generate_agent_workflows_omits_global_instructions_when_absent(
    tmp_path: Path,
) -> None:
    """Backward compat: a manifest without global_instructions renders no
    'Global instructions:' block on any surface."""
    manifest = _write_temp_manifest(
        tmp_path,
        {
            "version": 1,
    ...
    for surface in (
        prompts_out / "branch-review.prompt.md",
        codex_router_out / "codex-command-router.md",
        temp_repo / "CLAUDE.md",
        temp_repo / "docs" / "workstate" / "instructions.md",
    ):
        assert "Global instructions:" not in surface.read_text(), surface


def test_generate_agent_workflows_rejects_invalid_global_instructions(
    tmp_path: Path,
) -> None:
    """The manifest validation gate must refuse malformed global_instructions."""
    for invalid in ("not-a-list", [""], ["ok", 42], None):
        manifest = _write_temp_manifest(
            tmp_path,
            {
                "version": 1,
                "global_instructions": invalid,
                "commands": [
                    {
                        "command_id": "branch-review",
                        "skill": "branch-review",
        ...
        assert proc.returncode != 0, f"accepted invalid global_instructions: {invalid!r}"
        assert "global_instructions" in proc.stderr, proc.stderr
...
def test_default_mode_writes_generated_outputs_outside_payload(tmp_path: Path) -> None:
    """Regression for the implementation note S3 parents-math bug (review findings
    revA/revB ``payload-generated-leak``): in source-of-truth mode (no
    ``--target``) the generator must anchor generated adapters at the
    package-internal dir OUTSIDE ``workstate_system/payload/`` — never inside the
    shipped payload tree. The payload move pushed the manifest two levels deeper,
    so the sentinel root must be ``manifest.parents[2].parents[1]`` (payload ->
    workstate_system -> packages/workstate-system), not the payload root itself.
    Before the fix the sentinels resolved to the payload tree, polluting the
    wheel and tripping ``test_no_internal_material_under_payload``."""
    # Mirror the real package nesting so the generator's parents-arithmetic runs
    # against the shipped layout: <pkg>/workstate_system/payload/config/...
    payload = (
        tmp_path / "packages" / "workstate-system" / "workstate_system" / "payload"
    )
    manifest = payload / "config" / "agent-workflows" / "portable_commands.json"
    manifest.parent.mkdir(parents=True, exist_ok=True)
    manifest.write_text(
        json.dumps(
            {
    ...
    skill_dir = payload / "skills" / "scope"
    skill_dir.mkdir(parents=True, exist_ok=True)
    (skill_dir / "skill.yaml").write_text(
        "name: scope\n"
        "description: Test stub for scope.\n"
        "scope: harness\n"
        "mode: advisory\n"
        "context_budget: 8000\n"
        "makefile_target: make noop\n"
    ...
    pkg = tmp_path / "packages" / "workstate-system"
    # Generated artifacts land at the package-internal dir, OUTSIDE payload.
    assert (
        pkg / "docs" / "workstate" / "generated" / "codex-command-router.md"
    ).exists(), "default-mode generation must write the router doc outside payload"
    assert (pkg / ".github" / "prompts" / "scope.prompt.md").exists(), (
        "default-mode generation must write Copilot prompts outside payload"
    )
    # Nothing generated may leak back into the shipped payload tree.
    assert not (payload / "docs" / "workstate" / "generated").exists(), (
        "generated/ must never appear under workstate_system/payload/"
    )
    assert not (payload / ".github" / "prompts").exists(), (
        ".github/prompts/ must never appear under workstate_system/payload/"
    )
