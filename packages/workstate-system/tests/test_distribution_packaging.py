...
def test_wheel_includes_overlay_payload(tmp_path: Path) -> None:
    ...
    assert any(
        n.startswith("workstate_system/payload/skills/") and n.endswith("/body.md")
        for n in names
    ), "skills source must ship"
    assert (
        "workstate_system/payload/scripts/generate_agent_workflows.py" in names
    ), "generator must ship"
    assert (
        "workstate_system/payload/config/agent-workflows/portable_commands.json" in names
    ), "agent-workflows manifest must ship"
    assert any(
        n.startswith("workstate_system/payload/scripts/hooks/") for n in names
    ), "shared hook surface must ship"
    assert any(
        n.startswith("workstate_system/payload/docs/workstate/contracts/") for n in names
