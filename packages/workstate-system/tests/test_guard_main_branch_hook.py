...
HOOK_PATH = PACKAGE_ROOT / "scripts" / "hooks" / "guard-main-branch.sh"
HOOKS_DIR = PACKAGE_ROOT / "scripts" / "hooks"
CONTRACT_PATH = (
    PACKAGE_ROOT / "docs" / "agentic" / "contracts" / "harness-protocol.yaml"
)
...
def _build_fixture_repo(tmp_path: Path) -> Path:
    ...
    (repo / "docs" / "agentic" / "contracts").mkdir(parents=True)
    (repo / "docs" / "agentic" / "contracts" / "harness-protocol.yaml").symlink_to(
        CONTRACT_PATH
    )
