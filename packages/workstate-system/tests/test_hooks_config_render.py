...
def test_grok_plugin_hooks_json_has_no_post_tool_use_family() -> None:
    """implementation note implementation note / WORKSTATE-REF-06: grok plugin hooks stay PreToolUse-only."""
    module = _generator_module()
    contract = yaml.safe_load(CONTRACT_PATH.read_text())
    payload = json.loads(module._render_grok_plugin_hooks_json(contract))
    assert "PostToolUse" not in payload.get("hooks", {})
