...
def test_plugin_distribution_doc_records_grok_activation_flow() -> None:
    text = DOC_PATH.read_text()
    required_anchors = (
        "grok plugin install .grok/plugins/workstate-system --trust",
        "grok plugin enable workstate-system",
        "grok_plugin_activation",
        "Grok worktree adoption",
        "adopt receipt",
        "grok_activation_drift",
    )
    missing = [anchor for anchor in required_anchors if anchor not in text]
    assert not missing, (
        "plugin-distribution.md must document automated Grok activation and "
        f"worktree adoption: {missing}"
    )


def test_plugin_distribution_doc_pin_bump_uses_generated_model() -> None:
    """implementation note implementation note: the pin-bump checklist must route MCP pin changes
    through the canonical manifest + `make mcp-pins-sync` regeneration, not
    instruct the operator to hand-edit `DEFAULT_MCP_SERVERS` in lockstep."""
    text = DOC_PATH.read_text()
    assert "make mcp-pins-sync" in text, (
        "pin-bump checklist must reference `make mcp-pins-sync` as the way "
        "DEFAULT_MCP_SERVERS is regenerated from mcp_servers.yaml (implementation note)"
    )
    assert "`DEFAULT_MCP_SERVERS` in lockstep" not in text, (
        "stale two-hand-copies instruction: DEFAULT_MCP_SERVERS is generated "
        "from mcp_servers.yaml since implementation note; the doc must not tell the "
        "operator to hand-bump it in lockstep"
    )
