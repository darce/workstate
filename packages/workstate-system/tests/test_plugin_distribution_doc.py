...
EPIC_PATH = PACKAGE_ROOT.parents[1] / "docs" / "epics" / "agentic-plugin-distribution-epic.md"
ADR_003_PATH = (
    PACKAGE_ROOT / "docs" / "agentic" / "adrs" / "ADR-003-plugin-consumer-overrides.md"
)
...
def test_plugin_distribution_doc_distinguishes_base_and_effective_install_roots() -> None:
    text = DOC_PATH.read_text()
    required_anchors = (
        "No-override consumers keep their marketplace pins pointed at `.agentic/generated/plugins/workstate-system/base/{claude,codex}`.",
        "Override-aware consumers repoint those marketplace pins to `.agentic/generated/plugins/workstate-system/effective/{claude,codex}`.",
    )
    missing = [anchor for anchor in required_anchors if anchor not in text]
    assert not missing, (
        "plugin-distribution.md must distinguish base-tree pins from override-aware "
        f"effective-tree pins: {missing}"
    )
