...
def test_mcp_json_pins_uvx_servers_with_stdio_type(emitted_tree: Path) -> None:
    ...
    for name, expected_pkg in (
        ("workstate-handoff-mcp", "mcp-workstate-handoff@0.12.0"),
        ("workstate-orchestrator-mcp", "mcp-workstate-orchestrator@0.5.0"),
    ):
        entry = servers[name]
...
def test_plugin_overrides_patch_mcp_servers_and_emit_audit_receipt(
    ...
    assert handoff["args"] == ["mcp-workstate-handoff@0.12.0", "--profile", "consumer"]
