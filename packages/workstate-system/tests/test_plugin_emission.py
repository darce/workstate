...
def test_mcp_json_pins_uvx_servers_with_stdio_type(plugin_owned_tree: Path) -> None:
    ...
    for harness in ("claude", "codex"):
        ...
        for name, expected_pkg in (
            ("workstate-handoff-mcp", "mcp-workstate-handoff@0.12.5"),
            ("workstate-orchestrator-mcp", "mcp-workstate-orchestrator[bridge]@0.6.2"),
        ):
            entry = servers[name]
...
def _mcp_patch_override_root(tmp_path: Path, *, mode: str = "patch") -> Path:
    ...
    ops = (
        [{"op": "replace_args", "value": ["mcp-workstate-handoff@0.12.5", "--x"]}]
        if mode == "patch"
        else [
            {"op": "replace_command", "value": "uvx"},
