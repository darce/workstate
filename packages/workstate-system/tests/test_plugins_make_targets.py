...
def _run_make(
    target: str, *, plugin_out: Path | None = None
...
def test_plugins_build_emits_per_harness_trees(isolated_plugin_root: Path) -> None:
    ...
    assert (
        isolated_plugin_root / "claude" / ".claude-plugin" / "plugin.json"
    ).is_file()
    assert (isolated_plugin_root / "codex" / ".codex-plugin" / "plugin.json").is_file()
    # implementation note root ownership: plugin trees are skills-only, no MCP pins.
    assert not (isolated_plugin_root / "claude" / ".mcp.json").exists()
    assert not (isolated_plugin_root / "codex" / ".mcp.json").exists()
...
def test_plugins_check_fails_when_tree_is_mutated(isolated_plugin_root: Path) -> None:
    ...
    mutated = isolated_plugin_root / "claude" / ".claude-plugin" / "plugin.json"
    original = mutated.read_text()
    mutated.write_text(original.replace("workstate-system", "workstate-mutated"))
...
def test_plugins_build_default_matches_bootstrap_marketplace_root() -> None:
    ...
    assert ".workstate/generated/plugins/workstate-system/base" in proc.stdout
