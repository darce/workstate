...
Three adapter surfaces must surface the manifest's interaction mode so
consumers can route correctly:
  - .claude/commands/<id>.md  (Claude Code command stubs)
  - .github/prompts/<id>.prompt.md  (VS Code Copilot prompts)
  - codex command-router (CLAUDE.md, docs/agentic/instructions.md, and
...
def test_claude_command_stub_surfaces_mode_for_guide() -> None:
    out = gen._render_claude(_GUIDE_COMMAND)
    assert "Mode: `guide`" in out, (
        f"Claude command stub must surface `Mode: \\`guide\\``; got:\n{out}"
    )


def test_claude_command_stub_surfaces_mode_for_verify() -> None:
    out = gen._render_claude(_VERIFY_COMMAND)
    assert "Mode: `verify`" in out


def test_claude_command_stub_surfaces_mode_for_write() -> None:
    out = gen._render_claude(_WRITE_COMMAND)
