...
def test_review_parallel_skill_declares_harness_routing() -> None:
    ...
    rows = protocol["harness_capabilities"]["review_parallel"]["rows"]
    labels = {row["label"] for row in rows}
    primitives = {row["primitive"] for row in rows}
    required_labels = {
        "Claude Code (coordinator is an in-process Claude agent)",
        "Codex (coordinator is a Codex agent)",
        "Copilot / VS Code",
        "Grok (coordinator is a Grok agent)",
    }
    assert required_labels <= labels
    assert any("Agent` tool" in primitive for primitive in primitives)
    assert any("run_structured_turn" in primitive for primitive in primitives)
    assert any("task` tool" in primitive for primitive in primitives)
    assert any("BackendAdapter" in primitive for primitive in primitives)
    content = SKILL_PATH.read_text()
    assert "<!-- BEGIN GENERATED: harness-routing-table -->" in content
...
def test_review_parallel_skill_covers_orchestrator_unreachable_in_fallback() -> None:
    """Round-3 review finding
    codex_copilot_review_parallel_fallback_unreachable_when_orchestrator_fails:
    with the `[bridge]` uvx pin, a failed orchestrator *launch* (offline host /
    mirror without the bridge wheel) takes down both the probe surface and the
    Codex/Copilot fan-out primitive. The degradation contract must state that
    an unreachable orchestrator MCP is itself a negative probe signal and that
    the deterministic fallback runs in the coordinator's own process — it must
    not route through the orchestrator server."""
    content = SKILL_PATH.read_text()
    heading = "### Capability detection & deterministic degradation contract"
    section = content.split(heading, 1)[1].split("\n## ", 1)[0].split("\n### ", 1)[0]
    required_markers = (
    ...
    missing = [m for m in required_markers if m not in section]
    assert not missing, f"contract section missing orchestrator-down markers: {missing}"
