...
def rendered_prompts_dir(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """Render the Copilot prompt adapters from source into a temp tree.

    implementation note S2: the generated adapters are no longer committed, so the
    "guidance survives source -> rendered adapter" contract is verified against
    a fresh render rather than a checked-in file. ``--target`` contains every
    emitted surface under the temp dir (and makes the codex-router sync a no-op,
    since the temp target has no marker-bearing consumer docs).
    """
...
def test_rendered_handoff_lifecycle_prompt_carries_compaction_guidance(
    rendered_prompts_dir: Path,
) -> None:
    body = (rendered_prompts_dir / "handoff-lifecycle.prompt.md").read_text(
        encoding="utf-8"
    )
...
def test_branch_lifecycle_source_and_generated_adapter_carry_root_venv_recovery(
    rendered_prompts_dir: Path,
) -> None:
    ...
    source = (PAYLOAD_ROOT / "skills" / "branch-lifecycle" / "body.md").read_text(
        encoding="utf-8"
    )
    generated = (rendered_prompts_dir / "branch-lifecycle.prompt.md").read_text(
        encoding="utf-8"
    )
