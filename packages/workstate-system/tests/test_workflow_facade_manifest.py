...
WORKFLOW_DOC_REL = Path("docs") / "agentic" / "rules" / "development-workflow.md"
...
def _materialize_fixture_root(tmp_path: Path) -> Path:
    ...
    shutil.copy(PACKAGE_ROOT / MANIFEST_REL, root / MANIFEST_REL)
    shutil.copy(PACKAGE_ROOT / LIFECYCLE_MK_REL, root / LIFECYCLE_MK_REL)
    return root
...
def test_as_shipped_manifest_passes() -> None:
    ...
    proc = _run_facade_check(PACKAGE_ROOT)
...
def test_make_help_defers_step_order_to_workflow_doc() -> None:
    ...
    """
    workflow_doc = (PACKAGE_ROOT / WORKFLOW_DOC_REL).read_text(encoding="utf-8")
    assert "## Canonical Workflow Loop" in workflow_doc

    offenders: list[str] = []
    for mk_path in sorted((PACKAGE_ROOT / MAKEFILE_D_REL).glob("*.mk")):
        for line_no, line in enumerate(
            mk_path.read_text(encoding="utf-8").splitlines(), start=1
        ):
            if "## Workflow loop step" in line:
                rel_path = mk_path.relative_to(PACKAGE_ROOT)
