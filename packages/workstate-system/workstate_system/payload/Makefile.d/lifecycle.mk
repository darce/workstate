...
task-start: ## Create the task feature branch + linked worktree: TASK=<ref> OBJECTIVE="..." [PLAN=<glob>] [MODE=worktree|here] [LIFECYCLE_ARGS=--json]
	@WORKSTATE_WORKTREE_BOOTSTRAP_CMD='$(LIFECYCLE_WORKTREE_BOOTSTRAP)' $(LIFECYCLE) task-start $(if $(TASK),--task '$(TASK)') $(if $(OBJECTIVE),--objective '$(OBJECTIVE)') $(if $(SLUG),--slug '$(SLUG)') $(if $(MODE),--mode '$(MODE)') $(if $(PLAN),--plan '$(PLAN)') $(if $(PLAN_REVISION),--plan-revision '$(PLAN_REVISION)') $(LIFECYCLE_ARGS)
...
# errors-report (implementation note implementation note): harvest the agent_errors ledger into
...
errors-report: ## Cluster captured agent errors: [SOURCES="<db|jsonl> ..."] [SINCE="<ts>"] [ERRORS_ARGS=...]
	@mcp-workstate-handoff errors-report $(foreach s,$(SOURCES),--source '$(s)') $(if $(SINCE),--since '$(SINCE)') $(ERRORS_ARGS)
...
# Forwarded to the runner as WORKSTATE_WORKTREE_BOOTSTRAP_CMD on task-start
