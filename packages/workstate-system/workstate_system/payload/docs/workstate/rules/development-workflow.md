...
| ------------------------------------------ | --------------------------------- | ------- | ------------------------------------------------------------------------------- |
| `WORKSTATE_HANDOFF_COMPACTION_DISABLED`        | `AGENT_HANDOFF_COMPACTION_DISABLED`, `WORKSTATE_COMPACTION_DISABLED`       | unset   | Truthy value silences both surfaces: Stop hook short-circuits with `compaction skipped: disabled (source=env)` and `compute_compaction_advisory` returns `disabled=true,disabled_source="env"`. internal also exposes a writable DB equivalent via `make compaction-disable\|compaction-enable\|compaction-status [TASK=<ref>]` (workspace-default if `TASK` omitted). Resolver precedence: env → task row → workspace row → enabled. |
| `WORKSTATE_HANDOFF_COMPACTION_MIN_NEW_TURNS`   | `AGENT_HANDOFF_COMPACTION_MIN_NEW_TURNS`, `WORKSTATE_COMPACTION_MIN_NEW_TURNS`  | _(retired)_ | **Retired** as a trigger (implementation note / D1) — no longer read from env; field kept on `CompactionSettings` (default `0`) for backward compat only, no live gating. |
| `WORKSTATE_HANDOFF_COMPACTION_MIN_NEW_TOKENS`  | `AGENT_HANDOFF_COMPACTION_MIN_NEW_TOKENS`, `WORKSTATE_COMPACTION_MIN_NEW_TOKENS` | contract `threshold_tokens` (`50000`) | Sole trigger gate: write a compaction once the new-turn slice (cl100k_base) reaches the threshold. When unset, default resolves env → overlay `compaction.thresholds.tokens` → `harness-protocol.yaml` `compaction.threshold_tokens` → code constant. `0` = never fire the token gate (skip every turn); `1` = documented no-gate idiom; use `…_DISABLED` to silence entirely. |
| `WORKSTATE_HANDOFF_COMPACTION_THRESHOLD_TOKENS` | _(none)_ | contract `threshold_tokens` | Advisory-only token-threshold override (env > overlay > contract). Stop-hook gate uses `MIN_NEW_TOKENS` when set; otherwise overlay `compaction.thresholds.tokens`, then contract `threshold_tokens`. |
| `WORKSTATE_HANDOFF_COMPACTION_THRESHOLD_CHARS` | _(none)_ | contract `threshold_chars` | Advisory-only override for the char threshold (env > overlay > contract). |
| `WORKSTATE_HANDOFF_ACTIVE_TASK` | _(none)_ | unset | Pin active `task_ref` when multiple live rows share the workspace (hooks only; MCP writes still raise on ambiguity unless `task_ref` is explicit). |
...
reads ONLY the `WORKSTATE_HANDOFF_COMPACTION_*` names (no fallback). An alias
