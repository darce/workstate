...
For repos that adopt WORKSTATE-REF-73's eval surface, the root operator checks are:
...
2. Use a `target_branch=main` `WORKSTATE-REF-*` task for planning/review work that
...
## Workspace Cardinality and Task Kinds (WORKSTATE-REF-54)
...
- **Implementation tasks → per-worktree singleton (WORKSTATE-REF-66
...
### Task-Start Identity Resolution (WORKSTATE-REF-66)

WORKSTATE-REF-66 supersedes the pre-existing `workspace_ambiguous` veto with a
...
3. **Current branch resolved via registered live refs.** Uses
   ...
   importable from `workstate_protocol.branch_naming` (WORKSTATE-REF-65); falls
...
the WORKSTATE-REF-66 `_detect_real_conflict` path (i.e.
...
planning/maintenance discriminator, per CTP-WORKSTATE-REF-T-01):
...
**re-scoped** by WORKSTATE-REF-54 (CTP-WORKSTATE-REF-T-05 + CTP-PR-T3-01):
...
`post-merge` was retuned to `--mode doctor` in WORKSTATE-REF-53 implementation note so a
...
- The `<task-ref>` segment must contain at least one digit
  (e.g. `WORKSTATE-37`, `WORKSTATE-3`, `epic-12-3`).
- The `<slug>` is `[a-z0-9-]+`.

Examples that pass: `feature/WORKSTATE-37-pre-push-mirror`,
`feature/WORKSTATE-3-tool-surface`. Examples that reject:
`feature/bad-name` (no digit), `Feature/WORKSTATE-REF-37-x` (uppercase).
...
command line. Neither adapter is written by default — pass
`--install-claude-stop-hook` to write the shared (checked-in)
`<target>/.claude/settings.json` adapter, or
`--install-claude-stop-hook-local` to write the user-owned (gitignored)
`<target>/.claude/settings.local.json` adapter. Both can be combined.
Each merge is idempotent (sentinel-tagged `_managed_by: workstate-bootstrap`).

The PostToolUse(Bash) `capture-agent-errors` hook uses the same opt-in
pattern: pass `--install-claude-error-hook` for the shared
`<target>/.claude/settings.json` adapter or
`--install-claude-error-hook-local` for the user-owned
`<target>/.claude/settings.local.json` adapter. Example dogfood install:

```bash
make dogfood DOGFOOD_INSTALL_FLAGS="--install-claude-stop-hook --install-claude-error-hook"
```
...
Defaults preserve pre-WORKSTATE-REF-39 behavior: every turn-end with at least
...
| ------------------------------------------ | --------------------------------- | ------- | ------------------------------------------------------------------------------- |
| `AGENT_HANDOFF_COMPACTION_DISABLED`        | `WORKSTATE_COMPACTION_DISABLED`       | unset   | Truthy value silences both surfaces: Stop hook short-circuits with `compaction skipped: disabled (source=env)` and `compute_compaction_advisory` returns `disabled=true,disabled_source="env"`. WORKSTATE-REF-67 also exposes a writable DB equivalent via `make compaction-disable\|compaction-enable\|compaction-status [TASK=<ref>]` (workspace-default if `TASK` omitted). Resolver precedence: env → task row → workspace row → enabled. |
| `AGENT_HANDOFF_COMPACTION_MIN_NEW_TURNS`   | `WORKSTATE_COMPACTION_MIN_NEW_TURNS`  | `1`     | Skip when fewer new turns than the threshold (session-scoped delta).            |
| `AGENT_HANDOFF_COMPACTION_MIN_NEW_TOKENS`  | `WORKSTATE_COMPACTION_MIN_NEW_TOKENS` | `0`     | Skip when the new-turn slice tokenizes (cl100k_base) below the threshold.       |
...
`compaction(operation="status")` reporting `disabled=false` means the WORKSTATE-REF
...
`workstate-bootstrap install --install-claude-reinject-hook` writes the shared
managed `.claude/settings.json` SessionStart adapter, while
`--install-claude-reinject-hook-local` writes the user-owned
`.claude/settings.local.json` adapter. The hook is fail-open: disabled
...
- `<slug>` — short hyphen-or-underscore separated descriptor of the
  decision content (e.g. `WORKSTATE_37_pre_push_mirror`).

### Examples

```
claude_slice_complete_WORKSTATE-37_pre_push_mirror
hook_nonconforming_branch_override_WORKSTATE-37_late_commit
codex_branch_review_WORKSTATE-37_pass_with_findings
```
...
`sync-task-plan-checklist` handler (WORKSTATE-REF-64) keeps those boxes
...
### Audit and Historical Backfill (WORKSTATE-REF-70)

When the post-slice sync misses items — stale anchors, plans rewritten
mid-task, or pre-WORKSTATE-REF-64 plans that were never reconciled — two
...
historical WORKSTATE-REF-60 case), bare `Slice N` resolution is suppressed for
