...
def validate_event(
    ...
    *,
    ...
    Strict mode: when ``WORKSTATE_HOOK_PROTOCOL_STRICT=1`` is set in the
...
def is_protocol_validation_strict() -> bool:
    """Return True when WORKSTATE_HOOK_PROTOCOL_STRICT=1 is set.
    ...
    """
    return os.environ.get("WORKSTATE_HOOK_PROTOCOL_STRICT", "").lower() in {"1", "true", "yes"}
