...
def _deps_python() -> str:
    """Interpreter carrying the workstate stack deps, for Python subprocesses.

    Python hook handlers and the errors-record module fallback import the
    workstate stack; the launch ``python3`` may lack those deps after a system
...
def _errors_record_argv() -> list[str]:
    ...
    return [_deps_python(), "-m", "workstate_handoff_mcp", "errors-record"]
...
def main(argv: list[str]) -> int:
    ...
    if resolved.endswith(".sh"):
        ...
        # Python handlers may import the workstate stack; spawn them under a
