...
``workstate_protocol.branch_naming`` module when available, with a literal
...
def _load_task_ref_re() -> re.Pattern[str]:
    ...
    for parent in hook_path.parents:
        ...
        spec = importlib.util.spec_from_file_location(
            "_workstate_branch_naming_for_hook",
            candidate,
        )
    ...
    try:
        from workstate_protocol.branch_naming import TASK_REF_RE as task_ref_re  # noqa: PLC0415
    ...
    try:
        from workstate_handoff_mcp import TASK_REF_RE as task_ref_re  # noqa: PLC0415
