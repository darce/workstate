...
WORKSTATE-REF-21 / Layer 3 of the expected_revision bug class eradication. The
...
from shell heredocs by WORKSTATE-REF-20) and fails when any ``set_handoff_state``
...
def _collect_import_aliases(tree: ast.Module) -> dict[str, str]:
    ...
    This closes WORKSTATE-REF-21-BR-01: without the alias map, a renamed import
...
def find_violations(paths: Iterable[Path]) -> list[tuple[Path, int, str]]:
    ...
    for path in paths:
        try:
            ...
            continue
        # WORKSTATE-REF-21-BR-02: surface parse failures as violations instead of
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        ...
        "  WORKSTATE-REF-21 / expected_revision bug class eradication.",
        file=sys.stderr,
    )
    return 1
