...
WS-REINJ-01 implementation note (implementation note). Claude Code adds SessionStart hook
...
def _reinject(*, repo_root: str, budget_chars: int) -> int:
    ...
    try:
        with _get_db_connection() as conn:
            try:
                ...
                return 0
            # WORKSTATE-REF-67: a disabled compaction surface silences re-injection
    ...
    if latest is not None:
        lines.append(
            f"latest_compaction: {latest.compaction_id} "
            f"(turns {latest.turn_range.start_turn}-{latest.turn_range.end_turn})"
        )
    finding_ids = [
...
def main() -> int:
    ...
    return _reinject(repo_root=repo_root, budget_chars=budget_chars)
