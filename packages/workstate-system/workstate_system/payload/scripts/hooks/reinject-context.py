...
``session_reinjections`` telemetry row (including AB control arms that
suppress stdout with ``emitted_chars=0``); a failed write logs to stderr and
never blocks treatment emission.
...
- ``WORKSTATE_REINJECT_AB``            when ``1``, assign treatment/control arms
                                       from ``session_id`` hash parity for the
                                       slice-7 reorientation experiment
"""
...
def _reinject_ab_arm(session_id: str) -> str | None:
    """Return experiment arm when ``WORKSTATE_REINJECT_AB=1``, else ``None``."""
...
def _reinject(
    *,
    ...
    emitted_chars = 0
    if arm == "control":
        _emit("reinject suppressed: ab control arm")
    elif harness == "claude-code" and settings.compaction_notify:
