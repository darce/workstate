...
def test_warning_names_canonical_rule_and_override_path() -> None:
    ...
    assert "WORKSTATE_ALLOW_NONCONFORMING_BRANCH" in text
...
def test_pre_commit_blocks_non_conforming_branch() -> None:
    ...
    assert "feature/bad-name" in output
    assert "workstate_protocol.branch_naming.TASK_REF_RE" in output
    assert "WORKSTATE_ALLOW_NONCONFORMING_BRANCH" in output
...
def test_pre_commit_override_records_decision_and_allows(tmp_path: Path) -> None:
    """``WORKSTATE_ALLOW_NONCONFORMING_BRANCH=1`` allows the commit and
    ...
    code, output, calls = _capture_pre_commit(
        "feature/bad-name",
        env={
            "WORKSTATE_ALLOW_NONCONFORMING_BRANCH": "1",
            "WORKSTATE_ALLOW_NONCONFORMING_BRANCH_REASON": "release-cut",
        },
...
def test_pre_commit_override_falls_back_on_recorder_error(tmp_path: Path) -> None:
    ...
    code, output, calls = _capture_pre_commit(
        "feature/bad-name",
        env={"WORKSTATE_ALLOW_NONCONFORMING_BRANCH": "1"},
...
def test_pre_commit_override_writes_fallback_log_to_disk(tmp_path: Path) -> None:
    ...
    code = run_pre_commit(
        ...
        env={
            "WORKSTATE_ALLOW_NONCONFORMING_BRANCH": "1",
            "WORKSTATE_ALLOW_NONCONFORMING_BRANCH_REASON": "hotfix",
        },
        recorder=_failing_recorder,
...
def test_pre_commit_block_override_lines_are_one_var_per_line() -> None:
    ...
    required_lines = [ln for ln in lines if "WORKSTATE_ALLOW_NONCONFORMING_BRANCH=1" in ln]
    reason_lines = [ln for ln in lines if "WORKSTATE_ALLOW_NONCONFORMING_BRANCH_REASON" in ln]
...
def test_pre_push_blocks_non_conforming_branch() -> None:
    ...
    assert "feature/bad-name" in output
    assert "workstate_protocol.branch_naming.TASK_REF_RE" in output
    assert "WORKSTATE_ALLOW_NONCONFORMING_BRANCH_PUSH" in output
    assert calls == []


def test_pre_push_does_not_honor_commit_side_override_env() -> None:
    """Critical isolation invariant: WORKSTATE_ALLOW_NONCONFORMING_BRANCH
    ...
    code, output, calls = _capture_pre_push(
        ...
        env={
            "WORKSTATE_ALLOW_NONCONFORMING_BRANCH": "1",
            "WORKSTATE_ALLOW_NONCONFORMING_BRANCH_REASON": "set during commit",
        },
    )
...
def test_pre_push_override_records_decision_and_allows(tmp_path: Path) -> None:
    ...
    code, output, calls = _capture_pre_push(
        "feature/bad-name",
        env={
            "WORKSTATE_ALLOW_NONCONFORMING_BRANCH_PUSH": "1",
            "WORKSTATE_ALLOW_NONCONFORMING_BRANCH_PUSH_REASON": "publish-cut",
        },
        recorder=_fake_recorder,
        repo_root=tmp_path,
    )
...
def test_pre_push_override_falls_back_on_recorder_error(tmp_path: Path) -> None:
    ...
    code, output, calls = _capture_pre_push(
        "feature/bad-name",
        env={"WORKSTATE_ALLOW_NONCONFORMING_BRANCH_PUSH": "1"},
        recorder=_failing_recorder,
        repo_root=tmp_path,
    )
    assert code == 0
...
def test_pre_push_override_writes_fallback_log_to_disk(tmp_path: Path) -> None:
    ...
    code = run_pre_push(
        ...
        env={
            "WORKSTATE_ALLOW_NONCONFORMING_BRANCH_PUSH": "1",
            "WORKSTATE_ALLOW_NONCONFORMING_BRANCH_PUSH_REASON": "publish-fallback",
        },
        recorder=_failing_recorder,
        stream=stream,
        repo_root=tmp_path,
...
def test_pre_push_block_message_cites_canonical_rule_and_distinct_env_var() -> None:
    ...
    assert "workstate_protocol.branch_naming.TASK_REF_RE" in text
    assert "WORKSTATE_ALLOW_NONCONFORMING_BRANCH_PUSH" in text
...
def test_pre_push_block_override_lines_are_one_var_per_line() -> None:
    ...
    required_lines = [ln for ln in lines if "WORKSTATE_ALLOW_NONCONFORMING_BRANCH_PUSH=1" in ln]
    reason_lines = [ln for ln in lines if "WORKSTATE_ALLOW_NONCONFORMING_BRANCH_PUSH_REASON" in ln]
    assert len(required_lines) == 1, (
...
def test_git_push_blocked_on_non_conforming_branch_even_after_no_verify_commit(
    ...
    assert "BLOCKED" in combined
    assert "feature/bad-name" in combined
    assert "WORKSTATE_ALLOW_NONCONFORMING_BRANCH_PUSH" in combined
