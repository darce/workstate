...
def test_min_new_turns_threshold_skips(workspace: Path) -> None:
    """``WORKSTATE_HANDOFF_COMPACTION_MIN_NEW_TURNS`` larger than the transcript's
...
def test_invalid_int_env_var_emits_compaction_failed(workspace: Path) -> None:
    """Bad int values for ``WORKSTATE_HANDOFF_COMPACTION_MIN_NEW_TURNS`` must
    ...
    result = _run_hook(
        payload,
        workspace=workspace,
        extra_env={"WORKSTATE_HANDOFF_COMPACTION_MIN_NEW_TURNS": "abc"},
    )
...
def test_thresholds_default_match_pre_WORKSTATE_39_behavior(workspace: Path) -> None:
    """With no WORKSTATE_HANDOFF_COMPACTION_* env vars set, the hook must behave
