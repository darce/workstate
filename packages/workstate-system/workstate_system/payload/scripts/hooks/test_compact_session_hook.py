...
Strict-mode protocol violations (``WORKSTATE_HOOK_PROTOCOL_STRICT=1``
...
import pytest
from workstate_protocol import TurnRange
...
_COMPACTION_LOW_TOKEN_GATE = {"WORKSTATE_HANDOFF_COMPACTION_MIN_NEW_TOKENS": "1"}
...
def _prepare_source_imports() -> tuple[list[str], dict[str, ModuleType]]:
    ...
    # Some sibling tests may have already imported a stale workstate package
...
def _run_hook(
    ...
    *,
    ...
    if scrub_compaction_env:
        for key in list(env):
            if key.startswith("WORKSTATE_HANDOFF_COMPACTION_"):
                del env[key]
...
def test_compact_session_notify_claude_emits_system_message_envelope(
    ...
    assert envelope["systemMessage"].startswith("workstate: compacted C-internal-")
...
def test_compact_session_notify_off_skips_stdout_envelope(workspace: Path) -> None:
    """``WORKSTATE_HANDOFF_COMPACTION_NOTIFY=0`` suppresses stdout notify."""
    transcript = _write_transcript(
        workspace,
...
def test_compact_session_hook_uses_env_harness(workspace: Path) -> None:
    """The hook must derive the harness from WORKSTATE_HANDOFF_HARNESS so
    ...
    assert latest.summary.harness == "codex", (
        f"expected harness=codex from WORKSTATE_HANDOFF_HARNESS env; "
...
def test_disabled_env_var_skips(workspace: Path) -> None:
    """``WORKSTATE_HANDOFF_COMPACTION_DISABLED=1`` makes the hook emit a skip line
...
def test_min_new_turns_env_no_longer_gates_compaction(workspace: Path) -> None:
    ...
    result = _run_hook(
        payload,
        workspace=workspace,
        extra_env={"WORKSTATE_HANDOFF_COMPACTION_MIN_NEW_TURNS": "100"},
    )
...
def test_min_new_tokens_threshold_skips(workspace: Path) -> None:
    """``WORKSTATE_HANDOFF_COMPACTION_MIN_NEW_TOKENS`` larger than the encoded
    ...
    result = _run_hook(
        payload,
        workspace=workspace,
        extra_env={"WORKSTATE_HANDOFF_COMPACTION_MIN_NEW_TOKENS": "100000"},
    )
...
def test_workstate_compaction_disabled_skips(workspace: Path) -> None:
    """``WORKSTATE_HANDOFF_COMPACTION_DISABLED=1`` is the canonical
...
def test_invalid_int_env_var_emits_compaction_failed(workspace: Path) -> None:
    """Bad int values for ``WORKSTATE_HANDOFF_COMPACTION_MIN_NEW_TOKENS`` must
    ...
    result = _run_hook(
        payload,
        workspace=workspace,
        extra_env={"WORKSTATE_HANDOFF_COMPACTION_MIN_NEW_TOKENS": "abc"},
    )
...
def test_thresholds_default_match_plan_0037_50k_gate(workspace: Path) -> None:
    """With no WORKSTATE_HANDOFF_COMPACTION_* env vars set, the hook must
...
def test_compact_session_hook_workspace_ambiguous_shape_tiebreaks(
    ...
    assert "WORKSTATE_HANDOFF_ACTIVE_TASK" in result.stderr
...
def test_compact_session_hook_workspace_ambiguous_pinned_task_ref(
    ...
    """``WORKSTATE_HANDOFF_ACTIVE_TASK`` overrides tiebreak when set."""
    transcript = _write_transcript(
        workspace_ambiguous_tasks,
        "turn 1 user: hello\nturn 2 assistant: world\n",
    )
    ...
    result = _run_hook(
        ...
        extra_env={
            **_COMPACTION_LOW_TOKEN_GATE,
            "WORKSTATE_HANDOFF_ACTIVE_TASK": "AMBIG-A",
        },
    )
    assert result.returncode == 0, result.stderr
...
def test_min_new_tokens_zero_disables_gate(workspace: Path) -> None:
    """implementation note D5: ``WORKSTATE_HANDOFF_COMPACTION_MIN_NEW_TOKENS=0`` means
    *never fire* the token gate — every turn is skipped and no row is written.
    ...
    result = _run_hook(
        payload,
        workspace=workspace,
        extra_env={"WORKSTATE_HANDOFF_COMPACTION_MIN_NEW_TOKENS": "0"},
    )
...
def test_compact_session_skip_keeps_empty_stdout_under_claude(workspace: Path) -> None:
    ...
    ``WORKSTATE_HANDOFF_HARNESS=claude-code`` with notify on (default).
...
def test_jsonl_turn_derivation_hook_matches_library(workspace: Path) -> None:
    ...
    from workstate_handoff_mcp.compaction import _derive_turn_range, slice_new_turn_transcript
    ...
    from workstate_handoff_mcp import CompactionSettings, get_latest_compaction
    from workstate_handoff_mcp.compaction import _process_transcript_text
