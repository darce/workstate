...
def test_path_filter_includes_packages_docs_tasks() -> None:
    assert _path_should_be_scanned(
        "packages/mcp-workstate-orchestrator/docs/tasks/internal-tool-surface-consolidation-task-plan.md"
    )
...
def test_path_filter_excludes_instructions_md() -> None:
    assert not _path_should_be_scanned("docs/workstate/instructions.md")
