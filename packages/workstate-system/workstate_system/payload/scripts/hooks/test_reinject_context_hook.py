...
def test_reinject_telemetry_write_failure_still_emits(
    ...
    from workstate_handoff_mcp import compaction as compaction_mod
...
def test_reinject_ab_flag_off_records_null_arm(workspace: Path) -> None:
    ...
    assert result.returncode == 0
    assert _stdout_injection(result).startswith("```workstate-reinject")
    telemetry = _latest_reinjection_row()
    assert telemetry is not None
...
def test_reinject_ab_treatment_arm_emits_block(workspace: Path) -> None:
    """WORKSTATE_REINJECT_AB=1 treatment parity emits block + arm=treatment."""
    _seed_compaction_row(workspace)
    result = _run_hook(
        _payload("compact", session_id=_AB_TREATMENT_SESSION),
        workspace=workspace,
        extra_env={"WORKSTATE_REINJECT_AB": "1"},
    )
...
def test_reinject_ab_unknown_session_records_null_arm(workspace: Path) -> None:
    ...
    result = _run_hook(payload, workspace=workspace, extra_env={"WORKSTATE_REINJECT_AB": "1"})
...
def test_reinject_ambiguous_shape_pinned_task_ref(
    ...
    result = _run_hook(
        ...
        extra_env={"WORKSTATE_HANDOFF_ACTIVE_TASK": "AMBIG-A"},
    )
...
# ---------------------------------------------------------------------------
# internal — semantic top-K concept reinjection (WORKSTATE_REINJECT_SEMANTIC, default off)
# ---------------------------------------------------------------------------
...
class _FakeProvider:
    ...
    def embed(self, texts):
        import numpy as np
        from workstate_handoff_mcp.embeddings.store import text_hash
...
def _block_content_lines(stdout: str) -> list[str]:
    return [line for line in stdout.splitlines() if line not in ("```workstate-reinject", "```")]
...
def test_reinject_semantic_emits_ranked_concepts(
    ...
    """Flag on + provider + seeded embeddings: block carries a ``relevant:`` line
    with concepts ranked by cosine to the composed anchor (focus drives it)."""
    from workstate_handoff_mcp import get_handoff_state, set_handoff_state
    from workstate_handoff_mcp.compaction import CompactionSettings
    from workstate_handoff_mcp.embeddings import store as embedding_store
    from workstate_handoff_mcp.embeddings.store import store_concept_embedding
    from workstate_handoff_mcp.shared_schema import _get_db_connection
    ...
    mod = _load_reinject_module()
    try:
        focus_text = "semantic target focus marker xyzzy"
        identity = get_handoff_state(task_ref=TASK_REF, sections="identity")
        revision = identity["data"]["active"]["revision"]
        assert set_handoff_state(
            task_ref=TASK_REF, focus=focus_text, status="in_progress", expected_revision=revision
        ).get("ok")

        with _get_db_connection() as conn:
            store_concept_embedding(conn, provider, "decision.rationale", "777", TASK_REF, focus_text)
            store_concept_embedding(conn, provider, "finding.description", "999", TASK_REF, "utterly unrelated noise")
            conn.commit()
    ...
    assert "relevant:" in out, f"expected a semantic line; stdout={out!r}"
    assert "decision.rationale:777" in out
    assert out.index("decision.rationale:777") < out.index("finding.description:999")
...
def test_reinject_semantic_line_respects_byte_budget(
    ...
    try:
        ...
        identity = get_handoff_state(task_ref=TASK_REF, sections="identity")
        set_handoff_state(
            task_ref=TASK_REF, focus=focus_text, status="in_progress",
            expected_revision=identity["data"]["active"]["revision"],
        )
        with _get_db_connection() as conn:
            store_concept_embedding(
                conn, provider, "decision.rationale", "a-very-long-finding-identifier-0001", TASK_REF, focus_text
            )
            conn.commit()
        mod._reinject(
            repo_root=str(workspace), budget_chars=4000, settings=CompactionSettings(),
    ...
    assert small.startswith("```workstate-reinject") and small.rstrip().endswith("```")
...
def test_reinject_ab_arm_governs_semantic_selection(
    ...
    """internal: with WORKSTATE_REINJECT_AB=1 the single in-hook arm
    governs selection — treatment (arm B) forces semantic top-K, control (arm A)
    forces current selection — overriding the standalone WORKSTATE_REINJECT_SEMANTIC
    ...
    monkeypatch.setenv("WORKSTATE_HANDOFF_HARNESS", "codex")
    monkeypatch.setenv("WORKSTATE_REINJECT_AB", "1")
    ...
    assert "decision.rationale:777" in treatment_out
    ...
    assert control_out.startswith("```workstate-reinject")
