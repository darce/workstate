"""Tests for the reinject-context SessionStart hook (implementation note of WS-REINJ-01).
...
TASK_REF = "WS-REINJ-01-HOOK-TEST"
...
def test_reinject_emits_block_on_compact_source(workspace: Path) -> None:
    ...
    block = result.stdout
    assert block.startswith("```workstate-reinject"), f"stdout={block!r}"
...
def test_reinject_emits_block_on_resume_source(workspace: Path) -> None:
    ...
    assert TASK_REF in result.stdout
...
def test_reinject_block_without_compaction_row_still_emits_state(
    ...
    assert result.stdout.startswith("```workstate-reinject")
    assert TASK_REF in result.stdout
    assert "latest_compaction" not in result.stdout
...
def test_reinject_budget_truncation(workspace: Path) -> None:
    ...
    result = _run_hook(
        _payload("compact"),
        workspace=workspace,
        extra_env={"WORKSTATE_REINJECT_BUDGET_CHARS": str(budget)},
    )
...
def test_reinject_minimal_budget_always_carries_task_ref(workspace: Path) -> None:
    ...
    result = _run_hook(
        _payload("compact"),
        workspace=workspace,
        extra_env={"WORKSTATE_REINJECT_BUDGET_CHARS": str(floor)},
    )
...
def test_reinject_sanitizes_fence_tokens_in_field_values(
    ...
    assert result.returncode == 0, result.stderr
    block_lines = result.stdout.rstrip("\n").splitlines()
...
def test_reinject_disable_resolver_silences(workspace: Path) -> None:
    """A disabled compaction surface (WORKSTATE-REF-67 unified resolver) also
