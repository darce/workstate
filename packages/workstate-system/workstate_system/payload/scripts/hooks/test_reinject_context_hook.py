...
def test_reinject_ab_control_arm_suppresses_emission(workspace: Path) -> None:
    """WORKSTATE_REINJECT_AB=1 control parity suppresses stdout but logs telemetry."""
    _seed_compaction_row(workspace)
    result = _run_hook(
    ...
    assert "reinject suppressed: ab control arm" in result.stderr
    telemetry = _latest_reinjection_row()
    assert telemetry is not None
    assert telemetry["session_id"] == _AB_CONTROL_SESSION
    assert telemetry["arm"] == "control"
    assert int(telemetry["emitted_chars"]) == 0
