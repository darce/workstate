"""``close-check`` subcommand (implementation note implementation note).
...
def _probe_dirty_main_protected_paths(repo: Path) -> list[str]:
    ...
    WORKSTATE-REF-53 implementation note (finding WORKSTATE53-S4-BR-01): post-merge was retuned
...
def run(argv: list[str]) -> int:
    ...
    # WORKSTATE-REF-53 implementation note: close-only dirty-main probe.
    dirty_main_paths = _probe_dirty_main_protected_paths(repo)
    if dirty_main_paths:
        reasons.append("dirty_main_protected_paths")

    # WORKSTATE-REF-70 implementation note: close-only checklist guardrail. Blocks merge
