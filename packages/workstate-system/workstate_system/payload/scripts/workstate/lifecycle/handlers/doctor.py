...
# (WORKSTATE-REF-DOCTOR-PROBE-BUDGET-20260605). Operators can tune the budget via
...
def _load_hook_adapter_decls() -> list[tuple[str, dict[str, Any]]]:
    ...
    WS-REINJ-01 implementation note generalized the compact-session-only loader so the
...
def _probe_hooks(
    ...
    *,
    ...
    ``hooks[]`` family since WS-REINJ-01 implementation note, reported per family in the
    additive ``hook_adapters`` field while the legacy ``stop_adapters_*``
    keys keep their compact-session-only meaning (WORKSTATE-REF-80 contract).
    ...
    for hook_id, adapter in _load_hook_adapter_decls():
        ...
        # keeps the WORKSTATE-REF-80 "stop adapter" wording other tooling greps for.
        noun = (
            "stop adapter"
