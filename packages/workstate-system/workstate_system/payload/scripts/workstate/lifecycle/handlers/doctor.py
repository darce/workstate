...
def _stop_adapter_state(repo: Path, target: str, harness: str) -> str:
    """Classify the managed compact-session Stop entry at ``repo/target``.

    Returns ``"installed"``, ``"stale"`` (managed entry present but in a
    shape the harness ignores — e.g. the pre-fix flat Claude entry), or
    ``"absent"``.
    """
    path = repo / target
    try:
...
def _probe_hooks(
    ...
    *,
    ...
    for adapter in _load_compact_session_adapters():
        ...
        state = _stop_adapter_state(repo, target_path, str(adapter.get("harness", "")))
        if target_path in drifted_paths:
            ...
            drifted.append(key)
            remediation.append(
                f"re-install the {key} stop adapter (stale pre-fix flat shape "
