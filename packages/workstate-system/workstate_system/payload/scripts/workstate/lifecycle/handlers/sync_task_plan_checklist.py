...
def build_evidence_from_handoff_payload(
    ...
    # e.g. ``internal``). Lazy match the work_ref so the first
...
def resolve_workspace_root(plan_path: Path | None = None) -> Path:
    ...
    ``packages/mcp-workstate-handoff/docs/tasks/internal-...``), but relative
...
def _resolve_state_dir(repo: Path) -> Path:
    ...
    Honors ``WORKSTATE_HANDOFF_STATE_DIR`` (the same override the
    ...
    """
    override = os.environ.get("WORKSTATE_HANDOFF_STATE_DIR")
    if override:
