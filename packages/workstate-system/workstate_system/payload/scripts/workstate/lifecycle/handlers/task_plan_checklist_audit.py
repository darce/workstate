"""``task-plan-checklist-audit`` subcommand (WORKSTATE-REF-70 implementation note).

Read-only inspection of task-plan `- [ ]` / `- [x]` state against
recorded handoff evidence. Reuses the WORKSTATE-REF-64 ``sync_task_plan_checklist``
...
2. Archived snapshot lookup of the same fields (archived tasks are
   in scope — the WORKSTATE-REF-58..68 historical set is largely archived).
3. Filesystem glob ``<workspace_root>/packages/*/docs/tasks/<TASK_REF>-*-task-plan.md``
   so colliding task_refs (e.g. WORKSTATE-REF-60 in two packages) surface as
...
def _discover_plan_paths_by_glob(
    ...
    plans sharing one task_ref (the WORKSTATE-REF-60 collision) both appear.
...
def run(argv: list[str]) -> int:
    ...
    for raw_plan in plans:
        ...
        # Derive task_ref from filename: WORKSTATE-REF-67-... -> WORKSTATE-REF-67.
        ...
        if m:
            ...
            # number, so "WORKSTATE-REF-67" wins over just "WORKSTATE-REF".
