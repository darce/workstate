"""``task-plan-checklist-backfill`` subcommand (WORKSTATE-REF-70 implementation note).

Bulk historical reconciler: discovers task plans for the supplied
``--tasks`` refs (and / or ``--plans`` paths), runs the WORKSTATE-REF-64
...
one plan file (the WORKSTATE-REF-60 shape — two unrelated plans both using
``WORKSTATE-REF-60``), the bare ``Slice N`` anchor stops being authoritative —
