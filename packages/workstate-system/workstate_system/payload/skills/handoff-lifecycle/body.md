...
7a. To silence internal compaction at runtime (advisory + Stop hook together), use the unified disable surface (internal) — do not edit YAML. Workspace-default: `make compaction-disable` (or `compaction(operation="disable")`). Per-task: `make compaction-disable TASK=<ref>`. Re-enable with `compaction-enable`; inspect with `compaction-status [TASK=<ref>]`. The `WORKSTATE_HANDOFF_COMPACTION_DISABLED` env var still works and takes precedence over DB rows. When disabled, advisory envelopes carry `disabled=true, disabled_source=<env|db>` and the dashboard's Needs Attention prints `compaction: disabled via <source>`. Host-harness compaction (Claude Code, Codex, etc.) is unaffected.
...
- [../../../docs/workstate/in-session-skills.md](../../../docs/workstate/in-session-skills.md) — operator skills callable in-session (not portable `/commands`)
- [../../../docs/workstate/lifecycle-map.md](../../../docs/workstate/lifecycle-map.md)
- [../../../docs/workstate/instructions.md](../../../docs/workstate/instructions.md)
