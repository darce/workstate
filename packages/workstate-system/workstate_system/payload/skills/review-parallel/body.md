...
An **unreachable orchestrator MCP is itself a negative signal**, not a separate failure mode: if `list_available_backends` errors because `workstate-orchestrator-mcp` never launched (e.g. the pinned `uvx mcp-workstate-orchestrator[bridge]@<ver>` environment cannot be built on an offline host or a mirror without the bridge wheel), take the same deterministic fallback. The fallback is reachable by construction because it runs in the coordinator's own process — the coordinator performs the two sequential read-only review passes itself (Claude Code may also use its in-process `Agent` tool) and merges as usual; it must not route through `run_structured_turn` or any other tool on the orchestrator server. Record the same `routing_downgrade` decision, with the rationale naming the unreachable server. Only the orchestrator-recorded audit extras (lane activity, turn metrics) are lost in this mode; the handoff MCP write path (`review_findings`, `review_runs`, `record_event`) is a different server and stays available.
