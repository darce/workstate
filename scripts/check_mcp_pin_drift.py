...
implementation note (docs/plans/0028-mcp-registration-single-source-ownership.md):
...
Canonical pin site (authoritative):
  1. ``packages/workstate-system/workstate_system/payload/config/agent-workflows/mcp_servers.yaml``
     — ``mcp_servers[].args[0] == "<distribution>@<version>"``

Generated copy (must match; regenerate via ``make mcp-pins-sync``):
  2. ``packages/workstate-bootstrap/src/workstate_bootstrap/_mcp_pins.py``
...
Coupled surfaces (also verified so a stale doc cannot ship):
  ...
  4. ``docs/CONSUMER.md``
     — the consumer-facing MCP-server registration table (WS-PKGDOCS-01).
...
PIN_SURFACES: tuple[PinSurface, ...] = (
    ...
    PinSurface(
        ...
        relpath="packages/workstate-bootstrap/src/workstate_bootstrap/_mcp_pins.py",
        primary=False,
        generated=True,
    ),
    PinSurface(
