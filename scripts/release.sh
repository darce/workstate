...
# triggers `gh workflow run release-publish.yml -R darce/workstate` for OIDC upload.
# Requires `release-public --execute` to have synced sources to darce/workstate first.
...
cmd_preflight() {
    ...
    log "preflight: workstate-stack pin gate"
    if ! python scripts/stack_pins.py check; then
        warn "workstate-stack pins drift from member versions — run 'make stack-pins-sync' (+ stack version bump/CHANGELOG) before releasing."
        if [[ $started_run -eq 1 ]]; then
            write_release_summary_artifacts "preflight" "" "failed"
        fi
        return 1
...
dispatch_published_bytes_workflow() {
    ...
    if [[ $DRY_RUN -eq 1 ]]; then
        ...
        printf '\033[1;90m[dry-run]\033[0m %s\n' \
            "gh workflow run release-publish.yml -R darce/workstate -f package=$pkg -f dry_run=$dry_run_flag -f expected_sha256_wheel=<wheel> -f expected_sha256_sdist=<sdist> -f source_date_epoch=$source_date_epoch"
        return 0
    fi
    ...
    run "gh workflow run release-publish.yml -R darce/workstate -f package=$pkg -f dry_run=$dry_run_flag -f expected_sha256_wheel=$wheel_sha -f expected_sha256_sdist=$sdist_sha -f source_date_epoch=$source_date_epoch"
}
...
cmd_validate_gate() {
    ...
    if ! command -v gh &>/dev/null; then
        die "gh CLI not found — validate-gate dispatches release-publish.yml on darce/workstate"
    fi
...
cmd_package() {
    ...
    # publish while the workstate-stack pins still say the previous version.
    if ! python scripts/stack_pins.py check; then
        die "workstate-stack pin gate failed — run 'make stack-pins-sync' (+ stack version bump/CHANGELOG) so the meta-package anchors this release."
    fi

    # Pin archive timestamps so the locally gated dist is bit-comparable to the
    # runner rebuild on darce/workstate (implementation note A1-bit).
    local source_date_epoch
    source_date_epoch="$(git log -1 --format=%ct HEAD)"
    ...
    # Publish via Trusted Publishing on darce/workstate (primary path).
    # Requires `release-public --execute` to have synced the source + tag to
    # darce/workstate before this step. The workflow uses GitHub OIDC — no local
    # PyPI token needed.
    if ! command -v gh &>/dev/null; then
