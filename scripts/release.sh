...
cmd_preflight() {
    ...
    log "preflight: shipped-surface privacy gate"
    if ! python scripts/check_shipped_privacy.py; then
        warn "a publishable wheel ships personal info or internal project ids — run 'python scripts/scrub_shipped_source.py --apply' (+ version bump/CHANGELOG) before releasing."
        if [[ $started_run -eq 1 ]]; then
            write_release_summary_artifacts "preflight" "" "failed"
        fi
        return 1
...
cmd_package() {
    ...
    # Fail closed on personal info / internal project ids in the shipped wheel.
    if ! python scripts/check_shipped_privacy.py; then
        die "shipped-surface privacy gate failed — a wheel ships personal info or internal project ids; run 'python scripts/scrub_shipped_source.py --apply' (+ bump/CHANGELOG) before releasing."
    fi
    ...
    run "gh workflow run release-publish.yml -R darce/workstate -f package=$pkg -f dry_run=false"
