...
def main() -> int:
    ...
    # Gate: a managed MCP server (mcp-workstate-orchestrator / -handoff) must
    # not be bumped while its hand-maintained uvx pins still point at the old
    # version. update_dependency_floors below only touches pyproject floors, not
    # the two pin sites, so without this the bump would ship the same drift that
    # the v0.1.22 release did. Runs before any write (dry-run included) so the
    # operator updates the pins + their drift-guard tests + the doc deliberately.
